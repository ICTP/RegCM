...
!
  subroutine sethet(j,zmid1,phis1,tfld1,cmfdqr1,nrain1, &
                    nevapr1,delt,xhnm1,qin1,platl,ps2)
!
...
! I/O VARIABLES
  ...
  integer, intent(in) :: platl            ! lat dim size
  real(dp), intent(inout) :: phis1(:)      ! surf geopotential (m2/s2)
  real(dp), intent(inout) :: zmid1(:,:)    ! midpoint geopot convert (m) to (km)
  real(dp), intent(inout) :: tfld1(:,:)    ! temperature (K)
  ! dq/dt for convection (kg/kg/s) convert to (1/s) below
  real(dp), intent(inout) :: cmfdqr1(:,:)
  ! stratoform precip (1/s)
  real(dp), intent(inout) :: nrain1(:,:)
  ! evaporation (kg/kg/s) convert to (1/s) below
  real(dp), intent(inout) :: nevapr1(:,:)
  ! time step ( s )
  real(dp), intent(inout) :: delt
  ! total atms density (kg/m3) convert to (#/cm^3) below
  real(dp), intent(inout) :: xhnm1(:,:)
  ! exported species ( mmr )
  real(dp), intent(inout) :: qin1(:,:,:)
  ! Pressure ( cb )
  real(dp), intent(inout) :: ps2(:)
  ! LOCAL VARIABLES and CONSTANTS
  real(dp) :: phis(platl)        ! surf geopotential (m2/s2)
  real(dp) :: zmid(platl,kz)     ! midpoint geopot convert (m) to (km)
  real(dp) :: tfld(platl,kz)     ! temperature (K)
  real(dp) :: cmfdqr(platl,kz)   ! dq/dt for convection (1/s) 
  real(dp) :: nrain(platl,kz)    ! stratoform precip (1/s)
  real(dp) :: nevapr(platl,kz)   ! evaporation (1/s)
  real(dp) :: xhnm(platl,kz)     ! total atms density (#/cm^3)
  real(dp) :: qin(platl,kz,ntr)  ! exported species ( mmr )
  real(dp) :: temp_dep(platl)    ! temp var of wet dep rate
                                 ! (centibars_tracer/sec)
  real(dp) :: het_rates(platl,kz,ntr)  ! Effective henry's law constant (1/s)
  real(dp) :: vmr_hno3(platl,kz)       ! volume mixing ratio
  real(dp) :: vmr_h2o2(platl,kz)       ! volume mixing ratio
  ...
  integer :: i , m , k , kk , itr ! indicies
  real(dp) :: latitude     ! latitude in radians
  ...
  real(dp) , dimension(platl) :: xk0 , work1 , work2 , work3 , zsurf
  real(dp) , dimension(kz) :: xgas1, xgas2
  real(dp) , dimension(platl) :: tmp0_rates , tmp1_rates
  real(dp) , dimension(platl,kz) :: delz  ! layer depth about interfaces (cm)
  real(dp) , dimension(platl,kz) :: xhno3 ! hno3 concentration (molecules/cm^3)
  real(dp) , dimension(platl,kz) :: xh2o2 ! h2o2 concentration (molecules/cm^3)
  real(dp) , dimension(platl,kz) :: xliq  ! liquid rain water content in a
                                          ! grid cell (gm/m^3)
  real(dp) , dimension(platl,kz) :: rain ! conversion rate of water vapor into
                                         ! rain water (molecules/cm^3/s)
  real(dp) , dimension(platl,kz) :: xhen_hno3 , xhen_h2o2 , xhen_ch2o , &
    xhen_ch3ooh , xhen_ch3co3h , xhen_ch3cocho , xhen_xooh , xhen_onitr , &
    xhen_ho2no2 , xhen_glyald , xhen_ch3cho , xhen_mvk , xhen_macr
  real(dp) , dimension(platl,kz) :: xhen_nh3 , xhen_ch3cooh
  real(dp) , dimension(platl,kz,2) :: tmp_hetrates
  real(dp) , dimension(platl,kz) :: precip
  ...
  !-----------------------------------------------------------------
  cmfdqr = cmfdqr1(2:iym2,:)*(xhnm1(2:iym2,:)/rhoh2o) ![kg_h2o/kg_air/s -> 1/s]
  nevapr = nevapr1(2:iym2,:)*(xhnm1(2:iym2,:)/rhoh2o) ![kg_h2o/kg_air/s -> 1/s]
  zmid   = zmid1(2:iym2,:)*m2km                          ![m     -> km]
  xhnm   = xhnm1(2:iym2,:)*navgdr*cm3_2_m3/( amd*d_1000) ![kg/m3 -> #/cm3]
  phis   = phis1(2:iym2)
  qin    = qin1(2:iym2,:,:)
  tfld   = tfld1(2:iym2,:)
  nrain  = nrain1(2:iym2,:)
...
!
  ...
  real(dp) , dimension(iy,kz,mbin) :: colef , wetdep , rhsize ,rhop
  real(dp) , dimension(ntr) :: wetrem , wetrem_cvc 
  real(dp) :: agct , agingtend1 , agingtend2 , wtend
  integer :: n , k , i
  ...
  call  blcld (j,mbin,indp,rhsize,t,pressg,shj,rho,totppt,pdepv,rhop, &
               wetdep,colef)
  ...
  subroutine blcld(j,mbin,indp,rhsize,t,pressg,shj,rho,totppt,pdepv, &
                   rhop,wetdep,colef)
    implicit none 
    ...
    real(dp) :: totppt(iy,kz)

    real(dp) :: t(iy,kz) 
    real(dp) :: pres0sg(iy) , shj(kz) , rho(iy,kz) 
    real(dp) :: pdepv(iy,kz,ntr) ! care ,ntr dimension
    real(dp) :: rhsize(mbin) , rhop(mbin) 
    real(dp) :: totcld(iy,kz) 
    real(dp) :: colef(iy,kz,mbin) , wetdep(iy,kz,mbin) 

    integer :: l , i , ntp , n , nn , k
    intent(in) j , mbin , rhsize , t , pressg , shj , rho ,  &
               totppt ,  pdepv , rhop
    intent(out) wetdep , colef
      ...
      do k = 1 , kz
        do i = 2 , iym2
          if ( totppt(i,k) > 1.D-20 ) then 
            tl = t(i,k)-tzero
            !c----------------------------------------------------------c
            !c rain scavenging rate : wetdep in s-1   ?                 c
            !c units of totppt (mm.s-1) converted to m.s-1 (by 1.0e-3)  c
            !c----------------------------------------------------------c
            if ( tl > d_zero ) then 
              rrm = 0.35D0*(totppt(i,k)*3600.0D0)**0.25D0*1.0D-3 
              !c formulation for monoddipesre rain of radius rrm
              !c (cf seinfeld), gives a scavenging rate wetdep in
              !c s-1 ?? bcrain is 0.5 when it is 1.5 in seinfeld ?? 
              wetdep(i,k,n) = bcrain*totppt(i,k)*1.0D-3*colef(i,k,n)/rrm 
            end if 
            !c---------------------------------------------------------c
            !c snow scavenging rate                                    c
            !c * density of snow is set as 1/10 of liquid water.       c
            !c * factor of 1.0e-2 in wetdep calculation takes this and c
            !c * unit                                                  c
            !c      change (to m.s-1) into account                     c
            !c---------------------------------------------------------c 
            !c . . . . snow scavenging
...
!!$
!!$c*********************************************************************** 
!!$c****  a e r o s o l   c o l l e c t i o n   e f f i c i e n c y    ****
!!$c***********************************************************************
!!$c . . . calculation of the collection efficiency of the aerosols of  . .
!!$c . . . type n by collector droplets of radius rcol  . . . . . . . . . . 
!!$c . . . . . . . . . . . . . . . . . . . . . . . . . . . .  . . . . . . . 
!!$c . . . dec 19/96 - s.l.gong - vectorised the whole program & added  . .
!!$c . . . . . . . . . . . . . .  some working space  . . . . . . . . . . . 
...
!!$
  ...
  integer :: mbin
  real(dp) :: t(iy,kz), rhop(iy,kz,mbin), roarow(iy,kz) 
  real(dp) :: totppt(iy,kz), rho(iy,kz)
  real(dp) :: vpr(iy) ! average settling velocity
  real(dp) :: rhsize(iy,kz,mbin) 
  real(dp) :: pressg(iy) 
  real(dp) :: pdepv(iy,kz,ntr), shj(kz)
  integer , dimension(mbin) , intent(in) :: indp 
  real(dp) :: pdiff
  real(dp) :: colef(iy,kz,mbin) !collection efficiency
  real(dp) :: amu , anu !dynamic viscosity of air
  real(dp) :: amfp   ! mean molecular free path
  real(dp) :: schm   ! schmidt number
  real(dp) :: prii
  real(dp) :: priiv 
  real(dp) :: cfaca  ! cunningham slip correction factor
  real(dp) :: cfac
  ...
  real(dp) :: sstar 
  real(dp) :: colimp,pre
  real(dp) :: rrm(iy) ! mass mean raindrop radius (for rain)
  ...
  real(dp) , parameter :: tmin = 1.0D-10
  real(dp) , parameter :: rgasi = rgas/d_1000*amd
  real(dp) , parameter :: zcons1 = 5.2D0
    ...
    do k = 1 , kz
      do i = 2 , iym2
        ...
        if ( totppt(i,k) > 1.D-15 ) then
          ...
          schm = anu/pdiff 
          !c----------------------------------------------------c
          !c collector drop size                                c
          !c----------------------------------------------------c
          !c----------------------------------------------------c
          !c rain scavenging rate                               c
          !c units of totppt (mm.s-1) converted to m.s-1        c
          !c rrm = mass mean raindrop radius                    c
          !c----------------------------------------------------c
          if ( t(i,k) > tzero ) then
            rrm(i) = 0.35D0*(totppt(i,k)*3600.0D0)**0.25D0*1.D-3 
            !c what empirical parametization is that ?
            ! needs apparently rain rate in mm/hr 
            prii = 2.0D0/9.0D0*egrav/amu
            priiv = prii*(rhorain-rho(i,k)) 
            !c cunningham settling slip-correction factor
            !c settling velocity
            cfac = d_one+amfp/rrm(i)*(aa1r+aa2r*dexp(-aa3r*rrm(i)/amfp))
            vpr(i) = priiv*rrm(i)**d_two*cfac
          end if 
          !c----------------------------------------------------c
          !c snow scavenging                                    c
          !c data from slinn (1984) in atmospheric science &    c
          !c power production, ed. darryl randerson             c
          !c density of snow is set as 1/10 of liquid water.    c
          !c factor of 1.0e-2 in wetdep calculation takes this  c
          !c and unit change (to m.s-1) into account            c
          ...
          ! reynolds number: 
          re = rrm(i)*vpr(i)*rho(i,k)/amu
          ! stokes number of collected particles: 
          st = d_two*pdepv(i,k,indp(n))/egrav*(vpr(i) - &
               pdepv(i,k,indp(n)))/(d_two*rrm(i))
          ...
          colef(i,k,n) = (d_one/schm)**alpha(i) +  &
