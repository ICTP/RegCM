...
!EOP
    ...
    real(rk8) :: dtime                 ! time step increment (sec)
    integer(ik4)  :: nstep                 ! model time step
    real(rk8) :: calday                ! calendar day for nstep
    real(rk8) :: caldaym1              ! calendar day for nstep-1
    real(rk8) :: declin                ! solar declination angle in radians for nstep
    real(rk8) :: declinm1              ! solar declination angle in radians for nstep-1
...
!----------------------------------------------------------------------
    ...
    if (myid == italk) then
       ...
       write(stdout,*) '   nstep= ',get_nstep(), ' year= ',yr,' month= ',mon,&
