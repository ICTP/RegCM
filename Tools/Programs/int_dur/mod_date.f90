...
module mod_date
  ...
  subroutine set_timeunit(x, u)
    ...
    select case (u)
      ...
      case (ucnt)
        x%iunit = u
      case default
        write (0,*) 'Unknown time unit, assuming hours'
        x%iunit = uhrs
    end select
  ...
  subroutine print_rcm_time_interval(x)
    ...
    write (6,'(i16," ",a)') x%ival, trim(cintstr(x%iunit))
  ...
  subroutine check_cal(x,y)
    ...
    if ( x%calendar /= y%calendar ) then
      write (0,*) 'X calendar = ', calstr(x%calendar)
      write (0,*) 'Y calendar = ', calstr(y%calendar)
      write(0,*) 'Dates not comparable: not using same calendar.'
      stop
    end if
  ...
  integer(ik4) function idayofweek(x) result(iday)
    ...
    if (x%calendar /= gregorian) then
      write(0,*) 'Error: week concept works only for gregorian calendar.'
      stop
    end if
    call days_from_reference_to_date(x,d)
