...
module mod_clm_cndvlight
  ...
  subroutine Light(lbg, ubg, lbp, ubp, num_natvegp, filter_natvegp)
    ...
    real(rkx), pointer :: dsladlai(:) !dSLA/dLAI, projected area basis [m^2/gC]
    real(rkx), pointer :: woody(:)    ! ecophys const - woody pft or not
    real(rkx), pointer :: leafcmax(:) ! (gC/m2) leaf C storage
    real(rkx), pointer :: deadstemc(:) ! (gC/m2) dead stem C
    real(rkx), pointer :: dwood(:)     ! ecophys const - wood density (gC/m3)
    real(rkx), pointer :: reinickerp(:) ! ecophys const - parameter in allomet
    real(rkx), pointer :: crownarea_max(:) ! ecophys const - tree max crown a
    real(rkx), pointer :: allom1(:)        ! ecophys const - param in allomet
    ...
    real(rkx), pointer :: nind(:) ! number of individuals

    real(rkx), parameter :: fpc_tree_max = 0.95_rkx !maximum total tree FPC
    integer(ik4)  :: p,fp, g   ! indices
    real(rkx) :: fpc_tree_total(lbg:ubg)
    real(rkx) :: fpc_inc_tree(lbg:ubg)
    real(rkx) :: fpc_inc(lbp:ubp) ! foliar projective cover increment (fraction)
    real(rkx) :: fpc_grass_total(lbg:ubg)
    real(rkx) :: fpc_shrub_total(lbg:ubg)
    real(rkx) :: fpc_grass_max(lbg:ubg)
    real(rkx) :: fpc_shrub_max(lbg:ubg)
    integer(ik4)  :: numtrees(lbg:ubg)
    real(rkx) :: excess
    real(rkx) :: nind_kill
    real(rkx) :: lai_ind
    real(rkx) :: fpc_ind
    real(rkx) :: fpcgrid_old
    real(rkx) :: lm_ind           !leaf carbon (gC/individual)
    ...
    taper = 200._rkx ! make a global constant; used in Establishment + ?

    ! Initialize gridcell-level metrics

    do g = lbg, ubg
      fpc_tree_total(g) = 0._rkx
      fpc_inc_tree(g) = 0._rkx
      fpc_grass_total(g) = 0._rkx
      fpc_shrub_total(g) = 0._rkx
      numtrees(g) = 0
    end do
    ...
    do fp = 1,num_natvegp
      ...
      if ( woody(ivt(p)) == 1._rkx ) then
        if (fpcgrid(p) > 0._rkx .and. nind(p) > 0._rkx) then
          ...
          stemdiam = (24._rkx * deadstemc(p) / &
                  (mathpi * stocking * dwood(ivt(p)) * taper))**(1._rkx/3._rkx)
        else
          stemdiam = 0._rkx
        end if
...
!     else ! crownarea is 1 and does not need updating
      ...
      if ( crownarea(p) > 0._rkx .and. nind(p) > 0._rkx ) then
      ...
      fpc_inc(p) = max(0._rkx, fpcgrid(p) - fpcgrid_old)
    ...
    do g = lbg, ubg
      fpc_grass_max(g) = 1._rkx - min(fpc_tree_total(g), fpc_tree_max)
      fpc_shrub_max(g) = max(0._rkx, fpc_grass_max(g) - fpc_grass_total(g))
    end do
    ...
    do fp = 1,num_natvegp
      ...
      if ( woody(ivt(p)) == 1._rkx .and. tree(ivt(p)) == 1._rkx ) then
        if ( fpc_tree_total(g) > fpc_tree_max ) then
          if ( fpc_inc_tree(g) > 0._rkx ) then
          ...
          if ( fpcgrid(p) > 0._rkx ) then
            nind_kill = nind(p) * excess / fpcgrid(p)
            nind(p) = max(0._rkx, nind(p) - nind_kill)
            fpcgrid(p) = max(0._rkx, fpcgrid(p) - excess)
          else
            nind(p) = 0._rkx
            fpcgrid(p) = 0._rkx
          end if
      ...
      else if ( woody(ivt(p)) == 0._rkx ) then ! grass
      ...
      else if ( woody(ivt(p)) == 1._rkx .and. tree(ivt(p)) == 0._rkx ) then ! shrub
        if ( fpc_shrub_total(g) > fpc_shrub_max(g) ) then
          excess = 1._rkx - fpc_shrub_max(g) / fpc_shrub_total(g)
          ! Reduce individual density (and thereby gridcell-level biomass)
