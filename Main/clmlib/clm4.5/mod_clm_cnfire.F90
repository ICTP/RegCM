...
!
#if (defined CNDV)
   real(rk8), pointer :: nind(:)         ! number of individuals (#/m2)
#endif
   ...
   real(rk8), pointer :: lfc2(:)         ! conversion area frac. of BET+BDT that burned this timestep
   real(rk8), pointer :: fbac1(:)        ! burned area out of conversion region due to land use fire
   real(rk8), pointer :: baf_crop(:)     ! baf for cropland
   real(rk8), pointer :: baf_peatf(:)    ! baf for peatlabd
   real(rk8), pointer :: leafcmax(:)     ! (gC/m2) ann max leaf C
   real(rk8), pointer :: fbac(:)         ! total burned area out of conversion 
   
   real(rk8), pointer :: dtrotr_col(:)   ! annual decreased fraction coverage of BET+BDT (0-1) on the gridcell
   ...
   real(rk8), pointer :: somc_fire(:)          ! (gC/m2/s)fire carbon emissions due to peat burning
   ...
   real(rk8), pointer :: m_decomp_cpools_to_fire_vr(:,:,:) ! (gC/m3/s) vertically-resolved decomposing C fire loss 
...
!-----------------------------------------------------------------------
   ...
   do fc = 1,num_soilc
      ...
      do j = 1, nlevdecomp
         ...
         do l = 1, ndecomp_pools
            if ( is_litter(l) ) then
               m_decomp_cpools_to_fire_vr(c,j,l) = decomp_cpools_vr(c,j,l) * f * 0.4D0
            end if
            if ( is_cwd(l) ) then
         ...
         do l = 1, ndecomp_pools
            if ( is_litter(l) ) then
               m_decomp_npools_to_fire_vr(c,j,l) = decomp_npools_vr(c,j,l) * f * 0.4D0
            end if
            if ( is_cwd(l) ) then
