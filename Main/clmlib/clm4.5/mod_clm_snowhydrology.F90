...
! !USES:
    use mod_clm_type
    use mod_clm_varcon        , only : denh2o, denice, wimp, ssi, isturb,istsoil,istdlak
...
!
    ...
    real(rk8), pointer :: qflx_evap_soi(:)    ! evaporation flux from soil (W/m**2) [+ to atm]
...
!
    ...
    real(rk8), pointer :: mss_bcphi(:,:)      ! hydrophillic BC mass in snow (col,lyr) [kg]
    real(rk8), pointer :: mss_bcpho(:,:)      ! hydrophobic BC mass in snow (col,lyr) [kg]
    real(rk8), pointer :: mss_ocphi(:,:)      ! hydrophillic OC mass in snow (col,lyr) [kg]
    real(rk8), pointer :: mss_ocpho(:,:)      ! hydrophobic OC mass in snow (col,lyr) [kg]
    ...
    real(rk8), pointer :: flx_bc_dep_dry(:)   ! dry BC deposition (col) [kg m-2 s-1]
    real(rk8), pointer :: flx_bc_dep_wet(:)   ! wet BC deposition (col) [kg m-2 s-1]
    real(rk8), pointer :: flx_bc_dep(:)       ! total BC deposition (col) [kg m-2 s-1]
    real(rk8), pointer :: flx_bc_dep_pho(:)   ! hydrophobic BC deposition (col) [kg m-1 s-1]
    real(rk8), pointer :: flx_bc_dep_phi(:)   ! hydrophillic BC deposition (col) [kg m-1 s-1]
    real(rk8), pointer :: flx_oc_dep_dry(:)   ! dry OC deposition (col) [kg m-2 s-1]
    real(rk8), pointer :: flx_oc_dep_wet(:)   ! wet OC deposition (col) [kg m-2 s-1]
    real(rk8), pointer :: flx_oc_dep(:)       ! total OC deposition (col) [kg m-2 s-1]
    real(rk8), pointer :: flx_oc_dep_pho(:)   ! hydrophobic OC deposition (col) [kg m-1 s-1]
    real(rk8), pointer :: flx_oc_dep_phi(:)   ! hydrophillic OC deposition (col) [kg m-1 s-1]
    real(rk8), pointer :: flx_dst_dep_dry1(:) ! dry dust (species 1) deposition (col) [kg m-2 s-1]
    real(rk8), pointer :: flx_dst_dep_wet1(:) ! wet dust (species 1) deposition (col) [kg m-2 s-1]
    real(rk8), pointer :: flx_dst_dep_dry2(:) ! dry dust (species 2) deposition (col) [kg m-2 s-1]
    real(rk8), pointer :: flx_dst_dep_wet2(:) ! wet dust (species 2) deposition (col) [kg m-2 s-1]
    real(rk8), pointer :: flx_dst_dep_dry3(:) ! dry dust (species 3) deposition (col) [kg m-2 s-1]
    real(rk8), pointer :: flx_dst_dep_wet3(:) ! wet dust (species 3) deposition (col) [kg m-2 s-1]
    real(rk8), pointer :: flx_dst_dep_dry4(:) ! dry dust (species 4) deposition (col) [kg m-2 s-1]
    real(rk8), pointer :: flx_dst_dep_wet4(:) ! wet dust (species 4) deposition (col) [kg m-2 s-1]
    real(rk8), pointer :: flx_dst_dep(:)      ! total dust deposition (col) [kg m-2 s-1]
    real(rk8), pointer :: forc_aer(:,:)       ! aerosol deposition from atmosphere model (grd,aer) [kg m-1 s-1]
!
!
...
!
    ...
    real(rk8) :: vol_liq(lbc:ubc,-nlevsno+1:0)      !partial volume of liquid water in layer
    real(rk8) :: vol_ice(lbc:ubc,-nlevsno+1:0)      !partial volume of ice lens in layer
    real(rk8) :: eff_porosity(lbc:ubc,-nlevsno+1:0) !effective porosity = porosity - vol_ice
    ...
    do j = -nlevsno+1, 0
       do fc = 1, num_snowc
          ...
          if (j >= snl(c)+1) then
             ...
             vol_ice(c,j)      = min(1.D0, h2osoi_ice(c,j)/(dz(c,j)*frac_sno_eff(c)*denice))
...
!
    ...
    real(rk8), parameter :: dm = 100.0D0     ! Upper Limit on Destructive Metamorphism Compaction [kg/m3]
    real(rk8), parameter :: eta0 = 9.D+5   ! The Viscosity Coefficient Eta0 [kg-s/m2]
    real(rk8) :: burden(lbc:ubc) ! pressure of overlying snow [kg/m2]
    real(rk8) :: ddz1   ! Rate of settling of snowpack due to destructive metamorphism.
    ...
    real(rk8) :: fi     ! Fraction of ice relative to the total water content at current time step
    real(rk8) :: td     ! t_soisno - tfrz [K]
    real(rk8) :: pdzdtc ! Nodal rate of change in fractional-thickness due to compaction [fraction/s]
    ...
    do j = -nlevsno+1, 0
       do fc = 1, num_snowc
          ...
          if (j >= snl(c)+1) then
             ...
             void = 1.D0 - (h2osoi_ice(c,j)/denice + h2osoi_liq(c,j)/denh2o) / dz(c,j)
             wx = (h2osoi_ice(c,j) + h2osoi_liq(c,j))
             ...
             if (void > 0.001D0 .and. h2osoi_ice(c,j) > .1D0) then
                ...
                if (imelt(c,j) == 1) then
                    if(subgridflag==1 .and. (ltype(clandunit(c)) == istsoil .or. ltype(clandunit(c)) == istcrop)) then
                      ...
                      fsno_melt = 1. - (acos(2.*min(1.D0,wsum/int_snow(c)) - 1.D0)/rpi)**(n_melt(c))

                      ddz3 = ddz3 - max(0.D0,(fsno_melt - frac_sno(c))/frac_sno(c))
                      ddz3 = -1.D0/dtime * ddz3
                   else
                      ddz3 = - 1.D0/dtime * max(0.D0,(frac_iceold(c,j) - fi)/frac_iceold(c,j))
                   endif
                else
                   ddz3 = 0.D0
                end if
                ...
                dz(c,j) = max(dz(c,j) * (1.D0+pdzdtc*dtime),(h2osoi_ice(c,j)/denice+ h2osoi_liq(c,j)/denh2o)/frac_sno(c))
             end if
...
!
    ...
    real(rk8), pointer :: mss_bcphi(:,:)  ! hydrophilic BC mass in snow (col,lyr) [kg]
    real(rk8), pointer :: mss_bcpho(:,:)  ! hydrophobic BC mass in snow (col,lyr) [kg]
    real(rk8), pointer :: mss_ocphi(:,:)  ! hydrophilic OC mass in snow (col,lyr) [kg]
    real(rk8), pointer :: mss_ocpho(:,:)  ! hydrophobic OC mass in snow (col,lyr) [kg]
    real(rk8), pointer :: mss_dst1(:,:)   ! dust species 1 mass in snow (col,lyr) [kg]
    real(rk8), pointer :: mss_dst2(:,:)   ! dust species 2 mass in snow (col,lyr) [kg]
    real(rk8), pointer :: mss_dst3(:,:)   ! dust species 3 mass in snow (col,lyr) [kg]
    real(rk8), pointer :: mss_dst4(:,:)   ! dust species 4 mass in snow (col,lyr) [kg]
    ...
    do fc = 1, num_snowc
       ...
       if (snow_depth(c) > 0.D0) then
          if ((ltype(l) == istdlak .and. snow_depth(c) < 0.01D0 + lsadz ) .or. &
               ((ltype(l) /= istdlak) .and. ((frac_sno_eff(c)*snow_depth(c) < 0.01D0)  &
               .or. (h2osno(c)/(frac_sno_eff(c)*snow_depth(c)) < 50.D0)))) then
       ...
       if (snl(c) < -1) then
          ...
          do i = msn_old(c)+1,0
             if ((frac_sno_eff(c)*dz(c,i) < dzminloc(mssi(c))) .or. &
                  ((h2osoi_ice(c,i) + h2osoi_liq(c,i))/(frac_sno_eff(c)*dz(c,i)) < 50.D0)) then
                if (i == snl(c)+1) then
...
! !ARGUMENTS:
    implicit none
    real(rk8), intent(in)    :: dz2   ! nodal thickness of 2 elements being combined [m]
    ...
    real(rk8), intent(inout) :: dz    ! nodal thickness of 1 elements being combined [m]
...
!-----------------------------------------------------------------------
    ...
    do j = -nlevsno + 1, 0
       do fc = 1, num_snowc
          ...
          if (j == 0) then
             if ( abs(dztot(c)) > 1.D-10 .or. abs(snwicetot(c)) > 1.D-7 .or. &
                  abs(snwliqtot(c)) > 1.D-7 ) then
                write(stderr,*)'Inconsistency in SnowDivision_Lake! c, remainders', &
