...
! !PUBLIC DATA MEMBERS:
   ...
   real(rk8), public :: decomp_depth_efolding = 0.5D0    ! (meters) e-folding depth for reduction in decomposition [set to large number for depth-independance]
#endif
   real(rk8), public :: froz_q10 = 1.5D0                 ! separate q10 for frozen soil respiration rates.  default to same as above zero rates
#ifdef LCH4
   ...
   real(rk8), public, parameter :: spinup_vector(nsompools) = (/ 1.0D0, 1.0D0, 5.0D0, 70.0D0 /) ! multipliers for soil decomp during accelerated spinup
   ...
   if ( nlevdecomp .eq. 1 ) then
      ...
      do j = 1,nlev_soildecomp_standard
         do fc = 1,num_soilc
            ...
            !! t_scalar(c,1)=t_scalar(c,1) + (1.5**((t_soisno(c,j)-(tfrz+25.D0))/10.D0))*fr(c,j)
            if (t_soisno(c,j) .ge. tfrz) then
               t_scalar(c,1)=t_scalar(c,1) + (1.5**((t_soisno(c,j)-(tfrz+25.D0))/10.D0))*fr(c,j)
            else
               t_scalar(c,1)=t_scalar(c,1) + (1.5**(-25.D0/10.D0))*(froz_q10**((t_soisno(c,j)-tfrz)/10.D0))*fr(c,j)
            endif
         end do
      end do
            ...
            do fc = 1,num_soilc
               ...
               if (.not. anoxia_wtsat) then
                  ...
                  o_scalar(c,1) = o_scalar(c,1) + fr(c,j) * &
                                     (max(o2stress_unsat(c,j), mino2lim)*(1.D0 - finundated(c)) + &
         ...
         do fc = 1,num_soilc
            c = filter_soilc(fc)
               ...
               if (t_soisno(c,j) .ge. tfrz) then
                  ...
                  t_scalar(c,j)= (1.5**(-25.D0/10.D0))*(froz_q10**((t_soisno(c,j)-tfrz)/10.D0))
               endif
         end do
      end do
         ...
         do j = 1,nlevdecomp
            do fc = 1,num_soilc
               ...
               if (.not. anoxia_wtsat) then
                  ...
                  o_scalar(c,j) = max(o2stress_unsat(c,j), mino2lim) * (1.D0 - finundated(c)) + &
                                     max(o2stress_sat(c,j), mino2lim) * finundated(c)
               end if
            end do
         end do
