...
module mod_mkvic
  ...
  subroutine mkvic(vicfile,vic)
    ...
    if ( size(vic,3) < nvic ) then
      call die(__FILE__,'Size too small for vic in mkvic',__LINE__)
    end if
    ...
    allocate(rvar(sum(domain%ni),domain%nj,nvic))
    ...
    call bilinear(rvar,rmask,rlon,rlat,vic,xlon,xlat,vmin,vmisdat)
