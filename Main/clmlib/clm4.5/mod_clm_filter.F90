...
! !USES:
    use mod_clm_type
    use mod_clm_decomp , only : get_proc_clumps, get_clump_bounds
!
! !ARGUMENTS:
    implicit none
!
...
!------------------------------------------------------------------------
    ...
    if (ier /= 0) then
       write(stderr,*) 'allocFilters(): allocation error for clumpsfilters'
...
! !USES:
    ...
    use mod_clm_varcon, only : istsoil, isturb, icol_road_perv
