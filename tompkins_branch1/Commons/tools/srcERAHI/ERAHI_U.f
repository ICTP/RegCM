...
C
      real*4  u
      common /var_u/ u(NLONS,NLATS,NLEVS)
      integer nrec
      common /ncount/ nrec
C
...
c
 ...
 10      format('../DATA/ERAHI/',I4,'/',A2,'/u.',I10,'.grib')
 ...
 20      format('EHI_',I10)
     ...
     &          ,access='direct')
         ...
         do k=1,NLEVS
            nrec=nrec+1
            write(39,rec=nrec) ((u(i,j,k),i=1,NLONS),j=1,NLATS)
         enddo
         close(39)
         write(*,*) 'Zonal wind is added for DATE: ',IDATE
      ENDDO
      stop
      end
C
...
c     reduced N80 Gaussian grid:
     ...
     &               iarrayu,  words_read_into_rn80, rarrayrn80  )
      call check_error_status('intf', ierror)
      call get_field_data(words_per_mb,iarrayu,rarrayrn80)
      k=0
      do j=NLATS,1,-1
      do i=1,NLONS
         k=k+1
         u(i,j,lev)=rarrayrn80(k)
      enddo
      enddo
