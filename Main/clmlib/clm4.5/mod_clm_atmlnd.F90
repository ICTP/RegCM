...
! !USES:
  use mod_realkinds
  use mod_clm_varpar  , only : numrad, ndst, nlevgrnd !ndst = number of dust bins.
...
!----------------------------------------------------
  type atm2lnd_type
     ...
     real(rk8), pointer :: forc_u(:)       !atm wind speed, east direction (m/s)
     real(rk8), pointer :: forc_v(:)       !atm wind speed, north direction (m/s)
     real(rk8), pointer :: forc_wind(:)    !atmospheric wind speed   
     real(rk8), pointer :: forc_q(:)       !atmospheric specific humidity (kg/kg)
     real(rk8), pointer :: forc_hgt(:)     !atmospheric reference height (m)
     real(rk8), pointer :: forc_hgt_u(:)   !obs height of wind [m] (new)
     real(rk8), pointer :: forc_hgt_t(:)   !obs height of temperature [m] (new)
     real(rk8), pointer :: forc_hgt_q(:)   !obs height of humidity [m] (new)
     real(rk8), pointer :: forc_pbot(:)    !atmospheric pressure (Pa)
     real(rk8), pointer :: forc_th(:)      !atm potential temperature (Kelvin)
     real(rk8), pointer :: forc_vp(:)      !atmospheric vapor pressure (Pa) 
     real(rk8), pointer :: forc_rho(:)     !density (kg/m**3)
     real(rk8), pointer :: forc_rh(:)      !atmospheric relative humidity (%)
     real(rk8), pointer :: forc_psrf(:)    !surface pressure (Pa)
     real(rk8), pointer :: forc_pco2(:)    !CO2 partial pressure (Pa)
     real(rk8), pointer :: forc_lwrad(:)   !downwrd IR longwave radiation (W/m**2)
     real(rk8), pointer :: forc_solad(:,:) !direct beam radiation (numrad) (vis=forc_sols , nir=forc_soll )
     real(rk8), pointer :: forc_solai(:,:) !diffuse radiation (numrad) (vis=forc_solsd, nir=forc_solld)
     real(rk8), pointer :: forc_solar(:)   !incident solar radiation
     ...
     real(rk8), pointer :: rainf(:)        !ALMA rain+snow [mm/s]
     real(rk8), pointer :: forc_pc13o2(:)  !C13O2 partial pressure (Pa)
     real(rk8), pointer :: forc_po2(:)     !O2 partial pressure (Pa)
     real(rk8), pointer :: forc_flood(:)   ! rof flood (mm/s)
     real(rk8), pointer :: volr(:)   ! rof volr (m3)
     real(rk8), pointer :: forc_aer(:,:)   ! aerosol deposition array
#ifdef LCH4
...
!----------------------------------------------------
  type lnd2atm_type
     real(rk8), pointer :: t_rad(:)        !radiative temperature (Kelvin)
     real(rk8), pointer :: t_ref2m(:)      !2m surface air temperature (Kelvin)
     real(rk8), pointer :: q_ref2m(:)      !2m surface specific humidity (kg/kg)
     real(rk8), pointer :: u_ref10m(:)     !10m surface wind speed (m/sec)
     real(rk8), pointer :: h2osno(:)       !snow water (mm H2O)
     real(rk8), pointer :: albd(:,:)       !(numrad) surface albedo (direct)
     real(rk8), pointer :: albi(:,:)       !(numrad) surface albedo (diffuse)
     real(rk8), pointer :: taux(:)         !wind stress: e-w (kg/m/s**2)
     real(rk8), pointer :: tauy(:)         !wind stress: n-s (kg/m/s**2)
     real(rk8), pointer :: eflx_lh_tot(:)  !total latent HF (W/m**2)  [+ to atm]
     real(rk8), pointer :: eflx_sh_tot(:)  !total sensible HF (W/m**2) [+ to atm]
     real(rk8), pointer :: eflx_lwrad_out(:) !IR (longwave) radiation (W/m**2)
     real(rk8), pointer :: qflx_evap_tot(:)!qflx_evap_soi + qflx_evap_can + qflx_tran_veg
     real(rk8), pointer :: fsa(:)          !solar rad absorbed (total) (W/m**2)
     real(rk8), pointer :: nee(:)          !net CO2 flux (kg CO2/m**2/s) [+ to atm]
     real(rk8), pointer :: ram1(:)         !aerodynamical resistance (s/m)
     real(rk8), pointer :: fv(:)           !friction velocity (m/s) (for dust model)
     real(rk8), pointer :: h2osoi_vol(:,:) !volumetric soil water (0~watsat, m3/m3, nlevgrnd) (for dust model)
     real(rk8), pointer :: rofliq(:)       ! rof liq forcing
     real(rk8), pointer :: rofice(:)       ! rof ice forcing
     real(rk8), pointer :: flxdst(:,:)     !dust flux (size bins)
     real(rk8), pointer :: ddvel(:,:)      !dry deposition velocities
     real(rk8), pointer :: flxvoc(:,:)     ! VOC flux (size bins)
#ifdef LCH4
     real(rk8), pointer :: flux_ch4(:)     !net CH4 flux (kg C/m**2/s) [+ to atm]
#endif
...
!EOP
  ...
  real(rk8), parameter :: amCO2 = amC + 2.0D0*amO ! Atomic mass number for CO2
