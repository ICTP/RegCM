...
#endif
  subroutine kfpara(kts,kte,u0,v0,t0,qv0,p0,dzq,w0avg1d,dt,dx,dxsq, &
                    rho,dqdt,dqidt,dqcdt,dqrdt,dqsdt,dtdt,          &
                    raincv,pratec,nca,warm_rain,qi_flag,qs_flag)
    implicit none
    integer(ik4) , intent(in) :: kts , kte
    logical , intent(in) :: warm_rain
    logical , intent(in) :: qi_flag , qs_flag
    real(rk8) , dimension(kts:kte) , intent(in) :: u0 , v0 , t0 , &
                       qv0 , p0 , rho , dzq , w0avg1d
    real(rk8) , intent(in) :: dt , dx , dxsq
    real(rk8) , dimension(kts:kte) , intent(inout) :: dqdt , dqidt , &
                       dqcdt , dqrdt , dqsdt , dtdt

    real(rk8) , intent(inout) :: raincv , pratec , nca
    ...
    z0(1) = d_half * dzq(1)
    do k = 2 , kl
      z0(k) = z0(k-1) + d_half * (dzq(k)+dzq(k-1))
      dza(k-1) = z0(k) - z0(k-1)
    end do
    dza(kl) = d_zero
    kmix = 1
    ...
    do
      ...
      wkl = (w0avg1d(k) + (w0avg1d(klcl) - w0avg1d(k))*dlp) * &
              dx / 25.0D3 - wklcl
      wabs = abs(wkl) + 1.D-10
      wsigne = wkl/wabs
      ...
      plcl = p0(klcl) * exp(egrav / (rdry*tvavg) * (z0(klcl)-zlcl))
      qese = ep2 * es / (plcl-es)
      gdt = egrav * dtlcl * (zlcl-z0(lc)) / (tv0(lc)+tven)
      wlcl = d_one + d_half * wsigne * sqrt(abs(gdt) + 1.D-10)
      ...
      do nk = k , kl-1
        ...
        !
        thtes(nk1) = t0(nk1) * (p00/p0(nk1))**(c6*(d_one-c7*qes(nk1))) * &
                     exp((c1/t0(nk1) - c2)*qes(nk1) * (d_one + c8*qes(nk1)))
        ...
        !
        call envirtht(p0(nk1),t0(nk1),q0(nk1),thetee(nk1),ratio2(nk1),rl)
        !
        ...
        if ( tvqu(nk1) <= tv0(nk1) ) then
          ...
          tmpliq = f2*qliq(nk1)
          tmpice = f2*qice(nk1)
          call tpmix(p0(nk1),thttmp,ttmp,qtmp,tmpliq, &
          ...
          do 
            ...
            tmpliq = f2 * qliq(nk1)
            tmpice = f2 * qice(nk1)
            call tpmix(p0(nk1),thttmp,ttmp,qtmp,tmpliq,tmpice, &
      ...
      do nk = 1 , k
        ...
        tsat = tdpt - (tl1 + tl2*(tdpt-tzero) - &
                       tl3 * (t0(nk)-tzero)) * (t0(nk)-tdpt)
        thta = t0(nk) * (p00/p0(nk))**(c6*(d_one - c7*q0(nk)))
        thetee(nk) = thta * exp((c1/tsat - c2)*q0(nk)*(d_one + c8*q0(nk)))
        thtes(nk) = thta * exp((c1/t0(nk) - c2)*qes(nk)*(d_one + c8*qes(nk)))
        eqfrc(nk) = d_one
      end do
      ltop1 = ltop + 1
      ltopm1 = ltop - 1
      ...
      do nk = 1 , ltop
        ...
        ! scheme
        !
        exn(nk) = (p00/p0(nk))**(c6*(d_one - c7*qdt(nk)))
        thtau(nk) = tu(nk)*exn(nk)
        exn(nk) = (p00/p0(nk))**(c6*(d_one - c7*q0(nk)))
      ...
      if ( ratio2(lfs) > d_zero ) then
        call envirtht(p0(lfs),t0(lfs),q0(lfs),thetee(lfs),d_zero,rl)
      end if
      ...
      thtad(lfs) = tz(lfs) * (p00/p0(lfs))**(c6*(d_one - c7*qd(lfs)))
      if ( qd(lfs) >= qs ) then
        theted(lfs) = thtad(lfs)*exp((c1/tz(lfs) - c2)*qs*(d_one + c8*qs))
      else
        call envirtht(p0(lfs),tz(lfs),qd(lfs),theted(lfs),d_zero,rl)
      end if
      ddraft = .true.
      ddraft_find: &
      do nk = 1 , lfs
        ...
        if ( theted(lfs) > thtes(nd) .or. nd == 1 ) then
          ...
          !
          if ( nk == 1 .or. (p0(ldb)-p0(lfs)) < 50.D2 ) then
            ddraft = .false.
          end if
          exit ddraft_find
        end if
      ...
      if ( ddraft ) then
        ...
        !
        tvd(lfs) = t0(lfs) * (d_one + b61*qes(lfs))
        rdd = p0(lfs) / (rdry*tvd(lfs))
        a1 = (d_one-peff) * au0
        dmf(lfs) = -a1*rdd
        ...
        do nd = lfs-1 , ldb,-1
          ...
          if ( nd <= ldt ) then
            ...
            if ( ratio2(nd) > d_zero ) then
              call envirtht(p0(nd),t0(nd),q0(nd),thetee(nd),d_zero,rl)
            end if
        ...
        !
        do nd = ldb , ldt
          tz(nd) = tpdd(p0(nd),theted(ldt),t0(nd),qs,qd(nd),d_one)
          ...
          qd(nd) = qs
          thtad(nd) = tz(nd)*(p00/p0(nd))**(c6*(d_one-c7*qd(nd)))
        end do
      end if
      !
      ...
      do
        if ( ainc/aincmx > 0.999D0 ) then
          ...
          !
          do nk = 1 , ltop
            exn(nk) = (p00/p0(nk))**(c6*(d_one - c7*qg(nk)))
          ...
          do nk = k , ltopm1
            ...
            thtesg(nk1) = tg(nk1)*(p00/p0(nk1))**(c6*(d_one - c7*qese)) * &
