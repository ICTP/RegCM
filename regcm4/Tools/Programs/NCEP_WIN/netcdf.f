...
c &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
     ...
     &  lname,unit,factor,offset,vvarmin,vvarmax,xlat1d,xlon1d,sigh
...
c    vmisdat   real*8             value for missing data
      ...
      integer    cdfid,idim(3),izstag,ie,je,ke,iotype
      real     arr(ie,je,ke)
...
c     declarations needed for netcdf-stuff
      real*4 varmin(3),varmax(3),varstg(3),vmisdat,tstep,sigh(ke)
      integer*2 mdat
      integer strbeg,strend
      integer idtest,it
      integer vardim(3)
      real*4 offset,factor,rfac
...
c     if not then define it
      ...
      if (it.ne.0) then
        ...
        CALL PUTDEFCDF(cdfid,varnam,ndims,vmisdat,lname,unit
     &    ,  offset,factor,vardim,varmin,varmax,iotype,ierr)
...
c     loop over all vertical levels and write the data
      do k=1,kevar
        ...
        if (iotype.eq.1) then   ! Integer*2 format
          rfac = 1./factor
          CALL PUTDATCDFI2(cdfid,varnam(strbeg(varnam):strend(varnam))
     & ,      vtstep, k, iputlev, ievar, jevar, kevar, arr, ie, je, ke
     & ,      vmisdat, rfac, offset, ierr) 
        else if (iotype.eq.2) then   ! Real*4 format
          CALL PUTDATCDFR4(cdfid,varnam(strbeg(varnam):strend(varnam))
     & ,      vtstep,k,iputlev,ievar,jevar,kevar,arr,ie,je,ke,ierr)
        end if
...
C     Declaration of local variables
      ...
      integer ie, je, ke, ievar, jevar, kevar, ijmax
      real       arr(ie,je,ke)
      integer*2   dat(ie*je*ke)
      real*4     offset, rfac, vmisdat
      real misdat
      real*8    time, timeval
      real*8    dvrange(2)
      ...
      do i=1,ievar
        ij = ij + 1
        if (arr(i,j,k).lt.vmisdat) then 
          dat(ij) = filshort  ! the lowest integer value, should match min
        else 
...
c     get definitions of data
      CALL GETDEFI2 (cdfid, varnam(1:strend(varnam)), ndims, misdat, 
     &                           vardim, ierr)
...
C     Synchronize output to disk and close the files
     ...
     &         , clunits,offset,scale,vardim,varmin,varmax,iotype,error)
c-----------------------------------------------------------------------
c     Purpose:
...
c     Argument declarations.
      ...
      integer        vardim(*), ndim, error, cdfid, iotype
...
c     define missing data value
      if (iotype.eq.1) then
        CALL NCAPT(cdfid,id,'missing_value',NCSHORT,1,filshort,error)
      elseif (iotype.eq.2) then
...
c     Error exits.
 ...
 920  write (6, *) '*ERROR*: An error occurred while attempting to ',
     &             'write the data file in subroutine putcdf.'
      ...
      SUBROUTINE PUTDATCDFR4(cdfid, varnam, time, k, level
     &      , ievar, jevar, kevar, arr, ie, je, ke, error)
c-----------------------------------------------------------------------
c     Purpose:
c        This routine is called to write the data of a variable
c        to an IVE-NetCDF file for use with the IVE plotting package. 
...
c     get definitions of data
      CALL GETDEFCDFR4(cdfid, varnam(1:strend(varnam)), ndims
     &   , misdat, vardim, ierr)
      if (ierr.ne.0)  print *,'*ERROR* in getdef in putdat'
...
C     Synchronize output to disk and close the files
      ...
      SUBROUTINE GETDEFI2 (cdfid, varnam, ndim, misdat, 
     &                              vardim, error)
c-----------------------------------------------------------------------
c     Purpose:
...
c     Error exits.
 ...
 920  write (6, *) '*ERROR*: An error occurred while attempting to ',
     &             'read the data file in subroutine getcdf.'
      ...
      SUBROUTINE GETDEFCDFR4 (cdfid, varnam, ndim, misdat, 
     &                              vardim, error)
c-----------------------------------------------------------------------
c     Purpose:
c        This routine is called to get the dimensions and attributes of 
