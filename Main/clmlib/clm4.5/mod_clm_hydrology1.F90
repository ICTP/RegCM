  ...
  subroutine Hydrology1(lbc, ubc, lbp, ubp, num_nolakec, filter_nolakec, &
    ...
    do f = 1, num_nolakep
      ...
      if ( ltype(l) == istsoil .or. &
        ...
        qflx_candrip(p) = 0._rk8      ! rate of canopy runoff
        qflx_through_snow(p) = 0._rk8 ! rain precipitation direct through canopy
        qflx_through_rain(p) = 0._rk8 ! snow precipitation direct through canopy
        qflx_prec_intr(p) = 0._rk8    ! total intercepted precipitation
        fracsnow(p) = 0._rk8          ! fraction of input precip that is snow
        fracrain(p) = 0._rk8          ! fraction of input precip that is rain

        if ( ctype(c) /= icol_sunwall .and. &
             ctype(c) /= icol_shadewall ) then
          if ( frac_veg_nosno(p) == 1 .and. &
               (forc_rain(g) + forc_snow(g)) > 0._rk8 ) then
        ...
        h2ocan(p)            = 0._rk8
        qflx_candrip(p)      = 0._rk8
        qflx_through_snow(p) = 0._rk8
        qflx_through_rain(p) = 0._rk8
        qflx_prec_intr(p)    = 0._rk8
        fracsnow(p)          = 0._rk8
        fracrain(p)          = 0._rk8
      end if
      ...
      ! Urban sunwall and shadewall have no intercepted precipitation
        ...
        qflx_prec_grnd_snow(p) = 0.0_rk8
        qflx_prec_grnd_rain(p) = 0.0_rk8
      end if
    ...
    do f = 1 , num_nolakec
      ...
      if ( do_capsnow(c) ) then
        ...
        if ( forc_t(c) > tfrz + 2._rk8 ) then
          bifall = 50._rk8 + 1.7_rk8*(17.0_rk8)**1.5_rk8
        else if ( forc_t(c) > tfrz - 15._rk8 ) then
          bifall = 50._rk8 + 1.7_rk8*(forc_t(c) - tfrz + 15._rk8)**1.5_rk8
        else
          bifall = 50._rk8
        end if
        ...
        if ( h2osno(c) > 0.0 ) then
          ...
          if ( newsnow(c) > 0._rk8 ) then
            ...
            temp_intsnow = (h2osno(c) + newsnow(c)) / &
                   (0.5_rk8*(cos(rpi*(1._rk8-max(frac_sno(c),1e-6_rk8))** &
                   (1.0_rk8/n_melt(c)))+1._rk8))
            int_snow(c) = min(1.e8_rk8,temp_intsnow)
          end if
          ...
          if ( oldfflag == 1 ) then
            ...
            if ( snow_depth(c) > 0.0_rk8 )  then
              frac_sno(c) = tanh(snow_depth(c)/(2.5_rk8*zlnd* &
                  (min(800._rk8,(h2osno(c) + newsnow(c)) / &
                                   snow_depth(c))/100._rk8)**1._rk8) )
            end if
