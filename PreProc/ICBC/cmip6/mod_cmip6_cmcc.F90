...
module mod_cmip6_cmcc
  ...
  contains
    ...
    subroutine read_hcoord_cmcc(ncid,lon,lat)
      ...
      call getmem(lon,1,nlon,'cmip6_cmcc:lon')
      call getmem(lat,1,nlat,'cmip6_cmcc:lat')
    ...
    subroutine read_hcoord_sst_cmcc(ncid,lon,lat)
      ...
      call getmem(lon,1,nlon,1,nlat,'cmip6_cmcc:lon')
      call getmem(lat,1,nlon,1,nlat,'cmip6_cmcc:lat')
    ...
    subroutine read_vcoord_cmcc(ncid,a,b,p0)
      ...
      call getmem(a,1,nlev,'cmip6_cmcc:a')
      call getmem(b,1,nlev,'cmip6_cmcc:b')
    ...
    recursive subroutine read_3d_cmcc(idate,v,lonlyc)
      ...
      if ( .not. associated(v%var) ) then
        ...
        call getmem(v%var,1,v%ni,1,v%nj,1,v%nk,'cmip6:cmcc:'//trim(v%vname))
#ifdef DEBUG
    ...
    recursive subroutine read_2d_cmcc(idate,v,lonlyc)
      ...
      if ( .not. associated(v%var) ) then
        v%ni = size(v%hcoord%lon1d)
        v%nj = size(v%hcoord%lat1d)
        call getmem(v%var,1,v%ni,1,v%nj,'cmip6:cmcc:'//trim(v%vname))
#ifdef DEBUG
