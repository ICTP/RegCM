...
\begin{enumerate}
\item In the current version 4.4 the model parallelizes execution dividing
...
on. There is no plan to extend this feature to CLM3.5 model. This affects only
...
\begin{Verbatim}
 ...
 smoist_filename = 'moist.nc', ! Read initial moisture and snow from this file
 /
\end{Verbatim}
}
...
This namelist is used by all RegCM programs to enable/disable some debug printout.
...
value in $km$ specified in the \verb=geoparam= namelist at \ref{geoparam}.
...
\begin{Verbatim}
 ...
 enable_atm_vars = 64*.true., ! Mask to eventually disable variables ATM
 ...
 enable_sts_vars = 17*.true., ! Mask to eventually disable variables STS
...
\begin{Verbatim}
 ...
 cevaplnd  =   .100E-02, ! Raindrop evap rate coef [[(kg m-2 s-1)-1/2]/s]
 cevapoce  =   .100E-02, ! Raindrop evap rate coef [[(kg m-2 s-1)-1/2]/s]
 caccrlnd  =      3.000, ! Raindrop accretion rate [m3/kg/s]
 caccroce  =      3.000, ! Raindrop accretion rate [m3/kg/s]
...
\begin{Verbatim}
 &microparam
  ...
  kautoconv = 4,             !  Choose the autoconversion paramaterization
                             ...
                             ! => 3 Kessler (1969)
                             ! => 4 Sundqvist
  ksemi = 1.0D0,             ! Implicit/Explicit control
  ...
  vqxr = 4.0D0,              ! Rain fall speed (default is 4 m/s)
  vqxi = 0.15D0,             ! Ice fall speed (default is 0.15 m/s)
  vqxs = 1.0D0,              ! Snow fall speed (default is 1 m/s)
  zauto_rate_khair = 0.355D0,! Autoconversion coefficient for kautoconv=2
  zauto_rate_kessl = 1.D-3,  ! Autoconversion coefficient for kautoconv=3
  zauto_rate_klepi = 0.5D-3, ! Autoconversion coefficient for kautoconv=1
  rkconv = 1.666D-4,         ! Autoconversion coefficient for kautoconv=4
  rcovpmin = 0.1D0,          ! Minimum precipitation coverage
  rpecons = 5.547D-5,        ! Evaporation constant Kessler
 /
\end{Verbatim}
}
...
\begin{Verbatim}
 ...
 elcrit_ocn = 0.0011, ! Autoconversion threshold water content (g/g) over ocean
 ...
 detrpen = 0.75D-4,   ! Detrainment rate for penetrative convection iconv=4
 entshalp = 2.0D0,    ! shallow entrainment factor for entrorg iconv=4
 rcuc_lnd = 0.05D0,   ! Convective cloud cover for rain evporation iconv=4
 rcuc_ocn = 0.05D0,   ! Convective cloud cover for rain evporation iconv=4
 rcpec_lnd = 5.55D-5, ! Coefficient for rain evaporation below cloud iconv=4
 rcpec_ocn = 5.55D-5, ! Coefficient for rain evaporation below cloud iconv=4
 rhebc_lnd = 0.7D0,   ! Critical rh below cloud for evaporation iconv=4
 rhebc_ocn = 0.9D0,   ! Critical rh below cloud for evaporation iconv=4
 rprc_lnd = 1.4D-3,   ! conversion coefficient from cloud water iconv=4
 rprc_ocn = 1.4D-3,   ! conversion coefficient from cloud water iconv=4
 /

 &kfparam
 kf_trigger = 3,      ! Trigger mechanism. Allowed values 1 and 3
 kf_entrate = 0.03D0, ! Entrainment rate
 /
\end{Verbatim}
}
...
\begin{enumerate}
\item In case of the mixed schemes $96, 97, 98, 99$, both the selected schemes
configuration namelist are read in.
Note in this case for the schemes only the relevant (Ocean or Land) control
values are used.
...
\begin{Verbatim}
 ...
 rstbl = 1.5D0,   ! Scaling parameter for stable boundary layer eddy length
                  ...
                  ! conditions
 czero = 5.869D0, ! Czero constant in UW PBL (eqn 44a and pgs 856-857)
 /
\end{Verbatim}
}
...
\begin{Verbatim}
 ...
 idrv = 0,     ! Flag for calculation of dFdT, the change in upward flux as
...
\begin{Verbatim}
 ...
 idirect   = 1,  ! CHoice to enable or not aerosol feedbacks on radiation and
                 ! dynamics (aerosol direct and semi direct effects): possible choice 1 or 2:
 ...
 iindirect = 0,  ! Enable sulfate first indirect effect in radiation scheme based on Qian et al., 2001 
 rdstemfac = 1.0,! Dust emission adjustment factor ( soil erodibility) : linearly reduce or increase the dust flux
 /
\end{Verbatim}
}
