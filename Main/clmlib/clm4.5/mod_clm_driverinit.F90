...
! !USES:
    ...
    use mod_clm_varcon, only    : h2osno_max, rair, cpair, grav, istice_mec, lapse_glcmec
...
!
    ...
    real(rk8), pointer :: eflx_bot(:)        ! heat flux from beneath soil/ice column (W/m**2)
    real(rk8), pointer :: glc_topo(:)        ! sfc elevation for glacier_mec column (m)
    ...
    real(rk8) :: hsurf_g,hsurf_c,Hbot
    real(rk8) :: zbot_g, tbot_g, pbot_g, thbot_g, qbot_g, qs_g, es_g
    real(rk8) :: zbot_c, tbot_c, pbot_c, thbot_c, qbot_c, qs_c, es_c
    real(rk8) :: egcm_c, rhos_c
    real(rk8) :: dum1,   dum2
   ...
   ! For other columns the downscaling is a simple copy.
    ...
    do f = 1, num_nolakec
       ...
       if (ityplun(l) == istice_mec) then   ! downscale to elevation classes
          ...
          Hbot   = rair*0.5D0*(tbot_g+tbot_c)/grav        ! scale ht at avg temp
