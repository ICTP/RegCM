...
!  real(rkx), dimension(:,:), pointer, contiguous :: snow_er
  ...
  subroutine allocate_mod_wsm5
    ...
    call getmem(qv,is,ie,1,kz,'wsm5::qv')
    call getmem(qci,is,ie,1,kz,1,2,'wsm5::qci')
    call getmem(qrs,is,ie,1,kz,1,2,'wsm5::qrs')
    call getmem(qs,is,ie,1,kz,1,2,'wsm5::qs')
    call getmem(rh,is,ie,1,kz,1,2,'wsm5::rh')
    call getmem(fall,is,ie,1,kz,1,2,'wsm5::fall')
    call getmem(den,is,ie,1,kz,'wsm5::den')
    call getmem(delz,is,ie,1,kz,'wsm5::delz')
    call getmem(p,is,ie,1,kz,'wsm5::p')
    !call getmem(cloud_er,is,ie,1,kz,'wsm5::cloud_er')
    !call getmem(ice_er,is,ie,1,kz,'wsm5::ice_er')
    !call getmem(snow_er,is,ie,1,kz,'wsm5::snow_er')
    call getmem(ptfac,is,ie,'wsm5::ptfac')
    call getmem(rain,is,ie,'wsm5::rain')
    call getmem(snow,is,ie,'wsm5::snow')
...
!    end do
  ...
  subroutine wsm52d(delt,ims,ime)
    ...
    real(rkx) :: supcol, supcolt, coeres, supsat, dtcld, xmi, tr, &
      ...
      xlf, pfrzdtc, pfrzdtr, supice, temp, rdtcld, n0sfac
    ...
    do loop = 1, loops
      ...
      do concurrent ( i = ims:ime, k = kz:1:-1 )
        ...
        if ( qrs(i,k,2) <= 0.0_rkx ) works(i,k) = d_zero
      end do

      call nislfv_rain_plm(ims,ime,den,denfac,t, &
      ...
      do concurrent ( i = ims:ime, k = kz:1:-1 )
        ...
        if ( t(i,k) > tzero .and. qrs(i,k,2) > 0.0_rkx ) then
          !
      ...
      do concurrent ( i = ims:ime, k = 1:kz )
        ...
        !        (t0>t>-40c: c->i)
        !
        if ( supcol > d_zero .and. qci(i,k,1) > 0.0_rkx ) then
