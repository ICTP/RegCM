...
! local pointers to implicit out arrays
   real(rk8), pointer :: col_begcb(:)   ! carbon mass, beginning of time step (gC/m**2)
...
!
! local pointers to implicit out arrays
   real(rk8), pointer :: col_begnb(:)   ! nitrogen mass, beginning of time step (gN/m**2)
!
...
! local pointers to implicit out arrays
   ...
   real(rk8), pointer :: col_begcb(:)    ! carbon mass, beginning of time step (gC/m**2)
   real(rk8), pointer :: col_endcb(:)    ! carbon mass, end of time step (gC/m**2)
   real(rk8), pointer :: col_errcb(:)    ! carbon balance error for the timestep (gC/m**2)
!
! !OTHER LOCAL VARIABLES:
...
!-----------------------------------------------------------------------
   ...
   if (err_found) then
      ...
      write(stderr,*)'Latdeg,Londeg=',clm3%g%latdeg(clm3%g%l%c%gridcell(c)),clm3%g%londeg(clm3%g%l%c%gridcell(c))
...
! local pointers to implicit in arrays
   ...
   real(rk8), pointer :: som_n_leached(:)    ! total SOM N loss from vertical transport
!
...
! local pointers to implicit out arrays
   ...
   real(rk8), pointer :: col_begnb(:)             ! nitrogen mass, beginning of time step (gN/m**2)
   real(rk8), pointer :: col_endnb(:)             ! nitrogen mass, end of time step (gN/m**2)
   real(rk8), pointer :: col_errnb(:)             ! nitrogen balance error for the timestep (gN/m**2)
