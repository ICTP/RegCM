...
module mod_sst_cmip6
  ...
  character(len=*) , parameter , public :: miroc6_version = 'v20191114'
  ...
  contains
    ...
    recursive subroutine read_sst_miroc6(idate,v)
      implicit none
      type(rcm_time_and_date) , intent(in) :: idate
      type(cmip6_2d_var) , intent(inout) :: v
      integer(ik4) :: istatus , idimid , it , irec
      ...
      if ( v%ncid == -1 ) then
        ...
        if ( year < 2015 ) then
          ...
          write(v%filename,'(a,i4,a,i4,a)') &
            trim(cmip6_path(year,'Oday',miroc6_version,v%vname)), &
            y1,'0101-',y2,'1231.nc'
        else
