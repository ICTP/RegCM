...
! enter define mode
      iret = nf_create('sstERAIN.1989-2009.00.nc', NF_CLOBBER, ncid)
      call check_err(iret)
! define dimensions
...
! define variables
      ...
      call check_err(iret)
      sst_dims(3) = time_dim
      sst_dims(2) = latitude_dim
      sst_dims(1) = longitude_dim
      iret = nf_def_var(ncid, 'sst', NF_INT2, sst_rank, sst_dims, sst_id)
      call check_err(iret)
! assign attributes
      ...
      iret = nf_put_att_double(ncid, sst_id, 'scale_factor', NF_DOUBLE, 1, doubleval)
      call check_err(iret)
      doubleval(1) = 288.1350402832031
      iret = nf_put_att_double(ncid, sst_id, 'add_offset', NF_DOUBLE, 1, doubleval)
      call check_err(iret)
      int2val(1) = -32767
      iret = nf_put_att_int(ncid, sst_id, '_FillValue', NF_INT2, 1, int2val)
      call check_err(iret)
      int2val(1) = -32767
      iret = nf_put_att_int(ncid, sst_id, 'missing_value', NF_INT2, 1, int2val)
      call check_err(iret)
      iret = nf_put_att_text(ncid, sst_id, 'units', 1, 'K')
      call check_err(iret)
      iret = nf_put_att_text(ncid, sst_id, 'long_name', 23, 'Sea surface temperature')
      call check_err(iret)
...
! Write record variables
      call writerecs(ncid,time_id,sst_id)
       
      iret = nf_close(ncid)
      call check_err(iret)
      end
       
      subroutine writerecs(ncid,time_id,sst_id)
...
! starts and counts for array sections of record variables
      ...
      integer  sst_start(sst_rank), sst_count(sst_rank)
...
! data variables
      ...
      integer  sst_nr
      parameter (sst_nr = 29824)
      integer*2  sst(longitude_len, latitude_len, sst_nr)
       
      open(21,file='sstI2.dat',form='unformatted',recl=240*121*2,access='direct')
      do i=1,29824
         time(i) = i*6+780162
         read(21,rec=i) sst(:,:,i)
      enddo
...
! store sst
      ...
      sst_start(2) = 1
      sst_start(3) = 1
      sst_count(1) = longitude_len
      sst_count(2) = latitude_len
      sst_count(3) = sst_nr
      iret = nf_put_vara_int2(ncid, sst_id, sst_start, sst_count, sst)
      call check_err(iret)
