...
module mod_micro_nogtom
  ...
  real(rkx), pointer, contiguous, dimension(:,:,:):: sumh0, sumq0
  real(rkx), pointer, contiguous, dimension(:,:,:) :: sumh1, sumq1
  real(rkx), pointer, contiguous, dimension(:,:) :: errorq, errorh
  real(rkx), pointer, contiguous, dimension(:,:,:):: tentkp
  real(rkx), pointer, contiguous, dimension(:,:,:,:) :: tenqkp
  ! distance from the top of the cloud
  real(rkx), pointer, contiguous, dimension(:,:,:) :: cldtopdist
  ! Mass variables
  ! Microphysics
  ! for convection detrainment source and subsidence source/sink terms
  real(rkx), pointer, contiguous, dimension(:,:,:) :: eewmt
  ! fluxes convergence of species
  real(rkx), pointer, contiguous, dimension(:,:,:) :: qliq

  real(rkx), pointer, contiguous, dimension(:,:,:) :: eew
  ! ice water saturation
  real(rkx), pointer, contiguous, dimension(:,:,:) :: qsice
  ! diagnostic mixed phase RH
  real(rkx), pointer, contiguous, dimension(:,:,:) :: qsmix
  ! Storage for eeliq, eeice
  real(rkx), pointer, contiguous, dimension(:,:,:) :: eeliq
  real(rkx), pointer, contiguous, dimension(:,:,:) :: eeice
  ! water/ice saturation mixing ratio
  real(rkx), pointer, contiguous, dimension(:,:,:) :: eeliqt
  ! liq+rain sedim flux
  real(rkx), pointer, contiguous, dimension(:,:,:) :: pfplsl
  ! ice+snow sedim flux
  real(rkx), pointer, contiguous, dimension(:,:,:) :: pfplsn
  ! decoupled temperature tendency
  real(rkx), pointer, contiguous, dimension(:,:,:) :: ttendc
  ! critical factors
  real(rkx), pointer, contiguous, dimension(:,:) :: xlcrit
  real(rkx), pointer, contiguous, dimension(:,:) :: rhcrit
  ! fall speeds of three categories
  real(rkx), dimension(mxqx) :: vqx
  ! decoupled mixing ratios tendency
  real(rkx), pointer, contiguous, dimension(:,:,:,:) :: qxtendc
  ! j,i,n ! generalized precipitation flux
  real(rkx), pointer, contiguous, dimension(:,:,:,:) :: pfplsx
  real(rkx), pointer, contiguous, dimension(:,:,:,:) :: qx
  real(rkx), pointer, contiguous, dimension(:,:,:) :: tx
  real(rkx), pointer, contiguous, dimension(:,:,:) :: pf
  real(rkx), pointer, contiguous, dimension(:,:,:) :: qdetr
  real(rkx), pointer, contiguous, dimension(:,:,:) :: delz
  real(rkx), pointer, contiguous, dimension(:,:,:) :: ph
  real(rkx), pointer, contiguous, dimension(:,:,:) :: rho
  real(rkx), pointer, contiguous, dimension(:,:,:) :: ccn
  real(rkx), pointer, contiguous, dimension(:,:,:) :: pverv
  real(rkx), pointer, contiguous, dimension(:,:,:) :: heatrt
  real(rkx), pointer, contiguous, dimension(:,:,:) :: fcc
  real(rkx), pointer, contiguous, dimension(:,:,:) :: remrat
  ! saturation mixing ratio with respect to water
  real(rkx), pointer, contiguous, dimension(:,:,:) :: qsliq
  ! koop
  ...
  ! nucleation
  real(rkx), pointer, contiguous, dimension(:,:,:) :: koop
  ! Delta pressure
...
#include <esatice.inc>
  ...
  subroutine allocate_mod_nogtom
    implicit none
    call getmem(qliq,1,kz,jci1,jci2,ici1,ici2,'cmicro:qliq')
    call getmem(eewmt,1,kz,jci1,jci2,ici1,ici2,'cmicro:eewmt')
    call getmem(qsmix,1,kz,jci1,jci2,ici1,ici2,'cmicro:qsmix')
    call getmem(ttendc,1,kz,jci1,jci2,ici1,ici2,'cmicro:ttendc')
    call getmem(eew,1,kz,jci1,jci2,ici1,ici2,'cmicro:eew')
    call getmem(qsice,1,kz,jci1,jci2,ici1,ici2,'cmicro:qsice')
    call getmem(qx,1,nqx,1,kz,jci1,jci2,ici1,ici2,'cmicro:qx')
    call getmem(tx,1,kz,jci1,jci2,ici1,ici2,'cmicro:tx')
    call getmem(qdetr,1,kz,jci1,jci2,ici1,ici2,'cmicro:qdetr')
    call getmem(delz,1,kz,jci1,jci2,ici1,ici2,'cmicro:delz')
    call getmem(ph,1,kz,jci1,jci2,ici1,ici2,'cmicro:ph')
    call getmem(rho,1,kz,jci1,jci2,ici1,ici2,'cmicro:rho')
    call getmem(pverv,1,kz,jci1,jci2,ici1,ici2,'cmicro:pverv')
    call getmem(heatrt,1,kz,jci1,jci2,ici1,ici2,'cmicro:heatrt')
    call getmem(fcc,1,kz,jci1,jci2,ici1,ici2,'cmicro:fcc')
    if ( ichem == 1 ) then
      call getmem(remrat,1,kz,jci1,jci2,ici1,ici2,'cmicro:remrat')
      if ( iaerosol == 1 .and. iindirect == 2 ) then
        call getmem(ccn,1,kz,jci1,jci2,ici1,ici2,'cmicro:ccn')
      end if
    end if
    call getmem(pf,1,kzp1,jci1,jci2,ici1,ici2,'cmicro:pf')
    call getmem(qsliq,1,kz,jci1,jci2,ici1,ici2,'cmicro:qsliq')
    call getmem(cldtopdist,1,kz,jci1,jci2,ici1,ici2,'cmicro:cldtopdist')
    call getmem(pfplsl,1,kzp1,jci1,jci2,ici1,ici2,'cmicro:pfplsl')
    call getmem(pfplsn,1,kzp1,jci1,jci2,ici1,ici2,'cmicro:pfplsn')
    call getmem(koop,1,kz,jci1,jci2,ici1,ici2,'cmicro:koop')
    call getmem(xlcrit,jci1,jci2,ici1,ici2,'cmicro:xlcrit')
    call getmem(rhcrit,jci1,jci2,ici1,ici2,'cmicro:rhcrit')
    call getmem(eeliq,1,kz,jci1,jci2,ici1,ici2,'cmicro:eeliq')
    call getmem(eeice,1,kz,jci1,jci2,ici1,ici2,'cmicro:eeice')
    call getmem(eeliqt,1,kz,jci1,jci2,ici1,ici2,'cmicro:eeliqt')
    call getmem(qxtendc,1,nqx,1,kz,jci1,jci2,ici1,ici2,'cmicro:qxtendc')
    call getmem(pfplsx,1,nqx,1,kzp1,jci1,jci2,ici1,ici2,'cmicro:pfplsx')
    call getmem(dpfs,1,kz,jci1,jci2,ici1,ici2,'cmicro:dpfs')
    if ( budget_compute ) then
      call getmem(sumq0,1,kz,jci1,jci2,ici1,ici2,'cmicro:sumq0')
      call getmem(sumh0,1,kz,jci1,jci2,ici1,ici2,'cmicro:sumh0')
      call getmem(sumq1,1,kz,jci1,jci2,ici1,ici2,'cmicro:sumq1')
      call getmem(sumh1,1,kz,jci1,jci2,ici1,ici2,'cmicro:sumh1')
      call getmem(errorq,jci1,jci2,ici1,ici2,'cmicro:errorq')
      call getmem(errorh,jci1,jci2,ici1,ici2,'cmicro:errorh')
      call getmem(tenqkp,1,nqx,1,kz,jci1,jci2,ici1,ici2,'cmicro:tenqkp')
      call getmem(tentkp,1,kz,jci1,jci2,ici1,ici2,'cmicro:tentkp')
    end if
 ...
 !   implicit none
 !   real(rkx), pointer, contiguous, intent(inout), dimension(:,:) :: zsqa, zsqb
 !   real(rkx), pointer, contiguous, intent(inout), dimension(:) :: fg
