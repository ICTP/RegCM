...
module mod_cu_kf
  ...
  integer(ik4), dimension(:), pointer, contiguous :: imap, jmap
  real(rkx), dimension(:,:), pointer, contiguous :: u0, v0, z0, t0, q0, p0
  real(rkx), dimension(:,:), pointer, contiguous :: qes, ql0, qi0, qr0, qs0
  real(rkx), dimension(:,:), pointer, contiguous :: rho, dzq, w0avg, tke
  real(rkx), dimension(:), pointer, contiguous :: pratec
  real(rkx), dimension(:,:), pointer, contiguous :: qc_kf, qi_kf
  integer(ik4), dimension(:), pointer, contiguous :: ktop, kbot
  real(rkx), dimension(:,:), pointer, contiguous :: cldfra_dp_kf, cldfra_sh_kf
  real(rkx), dimension(:,:), pointer, contiguous :: pptliq, pptice

  real(rkx), dimension(:,:), pointer, contiguous :: dqdt, dtdt, dqcdt
  real(rkx), dimension(:,:), pointer, contiguous :: tpart_h, tpart_v

  ! IPPTLS == 2
  real(rkx), dimension(:,:), pointer, contiguous :: dqidt, dqrdt, dqsdt
  ...
  subroutine allocate_mod_cu_kf
    ...
    call getmem(imap,1,nipoi,'mod_cu_kf:imap')
    call getmem(jmap,1,nipoi,'mod_cu_kf:jmap')
    ii = 1
    do i = ici1, ici2
      do j = jci1, jci2
        if ( cuscheme(j,i) == 6 ) then
    ...
    call getmem(u0,1,kz,1,nipoi,'mod_cu_kf:u0')
    call getmem(v0,1,kz,1,nipoi,'mod_cu_kf:v0')
    call getmem(t0,1,kz,1,nipoi,'mod_cu_kf:t0')
    call getmem(z0,1,kz,1,nipoi,'mod_cu_kf:z0')
    call getmem(q0,1,kz,1,nipoi,'mod_cu_kf:q0')
    call getmem(qes,1,kz,1,nipoi,'mod_cu_kf:qes')
    call getmem(ql0,1,kz,1,nipoi,'mod_cu_kf:ql0')
    call getmem(qi0,1,kz,1,nipoi,'mod_cu_kf:qi0')
    call getmem(qr0,1,kz,1,nipoi,'mod_cu_kf:qr0')
    call getmem(qs0,1,kz,1,nipoi,'mod_cu_kf:qs0')
    call getmem(p0,1,kz,1,nipoi,'mod_cu_kf:p0')
    call getmem(rho,1,kz,1,nipoi,'mod_cu_kf:rho')
    call getmem(tke,1,kz,1,nipoi,'mod_cu_kf:tke')
    call getmem(dzq,1,kz,1,nipoi,'mod_cu_kf:dzq')
    call getmem(w0avg,1,kz,1,nipoi,'mod_cu_kf:w0avg')
    call getmem(dqdt,1,kz,1,nipoi,'mod_cu_kf:dqdt')
    call getmem(dqidt,1,kz,1,nipoi,'mod_cu_kf:dqidt')
    call getmem(dqcdt,1,kz,1,nipoi,'mod_cu_kf:dqcdt')
    call getmem(dqrdt,1,kz,1,nipoi,'mod_cu_kf:dqrdt')
    call getmem(dqsdt,1,kz,1,nipoi,'mod_cu_kf:dqsdt')
    call getmem(dtdt,1,kz,1,nipoi,'mod_cu_kf:dtdt')
    call getmem(cldfra_dp_kf,1,kz,1,nipoi,'mod_cu_kf:cldfra_dp_kf')
    call getmem(pptliq,1,kz,1,nipoi,'mod_cu_kf:pptliq')
    call getmem(pptice,1,kz,1,nipoi,'mod_cu_kf:pptice')
    call getmem(cldfra_sh_kf,1,kz,1,nipoi,'mod_cu_kf:cldfra_sh_kf')
    call getmem(qc_kf,1,kz,1,nipoi,'mod_cu_kf:qc_kf')
    call getmem(qi_kf,1,kz,1,nipoi,'mod_cu_kf:qi_kf')
    call getmem(pratec,1,nipoi,'mod_cu_kf:pratec')
    call getmem(ktop,1,nipoi,'mod_cu_kf:ktop')
    call getmem(kbot,1,nipoi,'mod_cu_kf:kbot')
    ...
    if ( ichem == 1 ) then
      do k = 1, kz
        ...
        do np = 1, nipoi
          i = imap(np)
          j = jmap(np)
          cu_convpr(j,i,kk) = pptliq(k,np) + pptice(k,np)
        end do
      end do
    end if
    do np = 1, nipoi
    ...
    do np = its, ite
      ...
      do
        ...
        if ( tlcl+dtlcl+dtrh < tenv ) then
          ...
          do nk = k, kl-1
            ...
            qliq(nk1) = qliq(nk)
            qice(nk1) = qice(nk)
            call tpmix2(p0(nk1,np),theteu(nk1),tu(nk1),qu(nk1),qliq(nk1), &
            ...
            if ( umf(nk)-udr(nk1) < d_10 ) then
              ...
              ! current model level.
              !
              pptliq(nk1,np) = qlqout(nk1)*umf(nk)
              pptice(nk1,np) = qicout(nk1)*umf(nk)
              trppt = trppt + pptliq(nk1,np) + pptice(nk1,np)
      ...
      if ( let == ltop ) then
        ...
        do nk = let+1, ltop
          ...
          if ( nk >= let+2 ) then
            trppt = trppt - pptliq(nk,np) - pptice(nk,np)
            pptliq(nk,np) = umf(nk-1)*qlqout(nk)
            pptice(nk,np) = umf(nk-1)*qicout(nk)
            trppt = trppt + pptliq(nk,np) + pptice(nk,np)
          end if
        end do
      end if
      ...
      if ( ishall == 1 ) then
        ...
        if ( (p0(kstart,np)-p0(lfs,np)) > 50.0e2_rkx ) then
          ...
          do nd = ldt, 1, -1
            ...
            !
            call tpmix2dd(p0(nd,np),theted(nd),tz(nd),qss)
            qsd(nd) = qss
            !
      ...
      do ncount = 1, maxiter
        ...
        do nk = k, ltopm1
          ...
          !
          thteeg(nk1) = envirtht(p0(nk1,np),tg(nk1),qg(nk1))
...
1030  format(' ',' P0(LET) = ',F6.1,' P0(LTOP) = ',F6.1,' VMFLCL =',    &
   ...
   1075   format(F8.2,3(F8.2),2(F8.3),F8.2,2F8.3,F8.2,6F8.3)
        ...
        do nk = 1, kl
          ...
          write(stdout,1090) k,p0(k,np)/d_100,z0(k,np),t0(k,np)-tzero, &
                  tg(k)-tzero,dtt,tuc,tdc,q0(k,np)*d_1000,qg(k)*d_1000,   &
   ...
   1090   format(I3,F7.2,F7.0,10F7.2,4F7.3,2F8.3)
        ...
        do nk = 1, kl
          k = kl - nk + 1
          write(stdout,'(8f11.3)') p0(k,np)/d_100,t0(k,np)-tzero, &
      ...
      if ( abs(err2) > 0.10_rkx .and. istop == 0 ) then
        ...
        do nk = 1, kl
          k = kl - nk + 1
          write(stdout,'(7f12.3)') p0(k,np)/d_100,t0(k,np)-tzero, &
...
1110 format(' ','INITIAL WATER =',E12.5,' FINAL WATER =',E12.5,   &
    ...
    subroutine dtfrznew(tu,p,thteu,qu,qfrz,qice)
      ...
      !
      rlc = 2.5e6_rkx - 2369.276_rkx*(tu-tzero)
      rls = 2833922.0_rkx - 259.532_rkx*(tu-tzero)
      rlf = rls - rlc
      cpp = cpd*(d_one + 0.89_rkx*qu)
      !
