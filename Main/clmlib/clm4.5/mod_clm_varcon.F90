module mod_clm_varcon
  ...
  real(rk8), public, parameter :: zlnd = 0.00085_rk8
  ! Roughness length for snow [m]
  real(rk8), public, parameter :: zsno = 0.000775_rk8
  ...
  ! Lake z (layers)
  real(rk8), public, pointer, contiguous, dimension(:) :: zlak
  ! Lake dz (thickness)
  real(rk8), public, pointer, contiguous, dimension(:) :: dzlak
  ! Soil z (layers)
  real(rk8), public, pointer, contiguous, dimension(:) :: zsoi
  ! Soil dz (thickness)
  real(rk8), public, pointer, contiguous, dimension(:) :: dzsoi
  ! Soil zi (interfaces)
  real(rk8), public, pointer, contiguous, dimension(:) :: zisoi
  ! Soil dz (thickness)
  real(rk8), public, pointer, contiguous, dimension(:) :: dzsoi_decomp
