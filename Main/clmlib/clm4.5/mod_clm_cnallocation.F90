...
!
   ...
   real(rk8), pointer :: psnsun(:)     ! sunlit leaf-level photosynthesis (umol CO2 /m**2/ s)
   real(rk8), pointer :: psnsha(:)     ! shaded leaf-level photosynthesis (umol CO2 /m**2/ s)

   real(rk8), pointer :: c13_psnsun(:) ! C13 sunlit leaf-level photosynthesis (umol CO2 /m**2/ s)
   real(rk8), pointer :: c13_psnsha(:) ! C13 shaded leaf-level photosynthesis (umol CO2 /m**2/ s)

   real(rk8), pointer :: c14_psnsun(:) ! C14 sunlit leaf-level photosynthesis (umol CO2 /m**2/ s)
   real(rk8), pointer :: c14_psnsha(:) ! C14 shaded leaf-level photosynthesis (umol CO2 /m**2/ s)
...
!
   ...
   real(rk8), pointer :: annmax_retransn(:)       ! annual max of retranslocated N pool
   ...
   real(rk8), pointer :: annsum_npp(:)            ! annual sum of NPP, for wood allocation
   ...
   real(rk8), pointer :: cpool_to_grainc_storage(:)        ! allocation to grain C storage (gC/m2/s)
   real(rk8), pointer :: npool_to_grainn(:)                ! allocation to grain N (gN/m2/s)
   real(rk8), pointer :: npool_to_grainn_storage(:)        ! allocation to grain N storage (gN/m2/s)
...
#else
   real(rk8), parameter :: compet_plant_no3   = 1.0 ! (unitless) relative compettiveness of plants for NO3
   real(rk8), parameter :: compet_plant_nh4   = 1.0 ! (unitless) relative compettiveness of plants for NH4
   real(rk8), parameter :: compet_decomp_no3  = 1.0 ! (unitless) relative competitiveness of immobilizers for NO3
   real(rk8), parameter :: compet_decomp_nh4  = 1.0 ! (unitless) relative competitiveness of immobilizers for NH4
   real(rk8), parameter :: compet_denit       = 1.0 ! (unitless) relative competitiveness of denitrifiers for NO3
   real(rk8), parameter :: compet_nit         = 1.0 ! (unitless) relative competitiveness of nitrifiers for NH4
   real(rk8) :: fpi_no3_vr(lbc:ubc,1:nlevdecomp)               ! fraction of potential immobilization supplied by no3(no units)
   real(rk8) :: fpi_nh4_vr(lbc:ubc,1:nlevdecomp)               ! fraction of potential immobilization supplied by nh4 (no units)
   real(rk8) :: sum_nh4_demand(lbc:ubc,1:nlevdecomp), sum_nh4_demand_scaled(lbc:ubc,1:nlevdecomp)
   real(rk8) :: sum_no3_demand(lbc:ubc,1:nlevdecomp), sum_no3_demand_scaled(lbc:ubc,1:nlevdecomp)
...
!-----------------------------------------------------------------------
   ...
   do fp=1,num_soilp
      ...
      if (ivt(p) >= npcropmin .and. grain_flag(p) == 1.D0) then
         ...
         avail_retransn(p) = (annmax_retransn(p)/2.D0)*(gpp(p)/annsum_potential_gpp(p))/dt
      else
...
!               (ivt(pfti(c)) >= npcropmin)) ) then
      ...
      do fc=1,num_soilc
         ...
         if (residual_plant_ndemand(c) .gt. 0.D0 ) then
            if (nlimit_nh4(c,j) .eq. 0) then
               residual_smin_nh4_vr(c,j) = max(smin_nh4_vr(c,j) - (actual_immob_vr(c,j) + smin_nh4_to_plant_vr(c,j) ) * dt, 0.D0)
      ...
      do fc=1,num_soilc
         ...
         if (residual_plant_ndemand(c) .gt. 0.D0 ) then
            if (nlimit_no3(c,j) .eq. 0) then
               residual_smin_no3_vr(c,j) = max(smin_no3_vr(c,j) - (actual_immob_vr(c,j) + smin_no3_to_plant_vr(c,j) ) * dt, 0.D0)
   ...
   do fp=1,num_soilp
      ...
      if (woody(ivt(p)) == 1.D0) then
         ...
         cpool_to_deadcrootc_storage(p) = nlc * f2 * f3 * (1.D0 - f4) * (1.D0 - fcur)
      end if
      if (ivt(p) >= npcropmin) then ! skip 2 generic crops
         cpool_to_livestemc(p)          = nlc * f3 * f4 * fcur
         cpool_to_livestemc_storage(p)  = nlc * f3 * f4 * (1.D0 - fcur)
      ...
      if (woody(ivt(p)) == 1.D0) then
         ...
         npool_to_deadstemn_storage(p)  = (nlc * f3 * (1.D0 - f4) / cndw) * (1.D0 - fcur)
         npool_to_livecrootn(p)         = (nlc * f2 * f3 * f4 / cnlw) * fcur
         npool_to_livecrootn_storage(p) = (nlc * f2 * f3 * f4 / cnlw) * (1.D0 - fcur)
         npool_to_deadcrootn(p)         = (nlc * f2 * f3 * (1.D0 - f4) / cndw) * fcur
         npool_to_deadcrootn_storage(p) = (nlc * f2 * f3 * (1.D0 - f4) / cndw) * (1.D0 - fcur)
      end if
      if (ivt(p) >= npcropmin) then ! skip 2 generic crops
         cng = graincn(ivt(p))
         npool_to_livestemn(p)          = (nlc * f3 * f4 / cnlw) * fcur
