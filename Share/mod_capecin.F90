...
#include <pfesat.inc>
  ...
  !
  pure subroutine getcape_new(j1,j2,i1,i2,nk,p,t,rh,cape,cin,td,pi,q,th,thv,z)
    implicit none
    integer(ik4), intent(in) :: i1, i2, j1, j2, nk
    real(rkx), dimension(:,:,:), pointer, contiguous, intent(in) :: t, p, rh
    real(rkx), dimension(:,:,:), pointer, contiguous, intent(inout) :: td
    real(rkx), dimension(:,:,:), pointer, contiguous, intent(inout) :: pi
    real(rkx), dimension(:,:,:), pointer, contiguous, intent(inout) :: q
    real(rkx), dimension(:,:,:), pointer, contiguous, intent(inout) :: th
    real(rkx), dimension(:,:,:), pointer, contiguous, intent(inout) :: thv
    real(rkx), dimension(:,:,:), pointer, contiguous, intent(inout) :: z
    real(rkx), dimension(:,:), pointer, contiguous, intent(inout) :: cape, cin

    logical :: doit, ice, not_converged
    integer(ik4) :: kmax, n, nloop, i, j, k, iter

    real(rkx) :: the, maxthe, parea, narea
    real(rkx) :: th1, p1, t1, qv1, ql1, qi1, b1
    real(rkx) :: qt, dp, dz, frac, zm1
    ...
    do concurrent( j = j1:j2, i = i1:i2, k = 1:nk )
      pi(j,i,k) = (p(j,i,k)*rp00)**rddcp
      td(j,i,k) = getdewp(t(j,i,k)-tzero,rh(j,i,k))
      q(j,i,k) = getqvs(p(j,i,k),td(j,i,k))
      th(j,i,k) = t(j,i,k)/pi(j,i,k)
      thv(j,i,k) = th(j,i,k)*(d_one+reps*q(j,i,k))/(d_one+q(j,i,k))
    end do
    ...
    do concurrent( j = j1:j2, i = i1:i2 )
      ...
      !$acc loop seq
      do k = 2, nk
        dz = -cpdg*0.5_rkx*(thv(j,i,k)+thv(j,i,k-1))*(pi(j,i,k)-pi(j,i,k-1))
        z(j,i,k) = zm1 + dz
        zm1 = z(j,i,k)
      end do
      ...
      select case (source)
        ...
        case (2)
          ...
          if ( p(j,i,1) < 50000.0_rkx ) then
            ! first report is above 500 mb ... just use the first level reported
            maxthe = getthe(p(j,i,1),t(j,i,1),td(j,i,1),q(j,i,1))
          else
            ! find max thetae below 500 mb
            maxthe = d_zero
            !$acc loop seq
            do k = 1, nk
              if ( p(j,i,k) >= 50000.0_rkx ) then
                the = getthe(p(j,i,k),t(j,i,k),td(j,i,k),q(j,i,k))
                if ( the > maxthe ) then
                  maxthe = the
                  kmax = k
                end if
        ...
        case default
          ! use mixed layer
          if ( (z(j,i,2)-z(j,i,1)) > ml_depth ) then
            ! the second level is above the mixed-layer depth:  just use the
            ! lowest level
            avgth = th(j,i,1)
            avgqv = q(j,i,1)
          else if ( z(j,i,nk) < ml_depth ) then
            ...
            avgqv = d_zero
            k = 2
            do while ( (z(j,i,k) <= ml_depth) .and. (k <= nk) )
              avgth = avgth + &
                      0.5_rkx*(z(j,i,k)-z(j,i,k-1))*(th(j,i,k)+th(j,i,k-1))
              avgqv = avgqv + &
                      0.5_rkx*(z(j,i,k)-z(j,i,k-1))*(q(j,i,k)+q(j,i,k-1))
              k = k + 1
            end do
            th2 = th(j,i,k-1) + (th(j,i,k)-th(j,i,k-1)) * &
                    (ml_depth-z(j,i,k-1))/(z(j,i,k)-z(j,i,k-1))
            qv2 =  q(j,i,k-1)+( q(j,i,k)- q(j,i,k-1)) * &
                    (ml_depth-z(j,i,k-1))/(z(j,i,k)-z(j,i,k-1))
            avgth = avgth + 0.5_rkx*(ml_depth-z(j,i,k-1))*(th2+th(j,i,k-1))
            avgqv = avgqv + 0.5_rkx*(ml_depth-z(j,i,k-1))*(qv2+q(j,i,k-1))
          ...
          p2   = p(j,i,kmax)
          t2   = th2*pi2
          b2   = egrav*( thv2-thv(j,i,kmax) )/thv(j,i,kmax)
      end select
      ...
      do while ( doit .and. (k < nk) )
        ...
        do n = 1, nloop
          ...
          iter = 0
          not_converged = .true.
          do while ( not_converged .and. iter < 100 )
            iter = iter + 1
            t2 = thlast*pi2
            if ( ice ) then
        ...
        b2 = egrav*(thv2-thv(j,i,k))/thv(j,i,k)
        dz = -cpdg*d_half*(thv(j,i,k)+thv(j,i,k-1))*(pi(j,i,k)-pi(j,i,k-1))
        ...
        cape(j,i) = cape(j,i) + max(d_zero,parea)

        if ( (p(j,i,k) <= 10000.0_rkx) .and. (b2 < d_zero) ) then
