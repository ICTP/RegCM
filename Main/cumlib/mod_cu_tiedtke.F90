...
module mod_cu_tiedtke
  ...
  !    EXTERNALS
  !    ---------
  !    *MOISTADJ* ADJUST T AND Q DUE TO CONDENSATION IN ASCENT
  !    *CUENTR*   CALCULATE ENTRAINMENT/DETRAINMENT RATES
  !    *CUBASMC*  CALCULATE CLOUD BASE VALUES FOR MIDLEVEL CONVECTION
  !
  !    REFERENCE
  ...
  subroutine cuascn(kidia,kfdia,klon,klev,ptsphy,ptenh,pqenh,pten,pqen, &
    ...
    do jk = klev - 1 , 3 , -1
      ...
      !   -------------------------------------
      ik = jk
      call cuentrx(kidia,kfdia,klon,klev,ik,kcbot,ldcum,llo3, &
      ...
      if ( llo3 ) then
        ...
        icall = 1
        if ( jlm > 0 ) then
          call moistadj(kidia,kfdia,klon,klev,ik,zph,ptu,pqu,llflag,icall)
        end if
        do jll = 1 , jlm
          jl = jlx(jll)
          if ( pqu(jl,jk) /= zqold(jl) ) then
    ...
    !
    subroutine cuentrx(kidia,kfdia,klon,klev,kk,kcbot,ldcum,ldwork, &
    ...
    end subroutine cuentrx
    !
    ! THIS ROUTINE CALCULATES CLOUD BASE VALUES
    ! FOR MIDLEVEL CONVECTION INTERFACE
    !
    ! INPUT ARE ENVIRONMENTAL VALUES T,Q ETC
    ! IT RETURNS CLOUDBASE VALUES FOR MIDLEVEL CONVECTION
    ! METHOD.
  ...
  !
  ! TO PRODUCE T,Q AND L VALUES FOR CLOUD ASCENT INTERFACE
  !
  ! THIS ROUTINE IS CALLED FROM SUBROUTINES:
  ...
  !          FOR CONDENSATION CALCULATIONS.
  !
  subroutine moistadj(kidia,kfdia,klon,klev,kk,psp,pt,pq,ldflag,kcall)
    implicit none
    integer(ik4) , intent(in) :: klon
    integer(ik4) , intent(in) :: klev
    integer(ik4) , intent(in) :: kidia
...
#ifndef TESTME
      call fatal(__FILE__,__LINE__, &
                 'Unknown method kcall in moistadj')
#endif
    end if
  ...
  !
  subroutine cududvx(kidia,kfdia,klon,klev,ktopm2,ktype,kcbot,kctop,ldcum,  &
  ...
  subroutine custrat(kidia,kfdia,klon,klev,ldcum,ptsphy,pap,paph,pgeo,   &
    ...
    do jk = klev - 1 , ilevh , -1
      ...
      ik = jk
      icall = 1
      call moistadj(kidia,kfdia,klon,klev,ik,zpp,ztc,zqc,llflag,icall)
      do jl = kidia , kfdia
        if ( llbl(jl) ) then
...
!
  subroutine cumastrn(kidia,kfdia,klon,klev,ldland,ptsphy,pten,pqen,   &
    ...
    if ( lmfdudv ) then
      ...
      !by
      !   PUEN,PVEN,PMFU,PMFD
      call cududvx(kidia,kfdia,klon,klev,itopm2,ktype,kcbot,kctop, &
    ...
    subroutine cuinin(kidia,kfdia,klon,klev,pten,pqen,pqsen,puen,pven,  &
      ...
      do jk = 2 , klev
        ...
        ik = jk
        icall = 3
        call moistadj(kidia,kfdia,klon,klev,ik,zph,ptenh,pqsenh,llflag,icall)
        do jl = kidia , kfdia
          pqenh(jl,jk) = min(pqen(jl,jk-1),pqsen(jl,jk-1)) + &
                            (pqsenh(jl,jk)-pqsen(jl,jk-1))
          pqenh(jl,jk) = max(pqenh(jl,jk),d_zero)
    ...
    ! INPUT PARAMETERS (INTEGER):
    ! *KFLAG*         FLAG TO DETECT CALL FROM CONVECTION  KFLAG=1
    !                                          OTHER       KFLAG=2
    ...
    subroutine cudlfsn(kidia,kfdia,klon,klev,kcbot,kctop,ldcum,ptenh,pqenh, &
      ...
      if ( lmfdd ) then
        ...
        do jk = 3 , ike
          ...
          ik = jk
          icall = 2
          call moistadj(kidia,kfdia,klon,klev,ik,zph,ztenwb,zqenwb,llo2,icall)
          !     2.2          DO MIXING OF CUMULUS AND ENVIRONMENTAL AIR
          !     AND CHECK FOR NEGATIVE BUOYANCY.
          !     THEN SET VALUES FOR DOWNDRAFT AT LFS.
          !     ----------------------------------------
    ...
    subroutine cuddrafn(kidia,kfdia,klon,klev,lddraf,ptenh,pqenh, &
      ...
      do jk = 3 , klev
        ...
        ik = jk
        icall = 2
        call moistadj(kidia,kfdia,klon,klev,ik,zph,ptd,pqd,llo2,icall)
        do jl = kidia , kfdia
          if ( llo2(jl) ) then
            zcond(jl) = zcond(jl) - pqd(jl,jk)
            zbuo = ptd(jl,jk)*(d_one+ep1*pqd(jl,jk)) - &
...
program testgate
  ...
  ! closure instead by putting the switch LMFSCL_WSTAR=.true.
