...
!
  ...
  integer, parameter, private ::  rundef = -9999999.D0
!
...
!
  ...
  real(rk8), public :: co2_ppmv     = 355.D0            ! atmospheric CO2 molar ratio (by volume) (umol/mol)
...
! NOTE: glc_smb must have the same values for CLM and GLC
  ...
  real(rk8), public, allocatable :: glc_topomax(:)           ! upper limit of each class (m)  (set in surfrd)
...
!
  ...
  real(rk8), public :: scmlat        = rundef            ! single column lat
  real(rk8), public :: scmlon        = rundef            ! single column lon
!
! instance control
!
...
!
  real(rk8), public :: pertlim = 0.0D0                  ! perturbation limit when doing error growth test
!
...
! !ARGUMENTS:
  ...
  real(rk8),           optional, intent(IN) :: scmlat_in                ! single column lat
  real(rk8),           optional, intent(IN) :: scmlon_in                ! single column lon
