  ...
  subroutine SurfaceRunoff (lbc, ubc, num_hydrologyc, &
    ...
    do j = 1 , nlevsoi
      do fc = 1 , num_hydrologyc
        ...
        if ( origflag == 1 ) then
          icefrac(c,j) = min(1.D0,h2osoi_ice(c,j)/(h2osoi_ice(c,j) + &
                                  h2osoi_liq(c,j)))
        else
          icefrac(c,j) = min(1.D0,vol_ice(c,j)/watsat(c,j))
        end if

        fracice(c,j) = max(0.D0,exp(-3.D0*(1.D0-icefrac(c,j))) - &
    ...
    do fc = 1 , num_hydrologyc
      ...
      if ( frost_table(c) > zwt(c) ) then
        ...
        if ( frost_table(c) > zwt_perched(c)) then
          fsat(c) = wtfact(c) * exp(-0.5D0*fff(c)*zwt_perched(c))
  ...
  subroutine Infiltration(lbc, ubc, num_hydrologyc, filter_hydrologyc, &
    ...
    do fc = 1, num_hydrologyc
      ...
      if ( ltype(clandunit(c)) == istsoil .or. &
        ...
        qflx_in_soil(c) = (1.D0 - frac_h2osfc(c)) * &
        ...
        qflx_in_soil(c) = qflx_in_soil(c) - &
                (1.0D0 - fsno - frac_h2osfc(c))*qflx_evap(c)
        ...
        qinmax = (1.D0 - fsat(c)) * &
                10.D0**(-e_ice*top_icefrac)*(qflx_in_soil(c) - rsurf_vic)
#else
        qinmax = (1.D0 - fsat(c)) * &
                minval(10.D0**(-e_ice*(icefrac(c,1:3)))*hksat(c,1:3))
#endif
        qflx_infl_excess(c) = max(0.D0,qflx_in_soil(c) - &
                (1.0D0 - frac_h2osfc(c))*qinmax)
        ...
        if ( h2osfc(c) >= h2osfc_thresh(c) .and. &
          ...
          k_wet = 1.0D0 * sin((rpi/180.0D0) * topo_slope(c))
        ...
        ! cutoff lower limit
        if ( qflx_h2osfc_surf(c) < 1.0D-8 ) qflx_h2osfc_surf(c) = 0.D0
    ...
    do j = 1 , nlevsoi
      do fc = 1 , num_hydrologyc
        ...
        ! and the equilibrium solution for the rest of the layer
          ...
          voleq1 = -sucsat(c,j)*watsat(c,j)/(1.D0-1.D0/bsw(c,j)) / &
          ...
          vol_eq(c,j) = -sucsat(c,j)*watsat(c,j) / &
                  (1.D0-1.D0/bsw(c,j))/(zimm(c,j)-zimm(c,j-1))*(tempi-temp0)
          vol_eq(c,j) = max(vol_eq(c,j),0.0D0)
          vol_eq(c,j) = min(watsat(c,j),vol_eq(c,j))
        end if
        zq(c,j) = -sucsat(c,j) * &
                (max(vol_eq(c,j)/watsat(c,j),0.01D0))**(-bsw(c,j))
    ...
    do fc = 1, num_hydrologyc
      ...
      if ( jwt(c) == nlevsoi ) then
        ...
        vol_eq(c,j+1) = -sucsat(c,j)*watsat(c,j) / &
                (1.D0-1.D0/bsw(c,j))/(zwtmm(c)-zimm(c,j))*(tempi-temp0)
        vol_eq(c,j+1) = max(vol_eq(c,j+1),0.0D0)
        vol_eq(c,j+1) = min(watsat(c,j),vol_eq(c,j+1))
        zq(c,j+1) = -sucsat(c,j) * &
                (max(vol_eq(c,j+1)/watsat(c,j),0.01D0))**(-bsw(c,j))
...
!-----------------------------------------------------------------------
    ...
    do fc = 1 , num_hydrologyc
      ...
      if ( jwt(c) == nlevsoi ) then
        ...
        if ( qcharge_tot > 0.0D0 ) then !rising water table
          do j = jwt(c)+1 , 1 , -1
            ! use analytical expression for specific yield
            s_y = watsat(c,j) * ( 1.0D0 - &
                    (1.0D0+1.D3*zwt(c)/sucsat(c,j))**(-1.0D0/bsw(c,j)))
            s_y = max(s_y,0.02D0)
            qcharge_layer = min(qcharge_tot,(s_y*(zwt(c) - zi(c,j-1))*1.D3))
          ...
          do j = jwt(c)+1 , nlevsoi
            ...
            s_y = watsat(c,j) * ( 1.0D0 - &
                    (1.0D0 + 1.D3*zwt(c)/sucsat(c,j))**(-1.0D0/bsw(c,j)))
            s_y = max(s_y,0.02D0)
            qcharge_layer = max(qcharge_tot,-(s_y*(zi(c,j) - zwt(c))*1.D3))
            qcharge_layer = min(qcharge_layer,0.D0)
            qcharge_tot = qcharge_tot - qcharge_layer

            if ( qcharge_tot >= 0.0D0 ) then
          ...
          if ( qcharge_tot > 0.0D0 ) zwt(c) = zwt(c) - qcharge_tot/1000.D0/rous
        end if
          ...
          do k = k_perch+1 , k_frz
            ...
            if ( rsub_top_tot >= 0.0D0 ) then
              zwt_perched(c) = zwt_perched(c) - &
                      rsub_top_layer/eff_porosity(c,k)/1000.D0
              exit
            else
    ...
    do fc = 1 , num_hydrologyc
      c = filter_hydrologyc(fc)
      xs1(c) = max(max(h2osoi_liq(c,1),0.D0) - &
                   max(0.D0,(pondmx+watsat(c,1)*dzmm(c,1) - &
                       h2osoi_ice(c,1))),0.D0)
      h2osoi_liq(c,1) = min(max(0.D0,pondmx+watsat(c,1)*dzmm(c,1) - &
      ...
      ! add in ice check
      xs1(c) = max(max(h2osoi_ice(c,1),0.D0) - &
                   max(0.D0,(pondmx+watsat(c,1)*dzmm(c,1) - &
                   h2osoi_liq(c,1))),0.D0)
      h2osoi_ice(c,1) = min(max(0.D0,pondmx+watsat(c,1)*dzmm(c,1) - &
    ...
    do fc = 1 , num_hydrologyc
      ...
      if ( snl(c)+1 >= 1 ) then
        ...
        h2osoi_liq(c,1) = h2osoi_liq(c,1) + &
                (1.D0 - frac_h2osfc(c))*qflx_dew_grnd(c) * dtsrf
        h2osoi_ice(c,1) = h2osoi_ice(c,1) + &
                (1.D0 - frac_h2osfc(c))*qflx_dew_snow(c) * dtsrf
        if (qflx_sub_snow(c)*dtsrf > h2osoi_ice(c,1)) then
          qflx_sub_snow(c) = h2osoi_ice(c,1)/dtsrf
          h2osoi_ice(c,1) = 0.D0
        else
          h2osoi_ice(c,1) = h2osoi_ice(c,1) - &
                  (1.D0 - frac_h2osfc(c)) * qflx_sub_snow(c) * dtsrf
        end if
      end if
    end do
