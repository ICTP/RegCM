...
! INITIAL SENHCAT = 1 FOR ODD HYDROGEN; 0 FOR OTHER SPECIES             
      ...
      if(c_iter.eq.1) then 
        do 1610 ii=1,30 
         is = 0 
         if((ii.eq.8.or.ii.eq.9.or.ii.eq.10).or.ii.eq.3) is=1 
!           if(c_kkw.gt.0) write(c_out,*) ii, is                        
!                              do  kk=1,c_kmax ! kk vector loop         
          senhcat(kk,ii) =   float(is) 
!                                                                       
!                              enddo ! kk vector loop                   
 1610   continue 
                                                                        
!      if(c_kkw.gt.0) write(c_out,1611) (senhcat(c_kkw,ii),ii=1,20)     
...
! LOOP TO CALL SPECIES SOLUTION                                         
       do 1660 i=1,c_nchem2 
       if(c_cascade(i,1).eq.0) go to 1661 
       do ic=1,nsdim 
...
!   MAKE XR,  RP, RL=sum ; write gas-masters only                       
        ...
        do j=1,c_nchem2 
          if(c_npequil(j).eq.j) then 
            ...
            gamma(1) = rpro(c_kkw,1) - rloss(c_kkw,1) 
! TEMPORARY CHANGE (45  )                                               
            if(.not.c_lsts(ic)) gamma(1) = gamma(1)-beta(1) 
            write(c_out,1803) j, c_tchem(j)                             &
     &       ,xrr(c_kkw,1),  c_xcin(c_kkw,j), xcfinr(c_kkw,j)           &
     &       ,rloss(c_kkw,1), rpro(c_kkw,1)                             &
     &        ,beta(1), gamma(1)                                        
!    *       ,geomavg(c_kkw,j)                                          
                            !if(c_npequil(j).eq.j) then                 
          endif 
...
! OPTION - INCLUDE REACTIONS                                            
 ...
 1806   format(//,' REACTION RATES:  ITER =',i3, '    VECTOR KKW=',i3) 
         do      nr=1,c_nreac 
          write(c_out,1807) nr, (c_treac(j,nr),j=1,5), c_rr(c_kkw ,nr), &
     &    ratek(c_kkw ,nr)                                              
...
!   NOTE NIGHTTIME ADDED EXIT ABOVE: XOHTEST,FOHTEST*0.001.             
      ...
      if(c_kkw.gt.0) write(c_out,1701) c_iter,c_ohtest,c_notest 
 1701 format(/' TEST FOR OH,NO CONVERGENCE =',i3,2(1pe10.3)) 
                                                                        
        if(c_kmax.eq.1) then 
          if(ratek(1,  1 ).ge.1.00E-04.and.c_ohtest.lt.xfohtest)        &
     &     c_ohtest=xfohtest                                            
                                                                        
           if(c_iter.gt.3.and.(c_ohtest.lt.c_converge.and.              &
     &               c_notest.lt.c_converge))                           &
     &          go to 1710                                              
...
!  LOCAL VARIABLES - INPUT/OUTPUT                                       
      ...
      DIMENSION BX(100),XX(100), AX(100,100) 
      DOUBLE PRECISION AX, BX, XX 
                                                                        
! LOCAL VARIABLES:  INTERNAL                                            
                                   ! Summed factor for pair group       
      double precision xpair 
!                        net pair group  production w/ cons. mass.      
                                   ! Summed factor for multi group      
      double precision xmulti 
!                        net multi group  production w/ cons. mass.     
                                                ! Rate of self-reaction 
      double precision rself(c_kvec, 20) 
...
!           do  kk=1,c_kmax           ! kk vector loop                  
        llexpo(kk)=.FALSE. 
        if(c_lexpo(kk)) then 
         if( (nsolv.eq.1).and.(c_nequil(ncsol(1)).eq.0)                 &
     &        .and. (c_tchem(ncsol(1) ).ne.'    H2O2')                  &
     &        .and.(.not.c_lsts(ncsol(1)))   ) llexpo(kk)=.TRUE.        
!    *          .and. (c_icat(ncsol(1)).eq.1.or.c_icat(ncsol(1)).eq.2)  
                                   !if(c_lexpo) then                    
        endif 
!           enddo                   ! kk vector loop                    
!                                                                       
...
!          do kk=1,c_kmax    ! kk vector loop                           
             xrppair(kk   ) = xrppair(kk   ) +  xrp(kk,nc)              &
     &                                      * c_pairfac(ics)            
             rppair(kk,icpair) =  rppair(kk,icpair)  +  rpro(kk,nc)     &
     &                                      * c_pairfac(ics)            
             rlpair(kk,icpair) =  rlpair(kk,icpair)  +  rloss(kk,nc)    &
     &                                      * c_pairfac(ics)            
!            enddo           ! kk vector loop                           
! CHEMSOLVE DEBUG WRITE                                                 
!            if(c_kkw.gt.0)                                             
...
!          do kk=1,c_kmax    ! kk vector loop                           
             ...
             rpmulti(kk)       =  rpmulti(kk)        +  rpro(kk,nc)     &
     &                                      * c_multfac(ics)            
             rlmulti(kk)       =  rlmulti(kk)        +  rloss(kk,nc)    &
     &                                      * c_multfac(ics)            
!            enddo           ! kk vector loop                           
...
! FEBRUARY 2005 CHANGE - icr1=npequil is before if(icr2.gt.0) BUG FIX.  
           do  i=1,c_nnrchem(ics) 
             ...
             if(icr2.gt.0) then 
               icr2 = c_npequil(icr2) 
               if(icr1.eq.ics.and.icr2.eq.ics) lself=.TRUE. 
...
!                                                                       
               if(c_nppair(icr1,2).eq.c_nppair(icr2,2)) lself =.TRUE. 
                                                                        
               do ncc=1,nsolv 
                                                                        
                 if(ncc.ne.nc) then 
                   if(ncsolv(ncc).eq.icr1.or.ncsolv(ncc).eq.icr2) then 
                               nc2=ncc 
                               ics2 = ncsolv(ncc) 
                   endif 
                                       !if(ncc.ne.nc) then              
...
!            do kk=1,c_kmax      ! kk vector loop                       
!              rloss(kk,nc) = rloss(kk,nc) + c_rr(kk,nr)*c_stoiloss(nr) 
...
!              do kk=1,c_kmax      ! kk vector loop                     
                 rloss(kk,nc) = rloss(kk,nc) + c_rr(kk,nr) 
                 rself(kk,nc) = rself(kk,nc) + 2.*c_rr(kk,nr) 
!              enddo            ! kk vector loop                        
                                ! if(icr1.eq.icr2) then                 
             endif 
...
!                                                                       
             do ncc=1,nsolv 
               if( c_prodarr(nr,ncsolv(ncc)).ne.0.) then 
                icc = ncsolv(ncc) 
                                                                        
! Adjust LOSS if PRODUCT EQUALS REACTANT, skip PRO.                     
                if(ncc.eq.nc.or.ncc.eq.nc2) then 
...
!     to avoid doublecounting in RPM.)                                  
                 ...
                 if( (c_nppair(ics,1).eq.icc                            &
     &                 .or.c_nppair(icc,1 ).eq.ics)                     &
     &              .or.(nssolv(nc).ne.nssolv(ncc))                     &
     &                      )then                                       
!                  do kk=1,c_kmax      ! kk vector loop                 
...
! CHANGE                                                                
               if(nc2.gt.0)                       then 
                 ...
                 if( (c_nppair(ics2,1).eq.icc                           &
     &                 .or.c_nppair(icc,1 ).eq.ics2)                    &
     &                      )then                                       
...
! END CPRO FROM 2ND REACTANT                                            
                                  !if( (c_nppair(ics2,1).eq.icc then    
                 endif 
...
!          endif         !if(c_kkw.gt.0.and.nsolv.gt.1)                 
...
!                cpro(kk,nc,ncc) = cpro(kk,nc,ncc) * xrr(kk,nc)         
!                rpro(kk,ncc) = rpro(kk,ncc) + cpro(kk,nc,ncc)          
! c              enddo               ! kk vector loop                   
!            endif              !if(nssolv(nc).eq.nssolv(ncc))          
!          enddo              !do ncc=(nc+1), nsolv                     
!        endif           !if(nsolv.gt.nc)                               
...
!    *      ,/,a8,2x,i5,(8(1pe10.3)))                                   
!            endif       !if(c_nppair(ic,2).eq.ic)                      
!          enddo          !do nc=1, nsolv                               
!        endif         !if(c_kkw.gt.0)   ... end debug write            
!                                                                       
!  -------                                                              
...
!  CUT JANUARY 2005                                                     
!             endif               ! if(ic2.ne.-1.and.ic2.ne.-3) then    
...
!           endif                !if(c_nicreac(nr).eq.ic1) then         
