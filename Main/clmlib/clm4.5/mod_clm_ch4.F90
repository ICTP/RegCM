module mod_clm_ch4
  ...
  ! checks for balance errors.
  ! Sources, sinks, "competition" for CH4 & O2, & transport are
  ! resolved in ch4_tran.
  !
  use mod_intkinds
  use mod_realkinds
  use mod_stdio
  ...
  subroutine ch4 (lbg,ubg,lbl,ubl,lbc,ubc,lbp,ubp,num_soilc,filter_soilc, &
    ...
    do fc = 1 , num_soilc
      ...
      qflx_surf_lag(c) = qflx_surf_lag(c) * exp(-dtsrf/qflxlags) &
                       + qflx_surf(c) * (1._rk8 - exp(-dtsrf/qflxlags))
      ...
      if (redoxlags > 0._rk8) then
        finundated_lag(c) = finundated_lag(c) * exp(-dtsrf/redoxlags) &
                          + finundated(c) * (1._rk8 - exp(-dtsrf/redoxlags))
      else
    ...
    do j = 1 , nlevsoi
      do fc = 1 , num_soilc
        ...
        if ( j == 1 ) then
          ch4_dfsat_flux(c) = 0._rk8
        end if

        if (fsat_bef(c) /= spval .and. &
            finundated(c) > fsat_bef(c)) then !Reduce conc_ch4_sat
             dfsat = finundated(c) - fsat_bef(c)
          conc_ch4_sat(c,j) = (fsat_bef(c)*conc_ch4_sat(c,j) + &
                  dfsat*conc_ch4_unsat(c,j)) / finundated(c)
          ...
          ch4_dfsat_flux(c) = ch4_dfsat_flux(c) + &
                  ...
                  conc_ch4_unsat(c,j)) * &
                  dz(c,j) / dtsrf * catomw / 1000._rk8 ! mol --> kg
        end if
      end do
    end do
    ...
    call ch4annualupdate(lbc, ubc, lbp, ubp, num_soilc, filter_soilc, &
    ...
    if ( nlevdecomp == 1 ) then
      ...
      do j = 1 , nlevsoi
        do fp = 1 , num_soilp
          ...
          else if (pwtc(p) < 0.99_rk8) then
            rootfraction(p,j) = spval
          else
            ! Set equal to uniform distribution
            rootfraction(p,j) = dz(c,j) / zi(c,nlevsoi)
          end if
        end do
      end do
    end if
    ...
    if ( nlevdecomp == 1 ) then
      call p2c (lbp, ubp, lbc, ubc, nlevgrnd, rootfraction, rootfr_col, 'unity')
    end if
    ! Needed to use non-filter form above so that spval would be
    ! treated properly.
    ...
    do sat = 0 , 1 ! 0 == unsaturated; 1 = saturated
          ...
          do fc = 1 , num_soilc
            ...
            if (j > jwt(c) .and. redoxlags_vertical > 0._rk8) then
              ...
              layer_sat_lag(c,j) = layer_sat_lag(c,j) * &
                      ...
                      + (1._rk8 - exp(-dtsrf/redoxlags_vertical))
            else if (redoxlags_vertical > 0._rk8) then
              layer_sat_lag(c,j) = layer_sat_lag(c,j) * &
                      exp(-dtsrf/redoxlags_vertical)
            else if (j > jwt(c)) then  ! redoxlags_vertical = 0
              layer_sat_lag(c,j) = 1._rk8
            else
              layer_sat_lag(c,j) = 0._rk8
            end if
          end do
        end do
      else ! saturated
      ...
      call ch4_prod (lbc, ubc, lbp, ubp, num_soilc, filter_soilc, &
              num_soilp, filter_soilp, jwt, sat, lake)
      ...
      call ch4_aere (lbc, ubc, lbp, ubp, num_soilc, filter_soilc, &
              num_soilp, filter_soilp, jwt, sat, lake)
    ...
    if ( allowlakeprod ) then
      ...
      call ch4_prod (lbc, ubc, lbp, ubp, num_lakec, filter_lakec, &
      ...
      call ch4_aere (lbc, ubc, lbp, ubp, num_lakec, filter_lakec, &
              0, dummyfilter, jwt, sat, lake)
      ! The p filter will not be used here; the relevant column vars
      ...
      do fc = 1 , num_soilc
        ...
        if ( j == 1 ) then
          ...
          ch4_surf_flux_tot(c) = (finundated(c)*totalsat + &
                  (1._rk8 - finundated(c))*totalunsat) * catomw / 1000._rk8
        ...
        ch4_oxid_tot(c) = ch4_oxid_tot(c) + &
             ...
             (1._rk8 - finundated(c))*ch4_oxid_depth_unsat(c,j))*dz(c,j) * catomw
        !Convert from mol to g C
        ch4_prod_tot(c) = ch4_prod_tot(c) + &
              (finundated(c)*ch4_prod_depth_sat(c,j) + &
              (1._rk8 - finundated(c))*ch4_prod_depth_unsat(c,j))*dz(c,j) * catomw
        !Convert from mol to g C
        if ( j == nlevsoi ) then
      ...
      do j = 1 , nlevsoi
        do fc = 1 , num_lakec
          ...
          if ( j == 1 ) then
            ...
            totalsat = ch4_surf_diff_sat(c) + &
                    ch4_surf_aere_sat(c) + ch4_surf_ebul_sat(c)
            ch4_surf_flux_tot(c) = totalsat*catomw / 1000._rk8
          end if

          ch4_oxid_tot(c) = ch4_oxid_tot(c) + &
                  ch4_oxid_depth_sat(c,j)*dz(c,j)*catomw
          ch4_prod_tot(c) = ch4_prod_tot(c) + &
                  ch4_prod_depth_sat(c,j)*dz(c,j)*catomw

          if ( .not. replenishlakec ) then
            !Adjust lake_soilc for production.
            lake_soilc(c,j) = lake_soilc(c,j) - &
                    2._rk8*ch4_prod_depth_sat(c,j)*dtsrf*catomw
          ...
          if ( j == nlevsoi ) then
            ...
            if ( .not. replenishlakec ) then
              nem_col(c) = nem_col(c) + ch4_prod_tot(c)
              ! Here this is positive because it is actually the CO2
              ! that comes off with the methane
              ! NOTE THIS MODE ASSUMES TRANSIENT CARBON SUPPLY FROM LAKES
    ...
    ! ch4_surf_flux_tot, ch4_oxid_tot, and ch4_prod_tot should be
    ...
    do j = 1 , nlevsoi
      do fc = 1 , num_soilc
        ...
        totcolch4(c) = totcolch4(c) + &
                ...
                (1._rk8-finundated(c))*conc_ch4_unsat(c,j))*dz(c,j)*catomw
        ! mol CH4 --> g C
  ...
  !
  ! Production is done below the water table, based on CN heterotrophic
  ! respiration. O2 is consumed by roots & by heterotrophic aerobes.
  ! Production is done separately for sat & unsat, and is adjusted
  ! for temperature, seasonal inundation, pH (optional), & redox lag factor.
  !
  subroutine ch4_prod (lbc,ubc,lbp,ubp,num_methc,filter_methc,num_methp, &
    ...
    ! number of soil points in pft filter
    integer(ik4), intent(in) :: num_methp
    ! pft filter for soil points
    ...
    do j = 1 , nlevsoi
      do fc = 1 , num_methc
        ...
        if (.not. lake) then
          ...
          ! Multiply base_decomp by factor accounting for lower carbon stock
          ! in seasonally inundated areas than if it were inundated all year.
          ! This is to reduce emissions in seasonally inundated zones,
          ...
          if (sat == 1) then
            ...
            if (.not. anoxia) then
              if (annavg_finrw(c) /= spval) then
                seasonalfin = max(finundated(c)-annavg_finrw(c), 0._rk8)
                if (seasonalfin > 0._rk8) then
...
! defined CN
          ...
          base_decomp = lake_decomp_fact * lake_soilc(c,j) * dz(c,j) * &
                        q10lake**( (t_soisno(c,j)-q10lakebase)/10._rk8) / catomw
          ! convert from g C to mol C
        end if

        ! For all landunits, prevent production or oxygen consumption
        ! when soil is at or below freezing.
        ...
        if (t_soisno(c,j) <= tfrz .and. &
            (nlevdecomp == 1 .or. lake)) base_decomp = 0._rk8

        ! depth dependence of production either from rootfr or decomp model
        if (.not. lake) then
          ! use default rootfr, averaged to the column level in the
          ! ch4 driver, or vert HR
          if (nlevdecomp == 1) then ! not VERTSOILC
            if (j <= nlev_soildecomp_standard) then
              ! Top 5 levels are also used in the CLM code for
              ! establishing temperature
              ! and moisture constraints on SOM activity
              partition_z = rootfr_col(c,j)*rootlitfrac + &
                   (1._rk8 - rootlitfrac)*dz(c,j)/zi(c,nlev_soildecomp_standard)
            else
              partition_z = rootfr_col(c,j)*rootlitfrac
            end if
          else
            if ( (somhr(c) + lithr(c)) > 0._rk8) then
        ...
        f_ch4_adj = 1.0_rk8
        if (.not. lake) then
          t_fact_ch4 = q10ch4**((t_soisno(c,j) - q10ch4base)/10._rk8)
        ...
        if (.not. lake .and. usephfact .and. &
          ...
          pH_fact_ch4 = 10._rk8**(-0.2235_rk8*pH(c)*pH(c) + 2.7727_rk8*pH(c) - 8.6_rk8)
        ...
        if ( (.not. lake) .and. sat == 1 .and. &
              finundated_lag(c) < finundated(c)) then
          f_ch4_adj = f_ch4_adj * finundated_lag(c) / finundated(c)
        else if (sat == 0 .and. j > jwt(c)) then
          ! Assume lag in decay of alternative electron acceptors vertically
          f_ch4_adj = f_ch4_adj * layer_sat_lag(c,j)
        end if
        ! Alternative electron acceptors will be consumed first after
        ! soil is inundated.

        f_ch4_adj = min(f_ch4_adj, 0.5_rk8)
        ...
        ! to the SOMHR calculated in CN, so that the NEE is sensible.
        ! Even perfectly anaerobic conditions with no alternative electron
        ! acceptors would predict no more than 0.5 b/c some oxygen is
        ! present in organic matter.
        ! e.g. 2CH2O --> CH4 + CO2.

        ! Decomposition uses 1 mol O2 per mol CO2 produced (happens below
        ! WT also, to deplete O2 below WT)
        ! o2_decomp_depth is the demand in the absense of O2 supply
        ! limitation, in addition to autotrophic respiration.
        ...
        if (.not. lake) then
          ...
          !         col_rr(c)*rootfr(c,j)/catomw/dz(c,j) ! mol/m^3/s
          o2_decomp_depth(c,j) = o2_decomp_depth(c,j) + &
                  rr_vr(c,j)/catomw/dz(c,j) ! mol/m^3/s
        ...
        if (.not. lake) then
          o2_decomp_depth(c,j) = o2_decomp_depth(c,j) + &
                  pot_f_nit_vr(c,j) * 2.0_rk8/14.0_rk8 ! g N/m^3/s  mol O2 / g N
        end if
#endif
        if (j > jwt(c)) then
          ...
          ch4_prod_depth(c,j) = f_ch4_adj * base_decomp * &
                  partition_z / dz (c,j)! [mol/m3-total/s]
        else ! Above the WT
          if (anoxicmicrosites) then
            ch4_prod_depth(c,j) = f_ch4_adj * base_decomp * &
                    partition_z / dz (c,j) / (1._rk8 + oxinhib*conc_o2(c,j))
          else
            ch4_prod_depth(c,j) = 0._rk8 ! [mol/m3 total/s]
  ...
  !
  ! Oxidation is based on double Michaelis-Mentin kinetics, and is
  ! adjusted for low soil moisture.
    ...
    do j = 1 , nlevsoi
      do fc = 1 , num_methc
        ...
        if (sat == 1 .or. j > jwt(c)) then
          ! Literature (e.g. Bender & Conrad, 1992) suggests lower
          ! k_m and vmax for high-CH4-affinity methanotrophs in
        ...
        porevol = max(watsat(c,j) - h2osoi_vol(c,j), 0._rk8)
        h2osoi_vol_min = min(watsat(c,j), h2osoi_vol(c,j))
        if (j <= jwt(c) .and. smp_l(c,j) < 0._rk8) then
        ...
        if (j <= jwt(c)) then ! Above the water table
          k_h_inv = exp(-c_h_inv(1) * (1._rk8 / t_soisno(c,j) - &
                  1._rk8 / kh_tbase) + log (kh_theta(1)))
          ! (4.21) Wania [(mol/m3w) / (mol/m3g)]
          k_h_cc = t_soisno(c,j) / k_h_inv * rgasLatm
          conc_ch4_rel = conc_ch4(c,j) / (h2osoi_vol_min + porevol/k_h_cc)
          k_h_inv = exp(-c_h_inv(2) * (1._rk8 / t_soisno(c,j) - &
                  1._rk8 / kh_tbase) + log (kh_theta(2)))
          ! (4.21) Wania [(mol/m3w) / (mol/m3g)]
          k_h_cc = t_soisno(c,j) / k_h_inv * rgasLatm
        ...
        oxid_a = vmax_eff * h2osoi_vol_min* conc_ch4_rel / &
                ...
                ((t_soisno(c,j) - t0) / 10._rk8) * smp_fact
        ...
        if (t_soisno(c,j) <= tfrz) oxid_a = 0._rk8
        ch4_oxid_depth(c,j) = oxid_a
        o2_oxid_depth(c,j) = ch4_oxid_depth(c,j) * 2._rk8
      end do
    end do
  end subroutine ch4_oxid
  !
  ! Arctic c3 grass (which is often present in fens) and all vegetation
  ! in inundated areas is assumed to have some root porosity. Currently,
  ! root porosity is allowed to be different for grasses & non-grasses.
  ! CH4 diffuses out and O2 diffuses into the soil.  CH4 is also lossed
  ! via transpiration, which is both included in the "aere" variables
  ! and output separately.  In practice this value is small.
  ! By default upland veg. has small 5% porosity but this can be
  ! switched to be equal to inundated porosity.
  !
  subroutine ch4_aere (lbc,ubc,lbp,ubp,num_methc,filter_methc,num_methp, &
    ...
    do j = 1 , nlevsoi
      do fc = 1 , num_methc
        ...
        o2_aere_depth(c,j) = 0._rk8
      end do
    end do

    diffus_aere = d_con_g(1,1)*1.e-4_rk8  ! for CH4: m^2/s
    ...
    if (.not. lake) then
      do j = 1 , nlevsoi
        do fp = 1 , num_methp
           ...
           if (transpirationloss .and. ivt(p) /= noveg) then
             !allow tloss above WT ! .and. j > jwt(c)) then
             ! Calculate water concentration
             h2osoi_vol_min = min(watsat(c,j), h2osoi_vol(c,j))
             k_h_inv = exp(-c_h_inv(1) * &
                   (1._rk8 / t_soisno(c,j) - 1._rk8 / kh_tbase) + log (kh_theta(1)))
             k_h_cc = t_soisno(c,j) / k_h_inv * rgasLatm
             conc_ch4_wat = conc_ch4(c,j) / &
                     ( (watsat(c,j)-h2osoi_vol_min)/k_h_cc + h2osoi_vol_min)

             tranloss = conc_ch4_wat * &
                     rootr(p,j)*qflx_tran_veg(p) / dz(c,j) / 1000._rk8
           ...
           if (j > jwt(c) .and. &
               t_soisno(c,j) > tfrz .and. ivt(p) /= noveg) then
             ! Attn EK: This calculation of aerenchyma properties is very
             ! uncertain. Let's check in once all the new components are in;
             ! if there is any tuning to be done to get a realistic global
             ! flux, this would probably be the place.  We will have to
             ! document clearly in the Tech Note any major changes from
             ! the Riley et al. 2011 version.
             ! (There are a few other minor ones.)
             anpp = annsum_npp(p) ! g C / m^2/yr
             ! NPP can be negative b/c of consumption of storage pools
             anpp = max(anpp, 0._rk8)

             if (annavg_agnpp(p) /= spval .and. annavg_bgnpp(p) /= spval .and. &
                 annavg_agnpp(p) > 0._rk8 .and. annavg_bgnpp(p) > 0._rk8) then
             ...
             ! m_tiller = 600._rk8 * 0.5_rk8 * 2._rk8  ! used to be 300
             ...
             n_tiller = m_tiller / 0.22_rk8
             if (ivt(p) == nc3_arctic_grass .or. crop(ivt(p)) == 1 .or. &
                 ivt(p) == nc3_nonarctic_grass .or. ivt(p) == nc4_grass) then
               poros_tiller = 0.3_rk8  ! Colmer 2003
             else
               poros_tiller = 0.3_rk8 * nongrassporosratio
             end if
             if (sat == 0) then
             ...
             area_tiller = scale_factor_aere * n_tiller * &
                     poros_tiller * rpi * 2.9e-3_rk8**2._rk8 ! (m2/m2)
             ...
             aerecond = 1._rk8 / (1._rk8/(aerecond+smallnumber) + &
                     1._rk8/(grnd_ch4_cond(p)+smallnumber))
             aere = aerecond * (conc_ch4(c,j)/watsat(c,j)/k_h_cc - &
                     c_atm(g,1)) / dz(c,j) ![mol/m3-total/s]
             !ZS: Added watsat & Henry's const.
             aere = max(aere, 0._rk8) ! prevent backwards diffusion
          ...
          ! By imposing the limitation here, don't allow aerenchyma access
          ! to methane from other PFTs.
          aeretran = min(aere+tranloss, conc_ch4(c,j)/dtsrf + &
                  ch4_prod_depth(c,j))
          !pft weight in col.
          ch4_aere_depth (c, j) = ch4_aere_depth(c,j) + aeretran*wtcol(p)
          ch4_tran_depth (c, j) = ch4_tran_depth(c,j) + &
                  min(tranloss, aeretran)*wtcol(p)
  ...
  !
  ! Bubbling is based on temperature & pressure dependent solubility
  ! (k_h_cc), with assumed proportion of bubbles which are CH4, and
  ! assumed early nucleation at vgc_max sat (Wania).
  ...
  subroutine ch4_ebul(lbc,ubc,num_methc,filter_methc,jwt,sat,lake)
    ...
    bubble_f = 0.57_rk8 ! CH4 content in gas bubbles (Kellner et al. 2006)
    vgc_min = vgc_max
    ...
    do j = 1 , nlevsoi
      do fc = 1 , num_methc
        ...
        if (j > jwt(c) .and. t_soisno(c,j) > tfrz) then
          ...
          k_h_inv = exp(-c_h_inv(1) * (1._rk8 / &
                  t_soisno(c,j) - 1._rk8 / kh_tbase) + log (kh_theta(1)))
          k_h = 1._rk8 / k_h_inv ! (mol/L.atm)
          ! (4.21) Wania [(mol/m3w) / (mol/m3g)]
          ...
          if (.not. lake) then
            pressure = forc_pbot(g) + denh2o * grav * &
                    (z(c,j)-zi(c,jwt(c))) ! (Pa)
            if (sat == 1 .and. frac_h2osfc(c) > 0._rk8) then
              ! Add ponding pressure head
              pressure = pressure + denh2o * grav * &
                      h2osfc(c)/1000._rk8/frac_h2osfc(c)
              ! mm     / mm/m
            end if
          else
          ...
          vgc = conc_ch4(c,j) / watsat(c,j) / &
                  k_h_cc * rgasm * t_soisno(c,j) / pressure
          ! [mol/m3t] [m3w/m3t] [m3g/m3w] [Pa/(mol/m3g)] [Pa]

          if (vgc > vgc_max * bubble_f) then
            ! If greater than max value, remove amount down to vgc_min
            ! [mol/m3t/s] [mol/m3t]  [s]
            ch4_ebul_depth (c,j) = (vgc - vgc_min * bubble_f) * &
                    conc_ch4(c,j) / ebul_timescale
          else
            ch4_ebul_depth (c,j) = 0._rk8
          end if
        ...
        if (lake .and. lake_icefrac(c,1) > 0.1_rk8) ch4_ebul_depth(c,j) = 0._rk8
  ...
  !  Then concentrations are apportioned into gas & liquid fractions; only
  !  the gas fraction is considered for diffusion in unsat.
  !  Snow and lake water resistance to diffusion is added as a bulk term
  !  in the ground conductance (which is really a surface layer conductance),
  !  but concentrations are not tracked and oxidation is not allowed inside
  !  snow and lake water.
  !  Diffusivity is set based on soil texture and organic matter fraction.
  ...
  subroutine ch4_tran(lbc,ubc,num_methc,filter_methc,jwt,dtime_ch4,sat,lake)
    ...
    do j = 1 , nlevsoi
      do fc = 1 , num_methc
        ...
        if (o2demand > 0._rk8) then
          o2stress(c,j) = min((conc_o2(c,j) / dtsrf + &
                  o2_aere_depth(c,j)) / o2demand, 1._rk8)
        else
          o2stress(c,j) = 1._rk8
        end if
    ...
    ! Accumulate ebullition to place in first layer above water table,
    ! or directly to atmosphere
    do j = 1,nlevsoi
      do fc = 1, num_methc
        c = filter_methc (fc)
        if (j == 1) ch4_ebul_total(c) = 0._rk8
        ...
        do s = 1 , 2
          if (j == 0) then
            ...
            k_h_inv = exp(-c_h_inv(s) * (1._rk8 / t_grnd(c) - &
                    1._rk8 / kh_tbase) + log (kh_theta(s)))
            ! (4.21) Wania [(mol/m3w) / (mol/m3g)]
            k_h_cc(c,j,s) = t_grnd(c) / k_h_inv * rgasLatm
          else
            k_h_inv = exp(-c_h_inv(s) * (1._rk8 / t_soisno(c,j) - &
                    1._rk8 / kh_tbase) + log (kh_theta(s)))
            ! (4.12) Wania (L atm/mol)
            ! (4.21) Wania [(mol/m3w) / (mol/m3g)]
    ...
    ! Note that because of the semi-implicit diffusion and the 30 mini
    ! timestep combined with explicit sources, occasionally negative
    ! concentration will result. In this case it is brought to zero and the
    ! surface flux is adjusted to conserve. This results in some inaccuracy
    ! as compared to a shorter timestep or iterative solution.
    do j = 1,nlevsoi
      do fc = 1, num_methc
        c = filter_methc (fc)
        ...
        if (aereoxid >= 0._rk8) then
          ...
          ch4_oxid_depth(c,j) = ch4_oxid_depth(c,j) + &
                  aereoxid * ch4_aere_depth(c,j)
          ch4_aere_depth(c,j) = ch4_aere_depth(c,j) - &
                  aereoxid * ch4_aere_depth(c,j)
        ...
        if (source(c,j,1) + conc_ch4(c,j) / dtsrf < -1.e-12_rk8) then
        ...
        else if (ch4stress(c,j) < 1._rk8 .and. &
                source(c,j,1) + conc_ch4(c,j) / dtsrf > 1.e-12_rk8) then
          ...
          write(stderr,*)'Latdeg,Londeg=',clm3%g%latdeg(g),clm3%g%londeg(g)
          call fatal(__FILE__,__LINE__, &
                 trim(subname)//' ERROR: Methane limited, yet some left over.')
        end if

        source(c,j,2) = -o2_oxid_depth(c,j) - &
                  o2_decomp_depth(c,j) + o2_aere_depth(c,j) ! O2 [mol/m3/s]
        if (source(c,j,2) + conc_o2(c,j) / dtsrf < -1.e-12_rk8) then
        ...
        else if (o2stress(c,j) < 1._rk8 .and. &
                 source(c,j,2) + conc_o2(c,j) / dtsrf > 1.e-12_rk8) then
          ...
          write(stderr,*)'Latdeg,Londeg=',clm3%g%latdeg(g),clm3%g%londeg(g)
          call fatal(__FILE__,__LINE__, &
               trim(subname)//' ERROR: Oxygen limited, yet some left over.')
        end if
    ...
    do fc = 1 , num_methc
      ...
      if (jwt(c) /= 0) then
        source(c,jwt(c),1) = source(c,jwt(c),1) + &
                ch4_ebul_total(c)/dz(c,jwt(c))
      end if
    end do ! fc

    ! Calculate concentration relative to m^3 of air or water:
    ! needed for the diffusion
    do j = 0,nlevsoi
      do fc = 1, num_methc
        c = filter_methc (fc)
        g = cgridcell(c)
        ...
        if (j == 0) then
          ...
          if (ch4frzout) then
            liqfrac(c,j) = max(0.05_rk8, (h2osoi_liq(c,j)/denh2o+smallnumber)/ &
    ...
    do s = 1 , 2 ! 1=CH4; 2=O2; 3=CO2
      ...
      do j = -nlevsno+1 , 0
        do fc = 1 , num_methc
          ...
          if (j >= snl(c) + 1) then
            ...
            airfrac = max(1._rk8 - icefrac - waterfrac, 0._rk8)
            ! Calculate snow diffusivity
            if (airfrac > 0.05_rk8) then
              ...
              ! (bsw) not available
              snowdiff = (d_con_g(s,1) + d_con_g(s,2)*t_soisno_c) * 1.e-4_rk8 * &
                        f_a**(10._rk8/3._rk8) / (airfrac+waterfrac)**2 &
              ...
              snowdiff = eps**satpow * (d_con_w(s,1) + &
                      ...
                      d_con_w(s,3)*t_soisno_c**2) * 1.e-9_rk8 &
          ...
          if (j == 0) then ! final loop
            ...
            if (.not. lake .and. snl(c) == 0 .and. &
                h2osoi_vol(c,1) > watsat(c,1)) then
            ...
            spec_grnd_cond(c,s) = 1._rk8/(1._rk8/grnd_ch4_cond(c) + &
                    snowres(c) + pondres)
          end if
        end do ! fc
      end do ! j
      ...
      do j = 1 , nlevsoi
        do fc = 1 , num_methc
          ...
          if (j <= jwt(c)) then  ! Above the WT
            f_a = 1._rk8 - h2osoi_vol_min(c,j) / watsat(c,j)
            ...
            diffus (c,j) = (d_con_g(s,1) + d_con_g(s,2)*t_soisno_c) * 1.e-4_rk8 * &
                  (om_frac * f_a**(10._rk8/3._rk8) / watsat(c,j)**2._rk8 + &
                  (1._rk8-om_frac) * eps**2._rk8 * f_a**(3._rk8 / bsw(c,j)) ) &
                  * scale_factor_gasdiff
          else
            ! Below the WT use saturated diffusivity and only water
            ! in epsilon_t
            ! Note the following is not currently corrected for the effect
            ! on diffusivity of excess ice in soil under
            ...
            diffus (c,j) = eps**satpow * (d_con_w(s,1) + &
                    d_con_w(s,2)*t_soisno_c + &
                    d_con_w(s,3)*t_soisno_c**2) * 1.e-9_rk8 * scale_factor_liqdiff
      ...
      do j = 1 , nlevsoi
        do fc = 1 , num_methc
          ...
          if (j == 1 .and. j /= jwt(c) .and. j /= jwt(c)+1) then
            ...
            dm1_zm1(c,j) = 2._rk8/(dz(c,j)/diffus(c,j) + &
      ...
      do j = 0 , nlevsoi
        do fc = 1 , num_methc
          ...
          if (j == 0) then
            ...
            at(c,j) = -0.5_rk8 / dzj * dm1_zm1(c,j)
            bt(c,j) = epsilon_t(c,j,s) / dtime_ch4 + &
                    0.5_rk8 / dzj * (dp1_zp1(c,j)*k_h_cc(c,j,s) + dm1_zm1(c,j))
            ct(c,j) = -0.5_rk8 / dzj * dp1_zp1(c,j)
          else if (j < nlevsoi .and. j == jwt(c)+1) then
            ! concentration above needs to be mult. by k_h_cc for dm1_zm1 term
            at(c,j) = -0.5_rk8 / dzj * dm1_zm1(c,j) * k_h_cc(c,j-1,s)
            bt(c,j) = epsilon_t(c,j,s) / dtime_ch4 + &
                    0.5_rk8 / dzj * (dp1_zp1(c,j) + dm1_zm1(c,j))
            ct(c,j) = -0.5_rk8 / dzj * dp1_zp1(c,j)
            ...
            ! concentration above needs to be mult. by k_h_cc for dm1_zm1 term
            at(c,j) = -0.5_rk8 / dzj * dm1_zm1(c,j) * k_h_cc(c,j-1,s)
            bt(c,j) = epsilon_t(c,j,s) / dtime_ch4 + 0.5_rk8 / dzj * dm1_zm1(c,j)
            ct(c,j) = 0._rk8
      ...
      if ( s == 1 ) then  ! CH4
        ...
        do j = 1,nlevsoi
          do fc = 1, num_methc
            ...
            if (j < nlevsoi .and. j == jwt(c)) then
              ...
              ! by k_h_cc for dm1_zm1 term
              rt(c,j) = epsilon_t_old(c,j,s)/dtime_ch4 * conc_ch4_rel(c,j) + &
                         0.5_rk8/dzj * ( - dm1_zm1(c,j) * (conc_ch4_rel(c,j) - &
                         conc_ch4_rel(c,j-1)*k_h_cc(c,j-1,s))) + &
                         0.5_rk8 * (source(c,j,s) + source_old(c,j,s))
            else  !j==nlevsoi
              rt(c,j) = epsilon_t_old(c,j,s)/dtime_ch4 * conc_ch4_rel(c,j) + &
        ...
        do fc = 1 , num_methc
          ...
          if ( jwt(c) /= 0 ) then
            ...
            ch4_surf_diff(c) = dm1_zm1(c,1) * &
                    ( (conc_ch4_rel(c,1)+conc_ch4_rel_old(c,1))/2._rk8 &
                    - c_atm(g,s)) ! [mol/m2/s]
            ch4_surf_ebul(c) = 0._rk8
        ...
        do j = 1,nlevsoi
          do fc = 1 , num_methc
            ...
            if (conc_ch4_rel(c,j) < 0._rk8) then
              ...
              if (deficit > 1.e-3_rk8 * scale_factor_gasdiff) then
                if (deficit > 1.e-2_rk8) then
          ...
          do fc = 1 , num_methc
            ...
            if (j < nlevsoi .and. j == jwt(c)) then
              ...
              rt(c,j) = epsilon_t_old(c,j,s) / dtime_ch4 * conc_o2_rel(c,j) + &
                         0.5_rk8 / dzj * (dp1_zp1(c,j) * (conc_o2_rel(c,j+1) - &
              ...
              ! concentration above needs to be mult. by k_h_cc for dm1_zm1 term
              rt(c,j) = epsilon_t_old(c,j,s) / dtime_ch4 * conc_o2_rel(c,j) + &
                         0.5_rk8 / dzj * ( - dm1_zm1(c,j) * (conc_o2_rel(c,j) - &
                         conc_o2_rel(c,j-1)*k_h_cc(c,j-1,s))) + &
                         0.5_rk8 * (source(c,j,s) + source_old(c,j,s))
            else  !j==nlevsoi
              rt(c,j) = epsilon_t_old(c,j,s) / dtime_ch4 * conc_o2_rel(c,j) + &
        ...
        do j = 1 , nlevsoi
          do fc = 1 , num_methc
            ...
            ! NPP or LAI. Also, oxygen above ambient will probably bubble.
            conc_o2_rel(c,j) = min(conc_o2_rel(c,j), &
                                   c_atm(g,2)/epsilon_t(c,j,2))
          end do
        end do
      end if  ! species
    end do  ! species
  ...
  !
  ! Finds the first unsaturated layer going up. Also allows a perched
  ...
  subroutine get_jwt (lbc, ubc, num_methc, filter_methc, jwt)
    ...
    ! The layer index of the first unsaturated layer, i.e.,
    ! the layer right above the water table.
    ! ZS: Loop is currently not vectorized.
    do fc = 1, num_methc
      c = filter_methc(fc)
      ! Check to see if any soil layers are frozen and saturated.
      ! If so, start looking at the first layer above the top
      ! such layer.  This is potentially important for perched
      ! water tables in the Tundra.
      perch = nlevsoi
      do j = nlevsoi, 1, -1
        if ( t_soisno(c,j) < tfrz .and. &
             h2osoi_vol(c,j) > f_sat * watsat(c,j) ) then
      ...
      if ( jwt(c) == perch .and. h2osoi_vol(c,1) > f_sat * watsat(c,1) ) then
        ! missed that the top layer is saturated
        jwt(c) = 0
      end if
    end do
  ...
  subroutine ch4annualupdate(lbc,ubc,lbp,ubp,num_methc,filter_methc, &
    ...
    do fc = 1 , num_methc
      ...
      if ( annsum_counter(c) == spval ) then
        ! These variables are now in restart files for completeness,
        ! but might not be in inicFile and are not.
        ! set for arbinit.
        newrun = .true.
        annsum_counter(c)    = 0._rk8
        tempavg_somhr(c)     = 0._rk8
        tempavg_finrw(c)     = 0._rk8
      end if
