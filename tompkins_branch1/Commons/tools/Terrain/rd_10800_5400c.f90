 ...
 !     netCDF file 'GTOPO30_2MIN.CDF' into memory.
 !  
 !  History: 
 !  Date       Name          Action
 !-------------------------------------------------------
 ...
 !     Below   5 variables is the data in netCDF file
      ...
      real*4         :: lat(          5400 )
      real*8         :: time(nrec)
      integer*2      ::  HT( 10800, 5400,nrec)
      integer*2      ::  HTSD( 10800, 5400,nrec)
      character*2    ::  a(10800,5400),char_2
      integer*1      :: I1
 ...
 ! Open netCDF file.
       status=nf_open('GTOPO30_2MIN.CDF',nf_nowrite,ncid)
 ...
 !     Here write your own code please!
       open(20,file='HT_pal.dat',form='unformatted',recl=10800*5400*2,access='direct')
       do j=1,5400
       do i=1,10800
          if(HT(i,j,1).lt.-1000) HT(i,j,1)=-1000
          I1 = (HT(i,j,1)+1000)/256
          char_2(1:1) = CHAR(I1)
          I1 = MOD(HT(i,j,1)+1000,256)
          char_2(2:2) = CHAR(I1)
          a(i,j) = char_2
       enddo
       enddo
