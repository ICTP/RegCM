...
!-----------------------------------------------------------------------
   ...
   if ( nfix_timeconst .gt. 0.D0 .and. nfix_timeconst .lt. 500.D0 ) then
      ...
      do fc = 1,num_soilc
         ...
         !t = cannsum_npp(c) * 0.001666D0 / (secspday * dayspyr)
         t = (1.8D0 * (1.D0 - exp(-0.003D0 * cannsum_npp(c))))/(secspday * dayspyr)
         nfix_to_sminn(c) = max(0.D0,t)
...
!
   ...
   !!! awaiting_new_frozen_hydrolgy real(rk8), pointer :: qflx_drain_perched(:)  ! sub-surface runoff from perched wt (mm H2O /s)
...
!
#ifndef NITRIF_DENITRIF
   real(rk8), pointer :: sminn_leached_vr(:,:) ! rate of mineral N leaching (gN/m3/s)
#else
   real(rk8), pointer :: smin_no3_leached_vr(:,:) ! rate of mineral NO3 leaching (gN/m3/s)
   real(rk8), pointer :: smin_no3_runoff_vr(:,:)  ! rate of mineral NO3 loss with runoff (gN/m3/s)
