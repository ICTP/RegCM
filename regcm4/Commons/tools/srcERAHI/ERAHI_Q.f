...
C A1
      ...
      PARAMETER (NLEVS=60,NLATS=160,NLONS=320)
C
...
C
      real*4  q
      common /var_q/ q(NLONS,NLATS,NLEVS)
      integer nrec
      common /ncount/ nrec
C
...
c
 ...
 10      format('../DATA/ERAHI/',I4,'/',A2,'/q.',I10,'.grib')
 ...
 20      format('EHI_',I10)
     ...
     &          ,access='direct')
         ...
         do k=1,NLEVS
            nrec=nrec+1
            write(39,rec=nrec) ((q(i,j,k),i=1,NLONS),j=1,NLATS)
         enddo
         close(39)
         write(*,*) 'Water vapor is added for DATE: ',IDATE
      ENDDO
      stop
      end
C
...
c     reduced N80 Gaussian grid:
     ...
     &               iarrayq,  words_read_into_n80r, rarrayn80r  )
      call check_error_status('intf', ierror)
      call get_field_data(words_per_mb,iarrayq,rarrayn80r)
      k=0
      do j=NLATS,1,-1
      do i=1,NLONS
         k=k+1
         q(i,j,lev)=rarrayn80r(k)
      enddo
      enddo
