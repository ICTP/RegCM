...
!
   ...
   real(rk8), pointer :: grainc_storage_to_xfer(:)    ! grain C shift storage to transfer (gC/m2/s)
...
! local pointers to implicit in/out arrays
   ...
   real(rk8), pointer :: decomp_cpools_sourcesink(:,:,:)    ! (gC/m3/timestep)  change in decomposing c pools.  Used to update concentrations concurrently with vertical transport equation
...
! local pointers for dynamic landcover fluxes and states
   ...
   real(rk8), pointer :: dwt_seedc_to_deadstem(:)
   real(rk8), pointer :: dwt_frootc_to_litr_met_c(:,:)
   real(rk8), pointer :: dwt_frootc_to_litr_cel_c(:,:)
   real(rk8), pointer :: dwt_frootc_to_litr_lig_c(:,:)
   real(rk8), pointer :: dwt_livecrootc_to_cwdc(:,:) ! (gC/m2/s) live coarse root to CWD due to landcover change
   real(rk8), pointer :: dwt_deadcrootc_to_cwdc(:,:) ! (gC/m2/s) dead coarse root to CWD due to landcover change
