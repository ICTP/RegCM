...
! !USES:
  ...
  use mod_mpmessage
  use mod_clm_type
  use mod_clm_varcon, only : spval, isturb,  icol_roof, icol_sunwall, &
...
! !ARGUMENTS:
    ...
    real(rk8), intent(in)  :: parr(lbp:ubp)         ! pft array
    real(rk8), intent(out) :: carr(lbc:ubc)         ! column array
...
!EOP
    ...
    real(rk8) :: scale_p2c(lbp:ubp)     ! scale factor for column->landunit mapping
    logical  :: found                  ! temporary for error check
    real(rk8) :: sumwt(lbc:ubc)         ! sum of weights
    logical , pointer :: pactive(:)    ! true=>do computations on this pft (see reweightMod for details)
    real(rk8), pointer :: wtcol(:)      ! weight of pft relative to column
...
!------------------------------------------------------------------------
    ...
    if (found) then
       write(stderr,*)'p2c error: sumwt is greater than 1.0 at c= ',index
       call fatal(__FILE__,__LINE__,'clm is now stopping')
    end if
...
! !ARGUMENTS:
    ...
    real(rk8), intent(in)  :: parr(lbp:ubp,num2d)   ! pft array
    real(rk8), intent(out) :: carr(lbc:ubc,num2d)   ! column array
    character(len=*), intent(in) :: p2c_scale_type ! scale type
!
! !REVISION HISTORY:
! Created by Mariana Vertenstein 12/03
...
!------------------------------------------------------------------------
    ...
    do j = 1,num2d
       ...
       if (found) then
          write(stderr,*)'p2c_2d error: sumwt is greater than 1.0 at c= ',index,' lev= ',j
          call fatal(__FILE__,__LINE__,'clm is now stopping')
       end if
    end do 
...
! !ARGUMENTS:
    ...
    real(rk8), pointer     :: pftarr(:)
    real(rk8), pointer     :: colarr(:)
!
! !REVISION HISTORY:
! Created by Mariana Vertenstein 12/03
!
...
! !ARGUMENTS:
    ...
    real(rk8), intent(in)  :: parr(lbp:ubp)         ! input column array
    real(rk8), intent(out) :: larr(lbl:ubl)         ! output landunit array
...
!EOP
    ...
    real(rk8) :: scale_p2c(lbc:ubc)     ! scale factor for pft->column mapping
    real(rk8) :: scale_c2l(lbc:ubc)     ! scale factor for column->landunit mapping
    logical , pointer :: pactive(:)    ! true=>do computations on this pft (see reweightMod for details)
...
! !ARGUMENTS:
    ...
    integer , intent(in)  :: lbl, ubl              ! beginning and ending landunit indices
    integer , intent(in)  :: num2d                 ! size of second dimension
    real(rk8), intent(in)  :: parr(lbp:ubp,num2d)   ! input pft array
    real(rk8), intent(out) :: larr(lbl:ubl,num2d)   ! output gridcell array
    character(len=*), intent(in) :: p2c_scale_type ! scale factor type for averaging
    character(len=*), intent(in) :: c2l_scale_type ! scale factor type for averaging
!
! !REVISION HISTORY:
...
!------------------------------------------------------------------------
    ...
    do j = 1,num2d
       ...
       if (found) then
          write(stderr,*)'p2l_2d error: sumwt is greater than 1.0 at l= ',index,' j= ',j
          call fatal(__FILE__,__LINE__,'clm is now stopping')
       end if
    end do
...
! !ARGUMENTS:
    ...
    real(rk8), intent(in)  :: parr(lbp:ubp)       ! input pft array
    real(rk8), intent(out) :: garr(lbg:ubg)       ! output gridcell array
    character(len=*), intent(in) :: p2c_scale_type ! scale factor type for averaging
    character(len=*), intent(in) :: c2l_scale_type ! scale factor type for averaging
...
!EOP
    ...
    logical  :: found                  ! temporary for error check
    real(rk8) :: scale_p2c(lbp:ubp)     ! scale factor
    real(rk8) :: scale_c2l(lbc:ubc)     ! scale factor
    real(rk8) :: scale_l2g(lbl:ubl)     ! scale factor
    real(rk8) :: sumwt(lbg:ubg)         ! sum of weights
    logical , pointer :: pactive(:)    ! true=>do computations on this pft (see reweightMod for details)
    real(rk8), pointer :: wtgcell(:)    ! weight of pfts relative to gridcells
    integer , pointer :: pcolumn(:)    ! column of corresponding pft
    integer , pointer :: plandunit(:)  ! landunit of corresponding pft
...
!------------------------------------------------------------------------
    ...
    if (found) then
       write(stderr,*)'p2g_1d error: sumwt is greater than 1.0 at g= ',index
       call fatal(__FILE__,__LINE__,'clm is now stopping')
    end if
...
! !ARGUMENTS:
    ...
    integer , intent(in)  :: num2d                 ! size of second dimension
    real(rk8), intent(in)  :: parr(lbp:ubp,num2d)   ! input pft array
    real(rk8), intent(out) :: garr(lbg:ubg,num2d)   ! output gridcell array
    character(len=*), intent(in) :: p2c_scale_type ! scale factor type for averaging
    character(len=*), intent(in) :: c2l_scale_type ! scale factor type for averaging
    character(len=*), intent(in) :: l2g_scale_type ! scale factor type for averaging
!
...
!------------------------------------------------------------------------
    ...
    do j = 1,num2d
       ...
       if (found) then
          write(stderr,*)'p2g_2d error: sumwt gt 1.0 at g/sumwt = ',index,sumwt(index)
          call fatal(__FILE__,__LINE__,'clm is now stopping')
       end if
    end do
...
! !ARGUMENTS:
    ...
    integer , intent(in)  :: lbc, ubc      ! beginning and ending column indices
    integer , intent(in)  :: lbl, ubl      ! beginning and ending landunit indices
    real(rk8), intent(in)  :: carr(lbc:ubc) ! input column array
    real(rk8), intent(out) :: larr(lbl:ubl) ! output landunit array
    character(len=*), intent(in) :: c2l_scale_type ! scale factor type for averaging
!
! !REVISION HISTORY:
...
!EOP
    ...
    real(rk8), pointer :: wtlunit(:)    ! weight of landunits relative to gridcells
...
!------------------------------------------------------------------------
    ...
    if (found) then
       write(stderr,*)'c2l_1d error: sumwt is greater than 1.0 at l= ',index
       call fatal(__FILE__,__LINE__,'clm is now stopping')
    end if
...
! !ARGUMENTS:
    ...
    integer , intent(in)  :: lbl, ubl            ! beginning and ending landunit indices
    integer , intent(in)  :: num2d               ! size of second dimension
    real(rk8), intent(in)  :: carr(lbc:ubc,num2d) ! input column array
    real(rk8), intent(out) :: larr(lbl:ubl,num2d) ! output landunit array
    character(len=*), intent(in) :: c2l_scale_type ! scale factor type for averaging
!
! !REVISION HISTORY:
! Created by Mariana Vertenstein 12/03
...
!EOP
    ...
    real(rk8) :: sumwt(lbl:ubl)         ! sum of weights
    logical , pointer :: cactive(:)    ! true=>do computations on this column (see reweightMod for details)
    real(rk8), pointer :: wtlunit(:)    ! weight of column relative to landunit
    integer , pointer :: clandunit(:)  ! landunit of corresponding column
    integer , pointer :: ncolumns(:)   ! number of columns in landunit
    integer , pointer :: coli(:)       ! initial column index in landunit
    integer , pointer :: ctype(:)      ! column type
...
!------------------------------------------------------------------------
    ...
    do j = 1,num2d
       ...
       if (found) then
          write(stderr,*)'c2l_2d error: sumwt is greater than 1.0 at l= ',index,' lev= ',j
          call fatal(__FILE__,__LINE__,'clm is now stopping')
       end if
    end do
...
!EOP
    ...
    real(rk8) :: sumwt(lbg:ubg)         ! sum of weights
    logical , pointer :: cactive(:)    ! true=>do computations on this column (see reweightMod for details)
    real(rk8), pointer :: wtgcell(:)    ! weight of columns relative to gridcells
    integer , pointer :: clandunit(:)  ! landunit of corresponding column
...
!------------------------------------------------------------------------
    ...
    if (found) then
       write(stderr,*)'c2g_1d error: sumwt is greater than 1.0 at g= ',index
       call fatal(__FILE__,__LINE__,'clm is now stopping')
    end if
...
!------------------------------------------------------------------------
    ...
    do j = 1,num2d
       ...
       if (found) then
          write(stderr,*)'c2g_2d error: sumwt is greater than 1.0 at g= ',index
          call fatal(__FILE__,__LINE__,'clm is now stopping')
       end if
    end do
...
! !ARGUMENTS:
    ...
    integer , intent(in)  :: lbg, ubg       ! beginning and ending gridcell indices
    real(rk8), intent(in)  :: larr(lbl:ubl)  ! input landunit array
    real(rk8), intent(out) :: garr(lbg:ubg)  ! output gridcell array
    character(len=*), intent(in) :: l2g_scale_type ! scale factor type for averaging
!
! !REVISION HISTORY:
...
!EOP
    ...
    real(rk8), pointer :: wtgcell(:)    ! weight of landunits relative to gridcells
...
!------------------------------------------------------------------------
    ...
    if (found) then
       write(stderr,*)'l2g_1d error: sumwt is greater than 1.0 at g= ',index
       call fatal(__FILE__,__LINE__,'clm is now stopping')
    end if
...
! !ARGUMENTS:
    ...
    integer , intent(in)  :: lbg, ubg             ! beginning and ending gridcell indices
    integer , intent(in)  :: num2d                ! size of second dimension
    real(rk8), intent(in)  :: larr(lbl:ubl,num2d)  ! input landunit array
    real(rk8), intent(out) :: garr(lbg:ubg,num2d)  ! output gridcell array
    character(len=*), intent(in) :: l2g_scale_type ! scale factor type for averaging
!
! !REVISION HISTORY:
...
!------------------------------------------------------------------------
    ...
    do j = 1,num2d
       ...
       if (found) then
          write(stderr,*)'l2g_2d error: sumwt is greater than 1.0 at g= ',index,' lev= ',j
          call fatal(__FILE__,__LINE__,'clm is now stopping')
       end if
    end do
...
! !ARGUMENTS:
     ...
     character(len=*), intent(in)  :: l2g_scale_type     ! scale factor type for averaging
     integer         , intent(in)  :: lbl, ubl           ! beginning and ending column indices
     real(rk8)        , intent(out) :: scale_l2g(lbl:ubl) ! scale factor 
!
! !REVISION HISTORY:
...
! !LOCAL VARIABLES:
!EOP
     real(rk8) :: scale_lookup(max_lunit) ! scale factor for each landunit type
     integer  :: l                       ! index
     integer , pointer :: ltype(:)       ! landunit type
!-----------------------------------------------------------------------
...
! !ARGUMENTS:
     implicit none
     character(len=*), intent(in)  :: l2g_scale_type           ! scale factor type for averaging
     real(rk8)        , intent(out) :: scale_lookup(max_lunit)  ! scale factor for each landunit type
!
! !REVISION HISTORY:
! Created by Bill Sacks 10/11
!
...
!-----------------------------------------------------------------------
     ...
     if (l2g_scale_type == 'unity') then
        ...
        write(stderr,*)'scale_l2g_lookup_array error: scale type ',l2g_scale_type,' not supported'
        call fatal(__FILE__,__LINE__,'clm is now stopping')
     end if
