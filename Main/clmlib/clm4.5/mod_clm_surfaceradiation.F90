...
!
     ...
     real(rk8), pointer :: parsun_z(:,:)    ! absorbed PAR for sunlit leaves in canopy layer
     real(rk8), pointer :: parsha_z(:,:)    ! absorbed PAR for shaded leaves in canopy layer
     ...
     real(rk8), pointer :: albgrd_bc(:,:)     ! ground albedo without BC (direct) (col,bnd)
     real(rk8), pointer :: albgri_bc(:,:)     ! ground albedo without BC (diffuse) (col,bnd)
     real(rk8), pointer :: albgrd_oc(:,:)     ! ground albedo without OC (direct) (col,bnd)
     real(rk8), pointer :: albgri_oc(:,:)     ! ground albedo without OC (diffuse) (col,bnd)
     real(rk8), pointer :: albgrd_dst(:,:)    ! ground albedo without dust (direct) (col,bnd)
     real(rk8), pointer :: albgri_dst(:,:)    ! ground albedo without dust (diffuse) (col,bnd)
     real(rk8), pointer :: albsnd_hst(:,:)    ! snow albedo, direct, for history files (col,bnd) [frc]
     real(rk8), pointer :: albsni_hst(:,:)    ! snow ground albedo, diffuse, for history files (col,bnd
     real(rk8), pointer :: sabg_lyr(:,:)      ! absorbed radiative flux (pft,lyr) [W/m2]
     ...
     real(rk8), pointer :: fsr_sno_vd(:)    ! reflected visible, direct radiation from snow (for history files) (pft) [W/m2]
     real(rk8), pointer :: fsr_sno_nd(:)    ! reflected near-IR, direct radiation from snow (for history files) (pft) [W/m2]
     real(rk8), pointer :: fsr_sno_vi(:)    ! reflected visible, diffuse radiation from snow (for history files) (pft) [W/m2]
     real(rk8), pointer :: fsr_sno_ni(:)    ! reflected near-IR, diffuse radiation from snow (for history files) (pft) [W/m2]
     real(rk8), pointer :: fsds_sno_vd(:)   ! incident visible, direct radiation on snow (for history files) (pft) [W/m2]
     real(rk8), pointer :: fsds_sno_nd(:)   ! incident near-IR, direct radiation on snow (for history files) (pft) [W/m2]
     real(rk8), pointer :: fsds_sno_vi(:)   ! incident visible, diffuse radiation on snow (for history files) (pft) [W/m2]
...
!
     ...
     real(rk8) :: trd(lbp:ubp,numrad) ! transmitted solar radiation: direct (W/m**2)
     real(rk8) :: tri(lbp:ubp,numrad) ! transmitted solar radiation: diffuse (W/m**2)
     real(rk8) :: cad(lbp:ubp,numrad) ! direct beam absorbed by canopy (W/m**2)
     real(rk8) :: cai(lbp:ubp,numrad) ! diffuse radiation absorbed by canopy (W/m**2)
     ...
     real(rk8) :: sabg_snl_sum        ! temporary, absorbed energy in all active snow layers [W/m2]
     real(rk8) :: absrad_pur          ! temp: absorbed solar radiation by pure snow [W/m2]
     real(rk8) :: absrad_bc           ! temp: absorbed solar radiation without BC [W/m2]
     real(rk8) :: absrad_oc           ! temp: absorbed solar radiation without OC [W/m2]
     real(rk8) :: absrad_dst          ! temp: absorbed solar radiation without dust [W/m2]
     real(rk8) :: sabg_pur(lbp:ubp)   ! solar radiation absorbed by ground with pure snow [W/m2]
     real(rk8) :: sabg_bc(lbp:ubp)    ! solar radiation absorbed by ground without BC [W/m2]
     real(rk8) :: sabg_oc(lbp:ubp)    ! solar radiation absorbed by ground without OC [W/m2]
     ...
     do ib = 1, nband
        do fp = 1,num_nourbanp
           ...
           if (pactive(p)) then
              ...
              absrad  = trd(p,ib)*(1.D0-albsnd_hst(c,ib)) + tri(p,ib)*(1.D0-albsni_hst(c,ib))
              sabg_snow(p) = sabg_snow(p) + absrad
              absrad  = trd(p,ib)*(1.D0-albgrd(c,ib)) + tri(p,ib)*(1.D0-albgri(c,ib))
              ...
              absrad_bc = trd(p,ib)*(1.D0-albgrd_bc(c,ib)) + tri(p,ib)*(1.D0-albgri_bc(c,ib))
              sabg_bc(p) = sabg_bc(p) + absrad_bc

              ! Solar radiation absorbed by ground surface without OC
              absrad_oc = trd(p,ib)*(1.D0-albgrd_oc(c,ib)) + tri(p,ib)*(1.D0-albgri_oc(c,ib))
              sabg_oc(p) = sabg_oc(p) + absrad_oc

              ! Solar radiation absorbed by ground surface without dust
              absrad_dst = trd(p,ib)*(1.D0-albgrd_dst(c,ib)) + tri(p,ib)*(1.D0-albgri_dst(c,ib))
              sabg_dst(p) = sabg_dst(p) + absrad_dst

              ! Solar radiation absorbed by ground surface without any aerosols
              absrad_pur = trd(p,ib)*(1.D0-albgrd_pur(c,ib)) + tri(p,ib)*(1.D0-albgri_pur(c,ib))
     ...
     do fp = 1,num_nourbanp
        p = filter_nourbanp(fp)
           ...
           if (abs(sum(sabg_lyr(p,:))-sabg_snow(p)) > 0.00001D0) then
              ...
              write(stderr,*) "flx_absdv1= ", trd(p,1)*(1.-albgrd(c,1)), "flx_absdv2= ", sum(flx_absdv(c,:))*trd(p,1)
