...
! !PUBLIC DATA MEMBERS:
   ...
   real(rk8), public :: som_adv_flux =  0.D0
   real(rk8), public :: cryoturb_diffusion_k =  5D-4 / (secspday * 365.D0)  ! [m^2/sec] = 5 cm^2 / yr = 1m^2 / 200 yr
   real(rk8), public :: max_depth_cryoturb = 3.D0          ! (m) this is the maximum depth of cryoturbation
   real(rk8), public :: max_altdepth_cryoturbation = 2.D0  ! (m) maximum active layer thickness for cryoturbation to occur
! !REVISION HISTORY:
!
!
...
!EOP
   ...
   real(rk8) :: diffus (lbc:ubc,1:nlevdecomp+1)                ! diffusivity (m2/s)  (includes spinup correction, if any)
   real(rk8) :: adv_flux(lbc:ubc,1:nlevdecomp+1)               ! advective flux (m/s)  (includes spinup correction, if any)
   real(rk8) :: aaa                                            ! "A" function in Patankar
   real(rk8) :: pe                                             ! Pe for "A" function in Patankar
   real(rk8) :: w_m1, w_p1                                     ! Weights for calculating harmonic mean of diffusivity
   real(rk8) :: d_m1, d_p1                                     ! Harmonic mean of diffusivity
   real(rk8) :: a_tri (lbc:ubc,0:nlevdecomp+1)                 ! "a" vector for tridiagonal matrix
   ...
   real(rk8), pointer :: spinup_factor(:) ! spinup accelerated decomposition factor, used to accelerate transport as well
...
!-----------------------------------------------------------------------
   ...
   do fc = 1, num_soilc
      ...
      if  ( ( max(altmax(c), altmax_lastyear(c)) .le. max_altdepth_cryoturbation ) .and. &
        ( max(altmax(c), altmax_lastyear(c)) .gt. 0.D0) ) then
         ...
         do j = 1,nlevdecomp+1
            if ( zisoi(j) .lt. max(altmax(c), altmax_lastyear(c)) ) then
               ...
               som_diffus_coef(c,j) = max(cryoturb_diffusion_k * ( 1.D0 - ( zisoi(j) - max(altmax(c), altmax_lastyear(c)) ) / &
                    ( max_depth_cryoturb - max(altmax(c), altmax_lastyear(c)) ) ), 0.D0)  ! go linearly to zero between ALT and max_depth_cryoturb
               som_adv_coef(c,j) = 0.D0
            endif
         end do
               ...
               do fc = 1, num_soilc
                  ...
                  if (j == 1) then
                     ...
                     if ( diffus(c,j+1) .gt. 0.D0 .and. diffus(c,j) .gt. 0.D0) then
                        d_p1 = 1.D0 / ((1.D0 - w_p1) / diffus(c,j) + w_p1 / diffus(c,j+1)) ! Harmonic mean of diffus
                     else
                        d_p1 = 0.D0
                     endif
                  ...
                  elseif (j == nlevdecomp+1) then
                     ...
                     if ( diffus(c,j) .gt. 0.D0 .and. diffus(c,j-1) .gt. 0.D0) then
                        d_m1 = 1.D0 / ((1.D0 - w_m1) / diffus(c,j) + w_m1 / diffus(c,j-1)) ! Harmonic mean of diffus
                     else
                        d_m1 = 0.D0
                     endif
                     ...
                     if ( diffus(c,j+1) .gt. 0.D0 .and. diffus(c,j) .gt. 0.D0) then
                        ...
                        d_p1 = (1.D0 - w_m1) * diffus(c,j) + w_p1 * diffus(c,j+1) ! Arithmetic mean of diffus
                     endif
                     d_m1_zm1(c,j) = d_m1 / dz_node(j)
                     d_p1_zp1(c,j) = d_p1 / dz_node(j+1)
                     f_m1(c,j) = adv_flux(c,j)
