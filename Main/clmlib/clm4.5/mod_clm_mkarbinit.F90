...
module mod_clm_mkarbinit
  ...
  real(rk8), dimension(22), parameter :: slmo = &
    [ 0.50_rk8, 0.50_rk8, 0.50_rk8, 0.50_rk8, 0.50_rk8, &
      0.50_rk8, 0.50_rk8, 0.01_rk8, 0.50_rk8, 0.50_rk8, &
      0.10_rk8, 0.50_rk8, 0.90_rk8, 1.00_rk8, 1.00_rk8, &
      0.50_rk8, 0.30_rk8, 0.50_rk8, 0.50_rk8, 0.80_rk8, &
      0.10_rk8, 0.50_rk8 ]
  ...
  subroutine mkarbinit()
      ...
      if ( .not. lakpoi(l) ) then  !not lake
        if ( ltype(l) == isturb ) then
          if ( ctype(c) == icol_road_perv ) then
            ...
            zwt(c) = (25._rk8 + zi(c,nlevsoi)) - wa(c)/0.2_rk8 /1000._rk8
          else
            wa(c)  = spval
            zwt(c) = spval
          end if
        ...
        if ( ltype(l) == istsoil .or. ltype(l) == istcrop ) then
          ...
          do j = 1, nlevs
            if ( h2osoi_liq(c,j) > 0._rk8 ) then
              ...
              soilpsi(c,j) = max(psi, -15.0_rk8)
              soilpsi(c,j) = min(soilpsi(c,j),0.0_rk8)
            end if
          end do
        end if
        fsat(c)   = 0.0_rk8
    ...
    do j = -nlevsno+1, 0
      do c = begc, endc
        ...
        if ( .not. lakpoi(l) ) then  !not lake
          if ( j > snl(c) ) then
            h2osoi_ice(c,j) = dz(c,j)*250._rk8
            h2osoi_liq(c,j) = 0._rk8
          end if
        end if
      end do
    ...
    do c = begc ,endc
      ...
      else if (h2osno(c) > 0._rk8) then
        snw_rds(c,0)             = snw_rds_min
        snw_rds(c,-nlevsno+1:-1) = 0._rk8
        snw_rds_top(c)           = spval
        sno_liq_top(c)           = spval
      else
        snw_rds(c,:)   = 0._rk8
  ...
  subroutine perturbIC( landunit )
    ...
    if ( pertlim /= 0.0_rk8 )then
      ...
      do c = begc, endc
        ...
        call random_number (pertval)
        pertval   = 2._rk8*pertlim*(0.5_rk8 - pertval)
        t_grnd(c) = t_grnd(c)*(1._rk8 + pertval)
      end do
    end if
    ...
    do c = lbc, ubc
      ...
      if ( .not. lakpoi(l) ) then
        if (snow_depth(c) < 0.01_rk8) then
          ...
          if ( (snow_depth(c) >= 0.01_rk8) .and. &
               (snow_depth(c) <= 0.03_rk8) ) then
            snl(c) = -1
            dz(c,0)  = snow_depth(c)
          else if ( (snow_depth(c) > 0.03_rk8) .and. &
                    (snow_depth(c) <= 0.04_rk8) ) then
            snl(c) = -2
            dz(c,-1) = snow_depth(c)/2._rk8
            dz(c, 0) = dz(c,-1)
          else if ( (snow_depth(c) > 0.04_rk8) .and. &
                    (snow_depth(c) <= 0.07_rk8) ) then
            snl(c) = -2
            dz(c,-1) = 0.02_rk8
            dz(c, 0) = snow_depth(c) - dz(c,-1)
          else if ( (snow_depth(c) > 0.07_rk8) .and. &
                    (snow_depth(c) <= 0.12_rk8) ) then
            snl(c) = -3
            dz(c,-2) = 0.02_rk8
            dz(c,-1) = (snow_depth(c) - 0.02_rk8)/2._rk8
            dz(c, 0) = dz(c,-1)
          else if ( (snow_depth(c) > 0.12_rk8) .and. &
                    (snow_depth(c) <= 0.18_rk8) ) then
            snl(c) = -3
            dz(c,-2) = 0.02_rk8
            dz(c,-1) = 0.05_rk8
            dz(c, 0) = snow_depth(c) - dz(c,-2) - dz(c,-1)
          else if ( (snow_depth(c) > 0.18_rk8) .and. &
                    (snow_depth(c) <= 0.29_rk8) ) then
            snl(c) = -4
            dz(c,-3) = 0.02_rk8
            dz(c,-2) = 0.05_rk8
            dz(c,-1) = (snow_depth(c) - dz(c,-3) - dz(c,-2))/2._rk8
            dz(c, 0) = dz(c,-1)
          else if ( (snow_depth(c) > 0.29_rk8) .and. &
                    (snow_depth(c) <= 0.41_rk8) ) then
            snl(c) = -4
            dz(c,-3) = 0.02_rk8
            dz(c,-2) = 0.05_rk8
            dz(c,-1) = 0.11_rk8
            dz(c, 0) = snow_depth(c) - dz(c,-3) - dz(c,-2) - dz(c,-1)
          else if ( (snow_depth(c) > 0.41_rk8) .and. &
                    (snow_depth(c) <= 0.64_rk8) ) then
            snl(c) = -5
            dz(c,-4) = 0.02_rk8
            dz(c,-3) = 0.05_rk8
            dz(c,-2) = 0.11_rk8
            ...
            dz(c, 0) = dz(c,-1)
          else if ( snow_depth(c) > 0.64_rk8 ) then
            snl(c) = -5
            dz(c,-4) = 0.02_rk8
            dz(c,-3) = 0.05_rk8
            dz(c,-2) = 0.11_rk8
