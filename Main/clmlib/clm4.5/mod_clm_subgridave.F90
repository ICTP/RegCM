module  mod_clm_subgridave
    ...
    do p = lbp, ubp
      if ( pactive(p) .and. wtcol(p) /= 0._rk8 ) then
        if ( parr(p) /= spval ) then
          c = pcolumn(p)
          if (sumwt(c) == 0._rk8) carr(c) = 0._rk8
      ...
      do p = lbp, ubp
        if (pactive(p) .and. wtcol(p) /= 0._rk8) then
          if (parr(p,j) /= spval) then
            c = pcolumn(p)
            if (sumwt(c) == 0._rk8) carr(c,j) = 0._rk8
  ...
  subroutine p2l_1d(lbp,ubp,lbc,ubc,lbl,ubl,parr,larr, &
    ...
    if ( c2l_scale_type == 'unity' ) then
      ...
      do c = lbc, ubc
        ...
        if ( ltype(l) == isturb ) then
          if ( ctype(c) == icol_sunwall ) then
            scale_c2l(c) = 3.0_rk8 * canyon_hwr(l)
      ...
      do c = lbc, ubc
        ...
        if ( ltype(l) == isturb ) then
          if ( ctype(c) == icol_sunwall ) then
            scale_c2l(c) = (3.0_rk8 * canyon_hwr(l))/(2.0_rk8*canyon_hwr(l)+1.0_rk8)
          else if ( ctype(c) == icol_shadewall ) then
            scale_c2l(c) = (3.0_rk8 * canyon_hwr(l))/(2.0_rk8*canyon_hwr(l)+1.0_rk8)
          else if ( ctype(c) == icol_road_perv .or. &
                    ctype(c) == icol_road_imperv ) then
            scale_c2l(c) = 3.0_rk8 / (2.0_rk8*canyon_hwr(l) + 1.0_rk8)
          else if ( ctype(c) == icol_roof ) then
            scale_c2l(c) = 1.0_rk8
          end if
        else
    ...
    do p = lbp,ubp
      if ( pactive(p) .and. wtlunit(p) /= 0._rk8 ) then
        ...
        if ( parr(p) /= spval .and. scale_c2l(c) /= spval ) then
          l = plandunit(p)
          if (sumwt(l) == 0._rk8) larr(l) = 0._rk8
  ...
  subroutine p2l_2d(lbp,ubp,lbc,ubc,lbl,ubl,num2d,parr,larr, &
    ...
    if ( c2l_scale_type == 'unity' ) then
      ...
      do c = lbc, ubc
        ...
        if ( ltype(l) == isturb ) then
          if ( ctype(c) == icol_sunwall ) then
            scale_c2l(c) = (3.0_rk8 * canyon_hwr(l)) / (2._rk8*canyon_hwr(l) + 1._rk8)
          else if ( ctype(c) == icol_shadewall ) then
            scale_c2l(c) = (3.0_rk8 * canyon_hwr(l)) / (2._rk8*canyon_hwr(l) + 1._rk8)
          else if ( ctype(c) == icol_road_perv .or. &
                    ctype(c) == icol_road_imperv ) then
            scale_c2l(c) = 3.0_rk8 / (2._rk8*canyon_hwr(l) + 1._rk8)
          else if ( ctype(c) == icol_roof ) then
            scale_c2l(c) = 1.0_rk8
          end if
        else
      ...
      do p = lbp, ubp
        if ( pactive(p) .and. wtlunit(p) /= 0._rk8 ) then
          ...
          if ( parr(p,j) /= spval .and. scale_c2l(c) /= spval ) then
            l = plandunit(p)
            if ( sumwt(l) == 0._rk8 ) larr(l,j) = 0._rk8
            larr(l,j) = larr(l,j) + &
    ...
    do concurrent ( p = lbp:ubp )
      if ( pactive(p) .and. wtgcell(p) /= 0._rk8 ) then
        ...
        if ( parr(p) /= spval .and. &
          ...
          g = pgridcell(p)
          if ( sumwt(g) == 0._rk8 ) garr(g) = 0._rk8
        end if
      end if
    end do
  ...
  subroutine p2g_2d(lbp,ubp,lbc,ubc,lbl,ubl,lbg,ubg,num2d,parr,garr, &
    ...
    ! scale factor type for averaging
    character(len=*), intent(in) :: l2g_scale_type
    integer(ik4) :: j, p, c, l, g, iind(2)     ! indices
    logical :: found                  ! temporary for error check
    real(rk8), dimension(lbp:ubp) :: scale_p2c   ! scale factor
    real(rk8), dimension(lbc:ubc) :: scale_c2l   ! scale factor
    real(rk8), dimension(lbl:ubl) :: scale_l2g   ! scale factor
    real(rk8), dimension(lbg:ubg,1:num2d) :: sumwt       ! sum of weights
    ! true=>do computations on this pft (see reweightMod for details)
    logical, pointer, contiguous, dimension(:) :: pactive
    ! weight of pfts relative to gridcells
    real(rk8), pointer, contiguous, dimension(:) :: wtgcell
    ...
    do g = lbg, ubg
      if ( sumwt(g,j) > 1.0_rk8 + 1.e-3_rk8 ) then
        ...
        iind(1) = g
        iind(2) = j
      else if ( sumwt(g,j) /= 0._rk8 ) then
        garr(g,j) = garr(g,j)/sumwt(g,j)
      end if
    end do
    end do
    if ( found ) then
      ...
      write(stderr,*) &
           'p2g_2d error: sumwt gt 1.0 at g/sumwt = ',iind(1),sumwt(iind(1),iind(2))
      call fatal(__FILE__,__LINE__,'clm is now stopping')
    end if
  ...
  subroutine l2g_2d(lbl,ubl,lbg,ubg,num2d,larr,garr,l2g_scale_type)
    ...
    ! scale factor type for averaging
    character(len=*), intent(in) :: l2g_scale_type
    integer(ik4) :: j, g, l, iind(2) ! indices
    logical :: found ! temporary for error check
    real(rk8), dimension(lbl:ubl) :: scale_l2g     ! scale factor
    real(rk8), dimension(lbg:ubg,1:num2d) :: sumwt         ! sum of weights
    ! true=>do computations on this landunit (see reweightMod for details)
