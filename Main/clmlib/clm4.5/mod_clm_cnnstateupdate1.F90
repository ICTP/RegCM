module mod_clm_cnnstateupdate1
  ...
  subroutine NStateUpdate1(num_soilc, filter_soilc, num_soilp, filter_soilp)
    ...
    real(rkx), pointer :: grainn_storage_to_xfer(:)
    ! grain N to food (gN/m2/s)
    real(rkx), pointer :: grainn_to_food(:)
    ! grain N growth from storage (gN/m2/s)
    real(rkx), pointer :: grainn_xfer_to_grainn(:)
...
#ifdef NITRIF_DENITRIF
    real(rkx), pointer :: smin_no3_vr(:,:) ! (gN/m3) soil NO3
    real(rkx), pointer :: smin_nh4_vr(:,:) ! (gN/m3) soil NH4
    real(rkx), pointer :: f_nit_vr(:,:)    ! (gN/m3/s) soil nitrification flux
    real(rkx), pointer :: f_denit_vr(:,:)  ! (gN/m3/s) soil denitrification flux
    real(rkx), pointer :: actual_immob_no3_vr(:,:)  ! (gN/m3/s)
    real(rkx), pointer :: actual_immob_nh4_vr(:,:)  ! (gN/m3/s)
    real(rkx), pointer :: smin_no3_to_plant_vr(:,:) ! (gN/m3/s)
    real(rkx), pointer :: smin_nh4_to_plant_vr(:,:) ! (gN/m3/s)
    real(rkx), pointer :: gross_nmin_vr(:,:)        ! (gN/m3/s)
#endif
    ...
    real(rkx), pointer :: decomp_npools_sourcesink(:,:,:)
    ...
    real(rkx), pointer :: dwt_seedn_to_deadstem(:)
    real(rkx), pointer :: dwt_frootn_to_litr_met_n(:,:)
    real(rkx), pointer :: dwt_frootn_to_litr_cel_n(:,:)
    real(rkx), pointer :: dwt_frootn_to_litr_lig_n(:,:)
    real(rkx), pointer :: dwt_livecrootn_to_cwdn(:,:)
    real(rkx), pointer :: dwt_deadcrootn_to_cwdn(:,:)
