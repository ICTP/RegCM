...
! !PUBLIC TYPES:
  ...
  implicit none
  save
   real(rk8),public,parameter :: SHR_ORB_UNDEF_REAL = 1.D36 ! undefined real 
   integer(ik4),public,parameter :: SHR_ORB_UNDEF_INT  = 2000000000        ! undefined int
...
! This is the value used in CAM3 by Pritchard et al., GRL, 35, 2008.
  ...
  real(rk8), public  :: albice(numrad) = &       ! albedo land ice by waveband (1=vis, 2=nir)
                       (/ 0.80D0, 0.55D0 /)
!
...
! !ARGUMENTS:
    ...
    real(rk8), intent(in) :: nextsw_cday                   ! calendar day at Greenwich (1.00, ..., days/year)
    real(rk8), intent(in) :: declinp1                   ! declination angle (radians) for next time step
!
! !CALLED FROM:
...
!
    ...
    real(rk8), pointer :: lon(:)       ! gridcell longitude (radians)
    ...
    real(rk8), pointer :: rhol(:,:)    ! leaf reflectance: 1=vis, 2=nir
    real(rk8), pointer :: rhos(:,:)    ! stem reflectance: 1=vis, 2=nir
    real(rk8), pointer :: taul(:,:)    ! leaf transmittance: 1=vis, 2=nir
    real(rk8), pointer :: taus(:,:)    ! stem transmittance: 1=vis, 2=nir
...
!
    ...
    real(rk8), pointer :: fabd(:,:)          ! flux absorbed by canopy per unit direct flux
    real(rk8), pointer :: fabd_sun(:,:)      ! flux absorbed by sunlit canopy per unit direct flux
    real(rk8), pointer :: fabd_sha(:,:)      ! flux absorbed by shaded canopy per unit direct flux
    real(rk8), pointer :: fabi(:,:)          ! flux absorbed by canopy per unit diffuse flux
    real(rk8), pointer :: fabi_sun(:,:)      ! flux absorbed by sunlit canopy per unit diffuse flux
    real(rk8), pointer :: fabi_sha(:,:)      ! flux absorbed by shaded canopy per unit diffuse flux
    real(rk8), pointer :: ftdd(:,:)          ! down direct flux below canopy per unit direct flux
    real(rk8), pointer :: ftid(:,:)          ! down diffuse flux below canopy per unit direct flux
    real(rk8), pointer :: ftii(:,:)          ! down diffuse flux below canopy per unit diffuse flux
    ...
    real(rk8), pointer :: h2osoi_liq(:,:)    ! liquid water content (col,lyr) [kg/m2]
    real(rk8), pointer :: h2osoi_ice(:,:)    ! ice lens content (col,lyr) [kg/m2]
    real(rk8), pointer :: mss_cnc_bcphi(:,:) ! mass concentration of hydrophilic BC (col,lyr) [kg/kg]
    real(rk8), pointer :: mss_cnc_bcpho(:,:) ! mass concentration of hydrophobic BC (col,lyr) [kg/kg]
    real(rk8), pointer :: mss_cnc_ocphi(:,:) ! mass concentration of hydrophilic OC (col,lyr) [kg/kg]
    real(rk8), pointer :: mss_cnc_ocpho(:,:) ! mass concentration of hydrophobic OC (col,lyr) [kg/kg]
    real(rk8), pointer :: mss_cnc_dst1(:,:)  ! mass concentration of dust aerosol species 1 (col,lyr) [kg/kg]
    real(rk8), pointer :: mss_cnc_dst2(:,:)  ! mass concentration of dust aerosol species 2 (col,lyr) [kg/kg]
    real(rk8), pointer :: mss_cnc_dst3(:,:)  ! mass concentration of dust aerosol species 3 (col,lyr) [kg/kg]
    real(rk8), pointer :: mss_cnc_dst4(:,:)  ! mass concentration of dust aerosol species 4 (col,lyr) [kg/kg]
    real(rk8), pointer :: albsod(:,:)        ! direct-beam soil albedo (col,bnd) [frc]
    real(rk8), pointer :: albsoi(:,:)        ! diffuse soil albedo (col,bnd) [frc]
    real(rk8), pointer :: flx_absdv(:,:)     ! direct flux absorption factor (col,lyr): VIS [frc]
    real(rk8), pointer :: flx_absdn(:,:)     ! direct flux absorption factor (col,lyr): NIR [frc]
    real(rk8), pointer :: flx_absiv(:,:)     ! diffuse flux absorption factor (col,lyr): VIS [frc]
    real(rk8), pointer :: flx_absin(:,:)     ! diffuse flux absorption factor (col,lyr): NIR [frc]
    real(rk8), pointer :: snw_rds(:,:)       ! snow grain radius (col,lyr) [microns]
    real(rk8), pointer :: albgrd_pur(:,:)    ! pure snow ground albedo (direct)
    real(rk8), pointer :: albgri_pur(:,:)    ! pure snow ground albedo (diffuse)
    real(rk8), pointer :: albgrd_bc(:,:)     ! ground albedo without BC (direct)
    real(rk8), pointer :: albgri_bc(:,:)     ! ground albedo without BC (diffuse)
    real(rk8), pointer :: albgrd_oc(:,:)     ! ground albedo without OC (direct)
    real(rk8), pointer :: albgri_oc(:,:)     ! ground albedo without OC (diffuse)
    real(rk8), pointer :: albgrd_dst(:,:)    ! ground albedo without dust (direct)
    real(rk8), pointer :: albgri_dst(:,:)    ! ground albedo without dust (diffuse)
    real(rk8), pointer :: albsnd_hst(:,:)    ! snow albedo, direct, for history files (col,bnd) [frc]
    real(rk8), pointer :: albsni_hst(:,:)    ! snow ground albedo, diffuse, for history files (col,bnd) [frc]
!
!
...
!
    ...
    real(rk8) :: wl(lbp:ubp)                ! fraction of LAI+SAI that is LAI
    real(rk8) :: ws(lbp:ubp)                ! fraction of LAI+SAI that is SAI
    real(rk8) :: dinc                       ! lai+sai increment for canopy layer
    real(rk8) :: dincmax                    ! maximum lai+sai increment for canopy layer
    real(rk8) :: dincmax_sum                ! cumulative sum of maximum lai+sai increment for canopy layer
    real(rk8) :: laisum                     ! sum of canopy layer lai for error check
    real(rk8) :: saisum                     ! sum of canopy layer sai for error check
    real(rk8) :: blai(lbp:ubp)              ! lai buried by snow: tlai - elai
    real(rk8) :: bsai(lbp:ubp)              ! sai buried by snow: tsai - esai
    real(rk8) :: rho(lbp:ubp,numrad)        ! leaf/stem refl weighted by fraction LAI and SAI
    real(rk8) :: tau(lbp:ubp,numrad)        ! leaf/stem tran weighted by fraction LAI and SAI
    real(rk8) :: albsnd(lbc:ubc,numrad)     ! snow albedo (direct)
    real(rk8) :: albsni(lbc:ubc,numrad)     ! snow albedo (diffuse)
    real(rk8) :: coszen_gcell(lbg:ubg)      ! cosine solar zenith angle for next time step (gridcell level)
    real(rk8) :: coszen_col(lbc:ubc)        ! cosine solar zenith angle for next time step (pft level)
    real(rk8) :: coszen_pft(lbp:ubp)        ! cosine solar zenith angle for next time step (pft level)
    ...
    real(rk8) :: albsnd_pur(lbc:ubc,numrad) ! direct pure snow albedo (radiative forcing)
    real(rk8) :: albsni_pur(lbc:ubc,numrad) ! diffuse pure snow albedo (radiative forcing)
    real(rk8) :: albsnd_bc(lbc:ubc,numrad)  ! direct snow albedo without BC (radiative forcing)
    real(rk8) :: albsni_bc(lbc:ubc,numrad)  ! diffuse snow albedo without BC (radiative forcing)
    real(rk8) :: albsnd_oc(lbc:ubc,numrad)  ! direct snow albedo without OC (radiative forcing)
    real(rk8) :: albsni_oc(lbc:ubc,numrad)  ! diffuse snow albedo without OC (radiative forcing)
    real(rk8) :: albsnd_dst(lbc:ubc,numrad) ! direct snow albedo without dust (radiative forcing)
    real(rk8) :: albsni_dst(lbc:ubc,numrad) ! diffuse snow albedo without dust (radiative forcing)
    integer  :: i                          ! index for layers [idx]
    real(rk8) :: flx_absd_snw(lbc:ubc,-nlevsno+1:1,numrad)   ! flux absorption factor for just snow (direct) [frc]
    real(rk8) :: flx_absi_snw(lbc:ubc,-nlevsno+1:1,numrad)   ! flux absorption factor for just snow (diffuse) [frc]
    real(rk8) :: foo_snw(lbc:ubc,-nlevsno+1:1,numrad)        ! dummy array for forcing calls
    real(rk8) :: albsfc(lbc:ubc,numrad)                      ! albedo of surface underneath snow (col,bnd) 
    real(rk8) :: h2osno_liq(lbc:ubc,-nlevsno+1:0)            ! liquid snow content (col,lyr) [kg m-2]
    real(rk8) :: h2osno_ice(lbc:ubc,-nlevsno+1:0)            ! ice content in snow (col,lyr) [kg m-2]
    integer  :: snw_rds_in(lbc:ubc,-nlevsno+1:0)            ! snow grain size sent to SNICAR (col,lyr) [microns]
    real(rk8) :: mss_cnc_aer_in_frc_pur(lbc:ubc,-nlevsno+1:0,sno_nbr_aer) ! mass concentration of aerosol species for forcing calculation (zero) (col,lyr,aer) [kg kg-1]
    real(rk8) :: mss_cnc_aer_in_frc_bc(lbc:ubc,-nlevsno+1:0,sno_nbr_aer)  ! mass concentration of aerosol species for BC forcing (col,lyr,aer) [kg kg-1]
    real(rk8) :: mss_cnc_aer_in_frc_oc(lbc:ubc,-nlevsno+1:0,sno_nbr_aer)  ! mass concentration of aerosol species for OC forcing (col,lyr,aer) [kg kg-1]
    real(rk8) :: mss_cnc_aer_in_frc_dst(lbc:ubc,-nlevsno+1:0,sno_nbr_aer) ! mass concentration of aerosol species for dust forcing (col,lyr,aer) [kg kg-1]
    real(rk8) :: mss_cnc_aer_in_fdb(lbc:ubc,-nlevsno+1:0,sno_nbr_aer)     ! mass concentration of all aerosol species for feedback calculation (col,lyr,aer) [kg kg-1]
  !-----------------------------------------------------------------------
...
! If radiative forcing is being calculated, first estimate clean-snow albedo
    ...
    do ib = 1, nband
       do fc = 1,num_nourbanc
          ...
          if (coszen(c) > 0.D0) then
             ...
             albgrd(c,ib) = albsod(c,ib)*(1.D0-frac_sno(c)) + albsnd(c,ib)*frac_sno(c)
             albgri(c,ib) = albsoi(c,ib)*(1.D0-frac_sno(c)) + albsni(c,ib)*frac_sno(c)
    ...
    do fp = 1,num_nourbanp
       ...
       if (abs(laisum-elai(p)) > 1.D-06 .or. abs(saisum-esai(p)) > 1.D-06) then
          write (stderr,*) 'multi-layer canopy error 01 in SurfaceAlbedo: ',nrad(p),elai(p),laisum,esai(p),saisum
       ...
       if (nlevcan > 1) then
          ...
          if (abs(laisum-tlai(p)) > 1.D-06 .or. abs(saisum-tsai(p)) > 1.D-06) then
             write (stderr,*) 'multi-layer canopy error 02 in SurfaceAlbedo: ',nrad(p),ncan(p)
             write (stderr,*) tlai(p),elai(p),blai(p),laisum,tsai(p),esai(p),bsai(p),saisum
             call fatal(__FILE__,__LINE__,'clm now stopping')
          end if
       end if
...
! !ARGUMENTS:
    ...
    integer , intent(in) :: filter_nourbanc(ubc-lbc+1) ! column filter for non-urban points
    real(rk8), intent(in) :: coszen(lbc:ubc)            ! cos solar zenith angle next time step (column-level)
    real(rk8), intent(in) :: albsnd(lbc:ubc,numrad)     ! snow albedo (direct)
    real(rk8), intent(in) :: albsni(lbc:ubc,numrad)     ! snow albedo (diffuse)
!
! !CALLED FROM:
...
!
    ...
    real(rk8), pointer :: albsod(:,:)        ! soil albedo (direct)
    real(rk8), pointer :: albsoi(:,:)        ! soil albedo (diffuse)
!
!
! !OTHER LOCAL VARIABLES:
!EOP
!
    ...
    real(rk8) :: sicefr        ! Lake surface ice fraction (based on D. Mironov 2010)
!-----------------------------------------------------------------------
       ...
       do fc = 1,num_nourbanc
          ...
          if (coszen(c) > 0.D0) then
             ...
             ! unfrozen lake, wetland
                      ...
                      lake_icefrac(c,1) < 1.D0 .and. lake_icefrac(c,2) > 0.D0) ) then
                ...
                if (ltype(l) == istdlak .and. .not. lakepuddling .and. snl(c) == 0) then
                   ...
                   albsod(c,ib) = sicefr*alblak(ib) + (1.D0-sicefr)*max(alblakwi(ib), &
                                  0.05D0/(max(0.001D0,coszen(c)) + 0.15D0))
                   albsoi(c,ib) = sicefr*alblak(ib) + (1.D0-sicefr)*max(alblakwi(ib), 0.10D0)
...
! !ARGUMENTS:
    ...
    real(rk8), intent(in)  :: coszen(lbp:ubp)          ! cosine solar zenith angle for next time step
    real(rk8), intent(in)  :: rho(lbp:ubp,numrad)      ! leaf/stem refl weighted by fraction LAI and SAI
    real(rk8), intent(in)  :: tau(lbp:ubp,numrad)      ! leaf/stem tran weighted by fraction LAI and SAI
!
! !CALLED FROM:
! subroutine SurfaceAlbedo in this module
!
...
!
    integer , pointer :: pcolumn(:)    ! column of corresponding pft
    real(rk8), pointer :: albgrd(:,:)   ! ground albedo (direct) (column-level)
    real(rk8), pointer :: albgri(:,:)   ! ground albedo (diffuse)(column-level)
    real(rk8), pointer :: t_veg(:)      ! vegetation temperature (Kelvin)
    real(rk8), pointer :: fwet(:)       ! fraction of canopy that is wet (0 to 1)
    integer , pointer :: ivt(:)        ! pft vegetation type
    real(rk8), pointer :: xl(:)         ! ecophys const - leaf/stem orientation index
    real(rk8), pointer :: elai(:)       ! one-sided leaf area index with burying by snow
    real(rk8), pointer :: esai(:)       ! one-sided stem area index with burying by snow
    integer , pointer :: nrad(:)       ! number of canopy layers, above snow for radiative transfer
    real(rk8), pointer :: tlai_z(:,:)   ! tlai increment for canopy layer
...
!
    ...
    real(rk8), pointer :: fabi_sun(:,:) ! flux absorbed by sunlit canopy per unit diffuse flux
    real(rk8), pointer :: fabi_sha(:,:) ! flux absorbed by shaded canopy per unit diffuse flux
    real(rk8), pointer :: ftdd(:,:)     ! down direct flux below canopy per unit direct flx
    real(rk8), pointer :: ftid(:,:)     ! down diffuse flux below canopy per unit direct flx
    real(rk8), pointer :: ftii(:,:)     ! down diffuse flux below canopy per unit diffuse flx
    real(rk8), pointer :: fabd_sun_z(:,:)! absorbed sunlit leaf direct  PAR (per unit lai+sai) for each canopy layer
    real(rk8), pointer :: fabd_sha_z(:,:)! absorbed shaded leaf direct  PAR (per unit lai+sai) for each canopy layer
    real(rk8), pointer :: fabi_sun_z(:,:)! absorbed sunlit leaf diffuse PAR (per unit lai+sai) for each canopy layer
    real(rk8), pointer :: fabi_sha_z(:,:)! absorbed shaded leaf diffuse PAR (per unit lai+sai) for each canopy layer
...
!
    ...
    real(rk8) :: chil(lbp:ubp)    ! -0.4 <= xl <= 0.6
    real(rk8) :: gdir(lbp:ubp)    ! leaf projection in solar direction (0 to 1)
    real(rk8) :: twostext(lbp:ubp)! optical depth of direct beam per unit leaf area
    real(rk8) :: avmu(lbp:ubp)    ! average diffuse optical depth
    real(rk8) :: omega(lbp:ubp,numrad)   ! fraction of intercepted radiation that is scattered (0 to 1)
    ...
    real(rk8) :: tmp0,tmp1,tmp2,tmp3,tmp4,tmp5,tmp6,tmp7,tmp8,tmp9 ! temporary
    real(rk8) :: p1,p2,p3,p4,s1,s2,u1,u2,u3                        ! temporary
    real(rk8) :: b,c1,d,d1,d2,f,h,h1,h2,h3,h4,h5,h6,h7,h8,h9,h10   ! temporary
    real(rk8) :: phi1,phi2,sigma                                   ! temporary
    real(rk8) :: temp0(lbp:ubp),temp1,temp2(lbp:ubp)               ! temporary
    real(rk8) :: t1                                                ! temporary
    real(rk8) :: a1,a2                                             ! parameter for sunlit/shaded leaf radiation absorption
    real(rk8) :: v,dv,u,du                                         ! temporary for flux derivatives
    real(rk8) :: dh2,dh3,dh5,dh6,dh7,dh8,dh9,dh10                  ! temporary for flux derivatives
    real(rk8) :: da1,da2                                           ! temporary for flux derivatives
    real(rk8) :: d_ftid,d_ftii                                     ! ftid, ftii derivative with respect to lai+sai
    real(rk8) :: d_fabd,d_fabi                                     ! fabd, fabi derivative with respect to lai+sai
    real(rk8) :: d_fabd_sun,d_fabd_sha                             ! fabd_sun, fabd_sha derivative with respect to lai+sai
    real(rk8) :: d_fabi_sun,d_fabi_sha                             ! fabi_sun, fabi_sha derivative with respect to lai+sai
    real(rk8) :: laisum                                            ! cumulative lai+sai for canopy layer (at middle of layer)
    real(rk8) :: extkb                                             ! direct beam extinction coefficient
    real(rk8) :: extkn                                             ! nitrogen allocation coefficient
!-----------------------------------------------------------------------
    ...
    do ib = 1, numrad
       do fp = 1,num_vegsol
          ...
          if (t_veg(p) > tfrz) then                             !no snow
             ...
             tmp0 =   (1.D0-fwet(p))*omegal        + fwet(p)*omegas(ib)
             tmp1 = ( (1.D0-fwet(p))*omegal*betadl + fwet(p)*omegas(ib)*betads ) / tmp0
             tmp2 = ( (1.D0-fwet(p))*omegal*betail + fwet(p)*omegas(ib)*betais ) / tmp0
          end if
          omega(p,ib) = tmp0           
          betad = tmp1 
          betai = tmp2  
          ...
          ftdd(p,ib) = s2
          fabd(p,ib) = 1.D0 - albd(p,ib) - (1.D0-albgrd(c,ib))*ftdd(p,ib) - (1.D0-albgri(c,ib))*ftid(p,ib)
          ...
          fabd_sun(p,ib) = (1.D0 - omega(p,ib)) * ( 1.D0 - s2 + 1.D0 / avmu(p) * (a1 + a2) )
          ...
          a1 = h7 * (1.D0 - s2*s1) / (twostext(p) + h) +  h8 * (1.D0 - s2/s1) / (twostext(p) - h)
          a2 = h9 * (1.D0 - s2*s1) / (twostext(p) + h) + h10 * (1.D0 - s2/s1) / (twostext(p) - h)
          ...
          if (ib == 1) then
             if (nlevcan == 1) then
                ...
                extkn = 0.30D0
                extkb = twostext(p)
                vcmaxcintsun(p) = (1.D0 - exp(-(extkn+extkb)*elai(p))) / (extkn + extkb)
                vcmaxcintsha(p) = (1.D0 - exp(-extkn*elai(p))) / extkn - vcmaxcintsun(p)
                ...
                if (iv == 1) then
                   ...
                   laisum = laisum + 0.5D0 * ((tlai_z(p,iv-1)+tsai_z(p,iv-1))+(tlai_z(p,iv)+tsai_z(p,iv)))
                end if
                ...
                d_fabd = -(dh2+dh3) + (1.D0-albgrd(c,ib))*twostext(p)*s2 - (1.D0-albgri(c,ib))*d_ftid
                d_fabd_sun = (1.D0 - omega(p,ib)) * (twostext(p)*s2 + 1.D0 / avmu(p) * (da1 + da2))
                ...
                da1 = h7*s2*s1 +  h8*s2/s1 + (1.D0-s2*s1)/(twostext(p)+h)*dh7 + (1.D0-s2/s1)/(twostext(p)-h)*dh8
                da2 = h9*s2*s1 + h10*s2/s1 + (1.D0-s2*s1)/(twostext(p)+h)*dh9 + (1.D0-s2/s1)/(twostext(p)-h)*dh10
...
!-------------------------------------------------------------------------------
   ...
   !------------------------------ Parameters ----------------------------------
   integer(ik4),parameter :: poblen =47 ! # of elements in series wrt obliquity
   integer(ik4),parameter :: pecclen=19 ! # of elements in series wrt eccentricity
   integer(ik4),parameter :: pmvelen=78 ! # of elements in series wrt vernal equinox
   real   (rk8),parameter :: psecdeg = 1.0D0/3600.0D0 ! arc sec to deg conversion
   ...
   real   (rk8), parameter :: obamp(poblen) =  & ! amplitudes for obliquity cos series
   ...
   real   (rk8), parameter :: obrate(poblen) = & ! rates for obliquity cosine series
   ...
   real   (rk8), parameter :: obphas(poblen) = & ! phases for obliquity cosine series
   ...
   real   (rk8), parameter :: ecamp (pecclen) = & ! ampl for eccen/fvelp cos/sin series
   ...
   real   (rk8), parameter :: ecrate(pecclen) = & ! rates for eccen/fvelp cos/sin series
   ...
   real   (rk8), parameter :: ecphas(pecclen) = & ! phases for eccen/fvelp cos/sin series
   ...
   real   (rk8), parameter :: mvamp (pmvelen) = & ! amplitudes for mvelp sine series 
   ...
   real   (rk8), parameter :: mvrate(pmvelen) = & ! rates for mvelp sine series 
          (/    31.609974D0, 32.620504D0, 24.172203D0,   &
   ...
   real   (rk8), parameter :: mvphas(pmvelen) = & ! phases for mvelp sine series
          (/    251.9025D0, 280.8325D0, 128.3057D0,   &
...
!    write(stdout,F00) svnURL
   ...
   IF ( iyear_AD == SHR_ORB_UNDEF_INT ) THEN
      ...
      if( obliq == SHR_ORB_UNDEF_REAL )then
         ...
         write(stdout,F00) 'iyear_AD is the year to simulate orbit for (ie. 1950): '
