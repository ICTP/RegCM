  ...
  subroutine Biogeophysics1(lbg, ubg, lbc, ubc, lbp, ubp, &
    ...
    do fc = 1,num_nolakec
        ...
        if (ityplun(l) == istsoil .or. ityplun(l) == istcrop) then
          ...
          do j = 1, nlevsoi
            if (t_soisno(c,j) >= tfrz) then
              ...
              fac = min( max(vol_liq-watdry(c,j),0.D0) / &
                             (watopt(c,j)-watdry(c,j)), 1.D0 )
            else
              fac = 0.D0
            end if
      ...
      if (ityplun(l) == istsoil .or. ityplun(l) == istcrop) then
        ...
        ! qg(c) = frac_sno(c)*qg_snow(c) + (1.D0 - frac_sno(c) - &
        !           frac_h2osfc(c))*qg_soil(c) &
        qg(c) = frac_sno_eff(c)*qg_snow(c) + &
          (1.D0 - frac_sno_eff(c) - frac_h2osfc(c))*qg_soil(c) &
      ...
      htvp(c) = hvap
      if ( h2osoi_liq(c,snl(c)+1) <= 0.D0 .and. &
           h2osoi_ice(c,snl(c)+1) > 0.D0 ) htvp(c) = hsub
