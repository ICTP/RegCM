...
! !USES:
  ...
  use mod_clm_subgridave, only: p2l_1d
...
!
  ...
  real(rk8) tmp1          !Factor in saltation computation (named as in Charlie's code)
...
! !USES
   ...
   use mod_clm_varcon , only : denh2o
!
! !ARGUMENTS:
    implicit none
...
!
    ...
    real(rk8), pointer :: gwc_thr(:)         ! threshold gravimetric soil moisture based on clay content
...
!
    real(rk8), pointer :: flx_mss_vrt_dst(:,:)   ! surface dust emission (kg/m**2/s) 
...
!
    ...
    real(rk8) :: liqfrac             ! fraction of total water that is liquid
    real(rk8) :: wnd_frc_rat         ! [frc] Wind friction threshold over wind friction
    real(rk8) :: wnd_frc_slt_dlt     ! [m s-1] Friction velocity increase from saltatn
    real(rk8) :: wnd_rfr_dlt         ! [m s-1] Reference windspeed excess over threshld
...
! constants
!
    real(rk8), parameter :: cst_slt = 2.61D0           ! [frc] Saltation constant
    real(rk8), parameter :: flx_mss_fdg_fct = 5.0D-4 ! [frc] Empir. mass flx tuning eflx_lh_vegt
    real(rk8), parameter :: vai_mbl_thr = 0.3D0        ! [m2 m-2] VAI threshold quenching dust mobilization
...
! initialize variables which get passed to the atmosphere
    ...
    do fp = 1,num_nolakep
       ...
       if (lnd_frc_mbl(p)>1.0D0 .or. lnd_frc_mbl(p)<0.0D0) then
          write(stderr,*)'Error dstmbl: pft= ',p,' lnd_frc_mbl(p)= ',lnd_frc_mbl(p)
          call fatal(__FILE__,__LINE__,'clm now stopping')
       end if
    end do
    ...
    do fp = 1,num_nolakep
       ...
       if (lnd_frc_mbl(p) > 0.0D0) then
          ...
          bd = (1.D0-watsat(c,1))*2.7D3      ![kg m-3] Bulk density of dry surface soil
          gwc_sfc = h2osoi_vol(c,1)*denh2o/bd    ![kg kg-1] Gravimetric H2O cont
          if (gwc_sfc > gwc_thr(c)) then
             frc_thr_wet_fct = sqrt(1.0D0 + 1.21D0 * (100.0D0*(gwc_sfc - gwc_thr(c)))**0.68D0)
          else
          ...
          liqfrac = max( 0.0D0, min( 1.0D0, h2osoi_liq(c,1) / (h2osoi_ice(c,1)+h2osoi_liq(c,1)+1.0D-6) ) )
          ...
          if (wnd_frc_slt > wnd_frc_thr_slt) then
             ...
             flx_mss_hrz_slt_ttl = cst_slt * forc_rho(g) * (wnd_frc_slt**3.0D0) * &
                  (1.0D0 - wnd_frc_rat) * (1.0D0 + wnd_frc_rat) * (1.0D0 + wnd_frc_rat) / grav
          ...
          dst_slt_flx_rat_ttl = 100.0D0 * exp( log(10.0D0) * (13.4D0 * mss_frc_cly_vld(c) - 6.0D0) )
...
!
    ...
    real(rk8) :: vsc_knm_atm(lbp:ubp)  ! [m2 s-1] Kinematic viscosity of atmosphere
    real(rk8) :: shm_nbr_xpn           ! [frc] Sfc-dep exponent for aerosol-diffusion dependence on Schmidt number
...
!
    ...
    real(rk8) :: ryn_nbr_frc_thr_prx_opt ! [frc] Threshold friction Reynolds number approximation for optimal size
    real(rk8) :: ryn_nbr_frc_thr_opt_fnc ! [frc] Threshold friction Reynolds factor for saltation calculation
    real(rk8) :: icf_fct                 ! Interpartical cohesive forces factor for saltation calc
    real(rk8) :: dns_fct                 ! Density ratio factor for saltation calculation
    ...
    ! constants
    real(rk8) :: dmt_vma_src(dst_src_nbr) =    &     ! [m] Mass median diameter
         (/ 0.832D-6 , 4.82D-6 , 19.38D-6 /)        ! BSM96 p. 73 Table 2
    real(rk8) :: gsd_anl_src(dst_src_nbr) =    &     ! [frc] Geometric std deviation
         (/ 2.10D0     ,  1.90D0   , 1.60D0     /)        ! BSM96 p. 73 Table 2
    real(rk8) :: mss_frc_src(dst_src_nbr) =    &     ! [frc] Mass fraction 
         (/ 0.036D0, 0.957D0, 0.007D0 /)                  ! BSM96 p. 73 Table 2
    real(rk8) :: dmt_grd(5) =                  &     ! [m] Particle diameter grid
         (/ 0.1D-6, 1.0D-6, 2.5D-6, 5.0D-6, 10.0D-6 /)
    real(rk8), parameter :: dmt_slt_opt = 75.0D-6    ! [m] Optim diam for saltation
    real(rk8), parameter :: dns_slt = 2650.0D0         ! [kg m-3] Density of optimal saltation particles
...
!------------------------------------------------------------------------
    ...
    if (ryn_nbr_frc_thr_prx_opt < 0.03D0) then
       ...
       ryn_nbr_frc_thr_opt_fnc = -1.0D0 + 1.928D0 * (ryn_nbr_frc_thr_prx_opt**0.0922D0)
       ryn_nbr_frc_thr_opt_fnc = 0.1291D0 * 0.1291D0 / ryn_nbr_frc_thr_opt_fnc
    else
       ryn_nbr_frc_thr_opt_fnc = 1.0D0 - 0.0858D0 * exp(-0.0617D0*(ryn_nbr_frc_thr_prx_opt-10.0D0))
       ryn_nbr_frc_thr_opt_fnc = 0.120D0 * 0.120D0 * ryn_nbr_frc_thr_opt_fnc * ryn_nbr_frc_thr_opt_fnc
    end if
    ...
    do m = 1, ndst
       ...
       do while(eps_crr > eps_max)
          ...
          if (ryn_nbr_grv(m) < 0.1D0) then
             cff_drg_grv(m) = 24.0D0 / ryn_nbr_grv(m) !Stokes' law Sep97 p.463 (8.32)
             ...
             write(stderr,*)'Dustini error: Reynolds number too large in stk_crc_get()'
             call fatal(__FILE__,__LINE__,'clm now stopping')
          end if
