...
module mod_cmip6_mpihr
  ...
  character(len=*), parameter :: mpihr_version = 'v20190710'
  character(len=*), parameter :: mpihr_version1 = 'v20190815'

  public :: read_3d_mpihr, read_2d_mpihr, read_fx_mpihr, read_sst_mpihr
  ...
  contains
    ...
    recursive subroutine read_3d_mpihr(idate,v,lonlyc)
      ...
      if ( .not. associated(v%var) ) then
        ...
        call getmem(v%var,1,v%ni,1,v%nj,1,v%nk,'cmip6:mpihr:'//trim(v%vname))
#ifdef DEBUG
    ...
    recursive subroutine read_2d_mpihr(idate,v,lonlyc)
      ...
      if ( .not. associated(v%var) ) then
        v%ni = size(v%hcoord%lon1d)
        v%nj = size(v%hcoord%lat1d)
        call getmem(v%var,1,v%ni,1,v%nj,'cmip6:mpihr:'//trim(v%vname))
#ifdef DEBUG
    ...
    recursive subroutine read_sst_mpihr(idate,v,lat,lon)
      ...
      if ( irec > v%nrec ) then
        ...
        v%nrec = -1
        irec = 1
        call read_sst_mpihr(idate,v,lat,lon)
      end if
      istart(1) = 1
      istart(2) = 1
      istart(3) = irec
