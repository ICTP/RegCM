...
module mod_savefile
  ...
  subroutine write_savefile(idate)
    ...
    if ( myid == iocpu ) then
...
#ifdef CLM45
        ncstatus = nf90_def_dim(ncid,'ndust',4,dimids(idndu))
        call check_ok(__FILE__,__LINE__,'Cannot create dimension ndust')
#endif
      end if
      if ( lakemod == 1 ) then
      ...
      call mydefvar(ncid,'tsoi',regcm_vartype,wrkdim,1,3,varids,ivcc)
      call mydefvar(ncid,'swvol',regcm_vartype,wrkdim,1,3,varids,ivcc)
#else
      ...
      if ( ichem == 1 ) then
        ...
        wrkdim(3) = dimids(idkh)
        wrkdim(4) = dimids(idntr)
        call mydefvar(ncid,'convpr',regcm_vartype,wrkdim,1,3,varids,ivcc)
        ...
        call mydefvar(ncid,'duflux',regcm_vartype,wrkdim,1,3,varids,ivcc)
        wrkdim(3) = dimids(idntr)
        call mydefvar(ncid,'voflux',regcm_vartype,wrkdim,1,3,varids,ivcc)
#else
