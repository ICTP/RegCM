      ...
      do k = 1 , kz
        do i = 2 , iym2
          rho(i,k) = (sps2%ps(i,j)*a(k)+r8pt)* &
                      1000./287./atm2%t(i,k,j)*sps2%ps(i,j)
          wl(i,k) = atm2%qc(i,k,j)/sps2%ps(i,j)*1000.*rho(i,k)
        end do
      end do
...
!           transport linked to downdraft
            ...
            cutend_dwd(i,kz) = +mflx(i,2)*gti*1.E-3*wk(i,kdwd)          &
                             & /(dsigma(kz))
...
!             cb*kg/kg do tests, suppose h2o2 always enough
              concmin(i,k) = chimol*64./28.9*sps2%ps(i,j)     ! cb*kg/kg
            end do
          end do
...
!           method based on bats diagnostic in routine interf.
            ...
            if ( (ivegcov(i).ne.0) ) then
              facv = dlog(za(i,kz,j)/10.)                               &
                   & /dlog(za(i,kz,j)/rough(ivegcov(i)))
              facb = dlog(za(i,kz,j)/10.)/dlog(za(i,kz,j)/zlnd)
              facs = dlog(za(i,kz,j)/10.)/dlog(za(i,kz,j)/zsno)
...
!           specific  humidity at 10m
            shu10 = (atm2%qv(i,kz,j)/sps2%ps(i,j))/ &
                    (1.+atm2%qv(i,kz,j)/sps2%ps(i,j))-sdelqk2d(i,j)*fact
...
!
            settend(i,kz) = -(wk(i,kz)*pdepv(i,kz,ibin)*gti*1.E-3)      &
                          & /dsigma(kz)
