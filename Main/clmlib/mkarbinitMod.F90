...
!!!         also convert from meters to kg/m^2         !!!
    ...
    allocate(temp_veg(lsmlon,lsmlat))
    allocate(temp_veg1(lsmlon,lsmlat))

    do jj = 1,lsmlat
    do ii = 1,lsmlon
        g = (jj-1)*lsmlon + ii
        temp_veg(ii,jj)  = satbrt_clm(ii,jj)
        temp_veg1(ii,jj) = satbrt_clm(ii,jj)
    enddo
    enddo
...
!abt added section below
             if(temp_veg(ii,jj).lt.15.1 .and. temp_veg(ii,jj).gt.13.9) then
                temp_veg(ii,jj) = 0
             endif

             nveg = nint(temp_veg(ii,jj))
             if(nveg.eq.0) then
                nveg = 15
             else
                nveg = nveg
