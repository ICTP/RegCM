...
!local pointers to original implicit in arrays
    ...
    real(rk8) :: sum_frac(1:nlayer)                  ! sum of fraction for each layer
   ...
   do i = 1, nlayer
      ...
      do j = 1, nlevsoi
         if( (zsum < lsum) .and. (zsum + dz(c,j) >= lsum ))  then
             call linear_interp(lsum, temp, zsum, zsum + dz(c,j), 0.D0, 1.D0)
             vic_clm_fract(c,i,j) = 1.D0 - temp
             if(lsum + deltal(i) < zsum + dz(c,j)) then
                   call linear_interp(lsum + deltal(i), temp, zsum, zsum + dz(c,j), 1.D0, 0.D0)
                   vic_clm_fract(c,i,j) = vic_clm_fract(c,i,j) - temp
             end if
         else if( (zsum < lsum + deltal(i)) .and. (zsum + dz(c,j) >= lsum + deltal(i)) ) then
             call linear_interp(lsum + deltal(i), temp, zsum, zsum + dz(c,j), 0.D0, 1.D0)
             ...
             if(zsum<=lsum) then
              ...
              vic_clm_fract(c,i,j) = 1.D0
         else
              vic_clm_fract(c,i,j) = 0.D0
         end if
         zsum = zsum + dz(c,j)
 ...
 ! Assign local pointers to derived subtypes components (column-level)
   ...
   do fc = 1, numf
      ...
      do i = 1, nlayer
         ...
         moist_vol(c,i) = max(0.01D0, moist_vol(c,i))
      end do
