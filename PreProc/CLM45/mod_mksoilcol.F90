module mod_mksoilcol
  ...
  subroutine mksoilcol(soilcolfile,soilcol)
    implicit none
    character(len=*) , intent(in) :: soilcolfile
    real(rk4) , dimension(:,:) , intent(out) :: soilcol
    ...
    call checkncerr(istatus,__FILE__,__LINE__, &
      'Cannot find variable soilcol in file '//trim(inpfile))
    ...
    do i = 1 , domain%ntiles
      ...
      call checkncerr(istatus,__FILE__,__LINE__, &
        'Cannot read variable soilcol from file '//trim(inpfile))
    ...
    call bilinear(rvar,rmask,rlon,rlat,soilcol,xlon,xlat,vmin,vmisdat)
