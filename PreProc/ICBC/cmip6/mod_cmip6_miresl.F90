...
module mod_cmip6_miresl
  ...
  contains
    ...
    subroutine read_hcoord_miresl(ncid,lon,lat)
      ...
      call getmem(lon,1,nlon,'cmip6_miresl:lon')
      call getmem(lat,1,nlat,'cmip6_miresl:lat')
    ...
    subroutine read_hcoord_sst_miresl(ncid,lon,lat)
      ...
      call getmem(lon,1,nlon,1,nlat,'cmip6_miresl:lon')
      call getmem(lat,1,nlon,1,nlat,'cmip6_miresl:lat')
    ...
    subroutine read_vcoord_miresl(ncid,a,b,p0)
      ...
      call getmem(a,1,nlev,'cmip6_miresl:a')
      call getmem(b,1,nlev,'cmip6_miresl:b')
    ...
    recursive subroutine read_3d_miresl(idate,v,lonlyc)
      ...
      if ( .not. associated(v%var) ) then
        ...
        call getmem(v%var,1,v%ni,1,v%nj,1,v%nk,'cmip6:miresl:'//trim(v%vname))
#ifdef DEBUG
    ...
    recursive subroutine read_2d_miresl(idate,v,lonlyc)
      ...
      if ( .not. associated(v%var) ) then
        v%ni = size(v%hcoord%lon1d)
        v%nj = size(v%hcoord%lat1d)
        call getmem(v%var,1,v%ni,1,v%nj,'cmip6:miresl:'//trim(v%vname))
#ifdef DEBUG
