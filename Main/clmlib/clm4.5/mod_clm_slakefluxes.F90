...
! !USES:
    ...
    use mod_clm_varcon         , only : hvap, hsub, hfus, cpair, cpliq, tkwat, tkice, tkair, &
...
!
    ...
    real(rk8), pointer :: z0mg_col(:)        ! roughness length over ground, momentum [m]
    real(rk8), pointer :: z0hg_col(:)        ! roughness length over ground, sensible heat [m]
    real(rk8), pointer :: z0qg_col(:)        ! roughness length over ground, latent heat [m]
...
!
    ...
    real(rk8), parameter :: beta1 = 1.D0  ! coefficient of convective velocity (in computing W_*) [-]
    real(rk8), parameter :: zii = 1000.D0 ! convective boundary height [m]
    ...
    real(rk8) :: ax                      ! used in iteration loop for calculating t_grnd (numerator of NR solution)
    real(rk8) :: bx                      ! used in iteration loop for calculating t_grnd (denomin. of NR solution)
    ...
    real(rk8) :: dzsur(lbc:ubc)          ! 1/2 the top layer thickness (m)
    real(rk8) :: eg                      ! water vapor pressure at temperature T [pa]
    real(rk8) :: htvp(lbc:ubc)           ! latent heat of vapor of water (or sublimation) [j/kg]
    real(rk8) :: obu(lbp:ubp)            ! monin-obukhov length (m)
    real(rk8) :: obuold(lbp:ubp)         ! monin-obukhov length of previous iteration
    ...
    real(rk8) :: stftg3(lbp:ubp)         ! derivative of fluxes w.r.t ground temperature
    ...
    real(rk8) :: tgbef(lbc:ubc)          ! initial ground temperature
    real(rk8) :: thm(lbp:ubp)            ! intermediate variable (forc_t+0.0098*forc_hgt_t_pft)
    real(rk8) :: thv(lbc:ubc)            ! virtual potential temperature (kelvin)
    real(rk8) :: thvstar                 ! virtual potential temperature scaling parameter
    real(rk8) :: tksur(lbc:ubc)          ! thermal conductivity of snow/soil (w/m/kelvin)
    ...
    real(rk8) :: z0hg(lbp:ubp)           ! roughness length over ground, sensible heat [m]
    real(rk8) :: z0qg(lbp:ubp)           ! roughness length over ground, latent heat [m]
    ...
    real(rk8) :: t_grnd_temp             ! Used in surface flux correction over frozen ground
    real(rk8) :: betaprime(lbc:ubc)      ! Effective beta: sabg_lyr(p,jtop) for snow layers, beta otherwise
    ...
    real(rk8), parameter :: kva0 = 1.51D-5   ! kinematic viscosity of air (m^2/s) at 20C and 1.013e5 Pa
    real(rk8) :: kva0temp                ! (K) temperature for kva0; will be set below
    real(rk8), parameter :: kva0pres = 1.013D5 ! (Pa) pressure for kva0
    real(rk8) :: kva                     ! kinematic viscosity of air at ground temperature and forcing pressure
    real(rk8), parameter :: prn = 0.713  ! Prandtl # for air at neutral stability
    real(rk8), parameter :: sch = 0.66   ! Schmidt # for water in air at neutral stability
...
!-----------------------------------------------------------------------
    ...
    do fp = 1, num_lakep
       ...
       if (t_grnd(c) > tfrz) then   ! for unfrozen lake
          z0mg(p) = z0mg_col(c)
          kva = kva0 * (t_grnd(c)/kva0temp)**1.5D0 * kva0pres/forc_pbot(g) ! kinematic viscosity of air
          sqre0 = (max(z0mg(p)*ust_lake(c)/kva,0.1D0))**0.5D0   ! Square root of roughness Reynolds number
          z0hg(p) = z0mg(p) * exp( -vkc/prn*( 4.D0*sqre0 - 3.2D0) ) ! SH roughness length
          z0qg(p) = z0mg(p) * exp( -vkc/sch*( 4.D0*sqre0 - 4.2D0) ) ! LH roughness length
          z0qg(p) = max(z0qg(p), minz0lake)
          z0hg(p) = max(z0hg(p), minz0lake)
       else if (snl(c) == 0) then    ! frozen lake with ice
          z0mg(p) = z0frzlake
          z0hg(p) = z0mg(p)/exp(0.13D0 * (ust_lake(c)*z0mg(p)/1.5D-5)**0.45D0) ! Consistent with BareGroundFluxes
          z0qg(p) = z0hg(p)
       ...
       if (snl(c) < 0) then
           betaprime(c) = sabg_lyr(p,jtop(c))/max(1.D-5,sabg(p))  ! Assuming one pft
...
!#endif
    ...
    ITERATION : do while (iter <= niters .and. fncopy > 0)
       ...
       do fp = 1, fncopy
          ...
          ax  = betaprime(c)*sabg(p) + emg_lake*forc_lwrad(g) + 3.D0*stftg3(p)*tgbef(c) &
...
!#endif
          ...
          if (t_grnd(c) > tfrz .and. snl(c) == 0) then ! t_grnd hasn't been corrected yet if snow layers but above frz
             ...
             if (lake_use_old_fcrit_minz0) then
                ...
                cur = cur0 + curm* exp( max( -(fetch(c)*grav/ur(p)/ur(p))**(1.D0/3.D0)/fcrit, &   ! Fetch-limited
                ...
                cur = cur0 + curm* exp( max( -(fetch(c)*grav/ustar(p)/ustar(p))**(1.D0/3.D0)/fcrit, &   ! Fetch-limited
                                          -(lakedepth(c)*grav/ur(p)/ur(p))**0.5D0 ) )           ! depth-limited
             end if
             kva = kva0 * (t_grnd(c)/kva0temp)**1.5D0 * kva0pres/forc_pbot(g) ! kinematic viscosity of air
             z0mg(p) = max(cus*kva/max(ustar(p),1.D-4), cur*ustar(p)*ustar(p)/grav) ! momentum roughness length
             ...
             sqre0 = (max(z0mg(p)*ustar(p)/kva,0.1D0))**0.5D0   ! Square root of roughness Reynolds number
             z0hg(p) = z0mg(p) * exp( -vkc/prn*( 4.D0*sqre0 - 3.2D0) ) ! SH roughness length
             z0qg(p) = z0mg(p) * exp( -vkc/sch*( 4.D0*sqre0 - 4.2D0) ) ! LH roughness length
             z0qg(p) = max(z0qg(p), minz0lake)
             z0hg(p) = max(z0hg(p), minz0lake)
             ...
             z0mg(p) = z0frzlake
             z0hg(p) = z0mg(p)/exp(0.13D0 * (ustar(p)*z0mg(p)/1.5D-5)**0.45D0) ! Consistent with BareGroundFluxes
             z0qg(p) = z0hg(p)
          else ! Snow layers
...
!#endif
       ...
       ! eflx_lwrad_out(p) = (1.D0-emg_lake)*forc_lwrad(g) + stftg3(p)*(-3.D0*tgbef(c)+4.D0*t_grnd(c))
       ! What is tgbef doing in this equation? Can't it be exact now? --Zack Subin, 4/14/09
       eflx_lwrad_out(p) = (1.D0-emg_lake)*forc_lwrad(g) + emg_lake*sb*t_grnd(c)**4.D0
