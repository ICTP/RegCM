...
! !USES:
    ...
    use mod_clm_soilhydrology, only : Infiltration, SoilWater, Drainage, SurfaceRunoff
    use mod_clm_time_manager, only : get_step_size, get_nstep, is_perpetual
#if (defined VICHYDRO)
...
!
    ...
    real(rk8), pointer :: frac_sno_eff(:)  !eff.  snow cover fraction (col) [frc]
...
!
    ...
    real(rk8), pointer :: wf(:)            ! soil water as frac. of whc for top 0.05 m  !F. Li and S. Levis
    real(rk8), pointer :: wf2(:)           ! soil water as frac. of whc for top 0.17 m added by F. Li and S. Levis 
    ...
    real(rk8), pointer :: tsoi17(:)         ! soil temperature in top 17cm of soil (Kelvin) added by F. Li and S. Levis
   real(rk8), pointer :: h2osoi_liqice_10cm(:) ! liquid water + ice lens in top 10cm of soil (kg/m2)
    ...
    real(rk8), pointer :: mss_bctot(:,:)     ! total mass of BC (pho+phi) (col,lyr) [kg]
    real(rk8), pointer :: mss_bc_col(:)      ! total mass of BC in snow column (col) [kg]
    real(rk8), pointer :: mss_bc_top(:)      ! total mass of BC in top snow layer (col) [kg]
    ...
    real(rk8), pointer :: mss_octot(:,:)     ! total mass of OC (pho+phi) (col,lyr) [kg]
    real(rk8), pointer :: mss_oc_col(:)      ! total mass of OC in snow column (col) [kg]
    real(rk8), pointer :: mss_oc_top(:)      ! total mass of OC in top snow layer (col) [kg]
    ...
    real(rk8), pointer :: mss_dst_col(:)     ! total mass of dust in snow column (col) [kg]
    real(rk8), pointer :: mss_dst_top(:)     ! total mass of dust in top snow layer (col) [kg]
    ...
    real(rk8), pointer :: qflx_glcice(:)     ! flux of new glacier ice (mm H2O /s)
...
!
    ...
    real(rk8) :: vol_liq(lbc:ubc,1:nlevgrnd)! partial volume of liquid water in layer
    real(rk8) :: dwat(lbc:ubc,1:nlevgrnd)   ! change in soil water
    real(rk8) :: hk(lbc:ubc,1:nlevgrnd)     ! hydraulic conductivity (mm h2o/s)
    real(rk8) :: dhkdw(lbc:ubc,1:nlevgrnd)  ! d(hk)/d(vol_liq)
    real(rk8) :: psi,vwc,fsattmp,psifrz     ! temporary variables for soilpsi calculation
#if (defined CN) 
    real(rk8) :: watdry                     ! temporary
    real(rk8) :: rwat(lbc:ubc)              ! soil water wgted by depth to maximum depth of 0.5 m
    real(rk8) :: swat(lbc:ubc)              ! same as rwat but at saturation
    real(rk8) :: rz(lbc:ubc)                ! thickness of soil layers contributing to rwat (m)
    ...
    real(rk8) :: fracl                      ! fraction of soil layer contributing to 10cm total soil water
