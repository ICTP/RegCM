c ** PGI compile: pgf77 WM2RCM.f -L../../Commons/env/liblinux -lnetcdf
c ** IFC compile: ifc -tpp7 -O3 -cm -w -w90 -w95 WM2RCM.f -L../../Commons/env/liblinux -lnetcdf -o wm
      PROGRAM RWDATA
     ...
     &   , clatx, clonx, platx, plonx, dsx
      ...
      real gridlalo(NLON,NLAT), LONI(NLON), LATI(NLAT)

C ******    NetCDF Output Stuff
      real xlat1d(ny), xlon1d(nx), vmisdat, xmin
      INTEGER idim(3), idt0(4), idt1(4), idt2(4), ndim, ncid, julnc
     &      , idcdf, idcdfout, ncopn, ncdid, ncvid, ncnowrit
      integer*2 imisdat
      parameter (vmisdat =-32767.,xmin=-100., ndim=3)
      parameter(ncnowrit = 0)
      character lnam*20, units*13
...
C ****** SET UP NetCDF STUFF
     ...
     &   , xlat,xlon,vvarmin,vvarmax,xlat1d,xlon1d,idim,ndim)
      ...
      sigf(2) = 1.
      sig1d(1) = 1.
      idatex0 = idatewm0*10000 + 1500
      idatex1 = idatewm1*10000 + 1500
      idatex2 = idatewm2*10000 + 1500
...
C ****** READ/WRITE DATA
      do ifld=1,nfld
        ...
        CALL rcrecdf (outfile(ifld),ncid,vvarmin,vvarmax,3,ierr)
        idatex = idatex1
        imo = idt1(2)
        iyr = idt1(1)
        idyr = idt1(1)-idt0(1)
     ...
     &           , gridlalo,scale,offset,vmisdat,lnam,units)
          CALL BILINX(GRIDLALO,LONI,LATI,NLON,NLAT
     A       , GRIDRCM,XLON,XLAT,NY,NX,1,xmin,vmisdat)
          do j=1,ny
          do i=1,nx
     ...
     &         , vals,scale,offset,vmisdat,lnam,units)
      ...
      real*4 scale,offset,error,vmisdat
      real vals(nlon,nlat)
      integer*2 igrid2d(nlon,nlat),imisdat
...
c  /*get variable and attributes*/
      ...
      do j=1,nlat
      do i=1,nlon
        if (igrid2d(i,j).gt.-32767) then
          vals(i,j) = float(igrid2d(i,j))*scale + offset
        else
          vals(i,j) = vmisdat
        end if
      end do
