...
module mod_cloud_subex
  ...
  subroutine subex_cldfrac(t,p,qv,rh,tc0,rh0,rhcrit,fcc)
    ...
    if ( use_dynamic_rhcrit ) then
      call random_number(rhcrit)
      do concurrent ( j = jci1:jci2, i = ici1:ici2, k = 1:kz )
        tku = min(301.0_rkx,max(216.0_rkx,t(j,i,k)))
        rhcrit(j,i,k) = rh0(j,i) + 0.10_rkx * (rhcrit(j,i,k)-0.5_rkx)
        rhcrit(j,i,k) = 0.01_rkx * &
          max(60.0_rkx, min(100.0_rkx, rhcrit(j,i,k)*100.0_rkx + &
    ...
    do concurrent ( j = jci1:jci2, i = ici1:ici2, k = 1:kz )
      ...
      if ( rhrng <= rhcrit(j,i,k) ) then
        fcc(j,i,k) = d_zero
      else if ( rhrng > 0.99999_rkx ) then
        fcc(j,i,k) = d_one
      else
        ! Use Pal et al. (2000) formula
        ! fcc(j,i,k) = sqrt((rhrng-rh0adj)/(rhmax-rhcrit(j,i,k)))
        ! Use Sundqvist (1989) formula
        fcc(j,i,k) = rhmax - sqrt((rhmax-rhrng)/(rhmax-rhcrit(j,i,k)))
      end if
    end do
    !
    ! Correction:
