...
! !USES:
    ...
    use mod_clm_varpar         , only : nlevgrnd, nlevurb, nlevsno, max_pft_per_gcell, nlevsoi
...
!
    ...
    real(rk8), pointer :: forc_hgt_q(:)    !observational height of specific humidity [m]
    ...
    real(rk8), pointer :: watfc(:,:)       !volumetric soil water at field capacity
    real(rk8), pointer :: watopt(:,:)      !volumetric soil moisture corresponding to no restriction on ET from urban pervious surface
    real(rk8), pointer :: watdry(:,:)      !volumetric soil moisture corresponding to no restriction on ET from urban pervious surface
...
!
    ...
    real(rk8) ,pointer :: soilbeta(:)      !factor that reduces ground evaporation
...
!
    ...
    real(rk8) :: fac_fc        !soil wetness of surface layer relative to field capacity
    ...
    integer  :: pi            !index
    real(rk8) :: fh2o_eff(lbc:ubc) ! effective surface water fraction (i.e. seen by atm)
!------------------------------------------------------------------------------
    ...
    do fc = 1,num_nolakec
       ...
       if (ityplun(l)/=istwet .AND. ityplun(l)/=istice ) then
          if (ityplun(l) == istsoil .or. ityplun(l) == istcrop) then
             ...
             qred = (1.D0 - frac_sno(c) - frac_h2osfc(c))*hr &
             ...
             if (wx < watfc(c,1) ) then  !when water content of ths top layer is less than that at F.C.
                fac_fc  = min(1.D0, wx/watfc(c,1))  !eqn5.66 but divided by theta at field capacity
                fac_fc  = max( fac_fc, 0.01D0 )
                ! modify soil beta by snow cover. soilbeta for snow surface is one
                soilbeta(c) = (1.D0-frac_sno(c)-frac_h2osfc(c)) &
                     *0.25D0*(1.D0 - cos(mathpi*fac_fc))**2.D0 &
                              + frac_sno(c)+ frac_h2osfc(c)
          ...
          ! Pervious road depends on water in total soil column
             ...
             do j = 1, nlevsoi
                if (t_soisno(c,j) >= tfrz) then
                   ...
                   fac = min( max(vol_liq-watdry(c,j),0.D0) / (watopt(c,j)-watdry(c,j)), 1.D0 )
                else
                   fac = 0.D0
                end if
       ...
       if (ityplun(l) == istsoil .or. ityplun(l) == istcrop) then
          ...
          qg_soil(c) = hr*qsatg
          dqgdT(c) = dqgdT(c) + (1.D0 - frac_sno(c) - frac_h2osfc(c))*hr*qsatgdT
          ...
          if (snl(c) >= 0) then
             qg_snow(c) = qg_soil(c)
             dqgdT(c) = (1.D0 - frac_h2osfc(c))*hr*dqgdT(c)
          endif
...
!          qg(c) = frac_sno(c)*qg_snow(c) + (1.D0 - frac_sno(c) - frac_h2osfc(c))*qg_soil(c) &
          qg(c) = frac_sno_eff(c)*qg_snow(c) + (1.D0 - frac_sno_eff(c) - frac_h2osfc(c))*qg_soil(c) &
       ...
       htvp(c) = hvap
       if (h2osoi_liq(c,snl(c)+1) <= 0.D0 .and. h2osoi_ice(c,snl(c)+1) > 0.D0) htvp(c) = hsub
    ...
    do fp = 1,num_nolakep
       ...
       eflx_sh_veg(p) = 0.D0
       qflx_evap_tot(p) = 0.D0
       qflx_evap_veg(p) = 0.D0
       ...
       avmuir = 1.D0
       emv(p) = 1.D0-exp(-(elai(p)+esai(p))/avmuir)
    ...
    do fp = 1,num_nolakep
       p = filter_nolakep(fp)
       c = pcolumn(p)
       thm(p)  = forc_t(c) + 0.0098D0*forc_hgt_t_pft(p)
    end do
