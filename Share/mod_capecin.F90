...
module mod_capecin
  ...
  pure subroutine getcape_new(nk,p,t,rh,cape,cin,td,pi,q,th,thv,z)
    ...
    do k = 1, nk
      pi(k) = (p(k)*rp00)**rddcp
      td(k) = getdewp_new(t(k)-tzero,rh(k))
      q(k) = getqvs_new(p(k),td(k))
      th(k) = t(k)/pi(k)
    ...
    select case (source)
      ...
      case (2)
        ...
        if ( p(1) < 50000.0_rkx ) then
          ...
          maxthe = getthe_new(p(1),t(1),td(1),q(1))
        else
          ! find max thetae below 500 mb
          maxthe = d_zero
          do k = 1, nk
            if ( p(k) >= 50000.0_rkx ) then
              the = getthe_new(p(k),t(k),td(k),q(k))
              if ( the > maxthe ) then
                maxthe = the
                kmax = k
              end if
    ...
    do while ( doit .and. (k < nk) )
      ...
      do n = 1, nloop
        ...
        do while ( not_converged .and. i < 100 )
          ...
          qv2 = min(qt, fliq*getqvs_new(p2,t2) + fice*getqvi_new(p2,t2) )
