...
! !USES:
    ...
    use mod_clm_varpar   , only: nlevdecomp, nlevgrnd, nlevdecomp_full, maxpatch_pft
    ...
    real(rk8) :: rootfr_tot
    real(rk8) :: cinput_rootfr(lbp:ubp, 1:nlevdecomp_full)      ! pft-native root fraction used for calculating inputs
    ...
    do fp = 1,num_soilp
       ...
       if ( (altmax_lastyear_indx(c) .gt. 0) .and. (rootfr_tot .gt. 0.D0) .and. (surface_prof_tot .gt. 0.D0) ) then
    ...
    do fc = 1,num_soilc
       ...
       if ( ( abs(ndep_prof_sum - 1.D0) .gt. delta ) .or.  ( abs(nfixation_prof_sum - 1.D0) .gt. delta ) ) then
          ...
          call fatal(__FILE__,__LINE__, &
            trim(subname)//" ERROR: _prof_sum-1>delta" )
       endif
    end do
    ...
    do fp = 1,num_soilp
       ...
       if ( ( abs(froot_prof_sum - 1.D0) .gt. delta ) .or.  ( abs(croot_prof_sum - 1.D0) .gt. delta ) .or. &
            ( abs(stem_prof_sum - 1.D0) .gt. delta ) .or.  ( abs(leaf_prof_sum - 1.D0) .gt. delta ) ) then
          write(stderr, *) 'profile sums: ', froot_prof_sum, croot_prof_sum, leaf_prof_sum, stem_prof_sum
          call fatal(__FILE__,__LINE__, &
            trim(subname)//' ERROR: sum-1 > delta' )
       endif
    end do
