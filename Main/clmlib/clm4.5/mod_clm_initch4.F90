module mod_clm_initch4
      ...
      if ( ltype(l) == istsoil .or. ltype(l) == istcrop ) then
        do j = 1 , nlevsoi
          if ( conc_ch4_sat(c,j) == spval .or. arbinit )   &
            conc_ch4_sat(c,j)   = 0.D0
          if ( conc_ch4_unsat(c,j) == spval .or. arbinit ) &
            conc_ch4_unsat(c,j) = 0.D0
          if ( conc_o2_sat(c,j) == spval .or. arbinit )    &
            conc_o2_sat(c,j)    = 0.D0
          if ( conc_o2_unsat(c,j) == spval .or. arbinit )  &
            conc_o2_unsat(c,j)  = 0.D0
          if ( o2stress_sat(c,j) == spval .or. arbinit )   &
            o2stress_sat(c,j) = 1.D0
          if ( o2stress_unsat(c,j) == spval .or. arbinit ) &
            o2stress_unsat(c,j) = 1.D0
          if ( layer_sat_lag(c,j) == spval .or. arbinit )  &
            layer_sat_lag(c,j) = 1.D0
        end do
        if ( qflx_surf_lag(c) == spval .or. arbinit ) qflx_surf_lag(c) = 0.D0
        if ( finundated_lag(c) == spval .or. arbinit ) finundated_lag(c) = 0.D0
        ...
        do j = 1 , nlevsoi
          ...
          if ( lake_soilc(c,j) == spval .or. arbinit )   &
            lake_soilc(c,j) = 580.D0*cellorg(c,j)
  ...
  subroutine initTimeConst_ch4
    ...
    do c = begc , endc
      ...
      ch4_prod_depth_sat(c,nlevsoi+1:nlevgrnd) = 0.D0
      ch4_prod_depth_unsat(c,nlevsoi+1:nlevgrnd) = 0.D0
      ch4_prod_depth_lake(c,nlevsoi+1:nlevgrnd) = 0.D0
      ch4_oxid_depth_sat(c,nlevsoi+1:nlevgrnd) = 0.D0
      ch4_oxid_depth_unsat(c,nlevsoi+1:nlevgrnd) = 0.D0
      ch4_oxid_depth_lake(c,nlevsoi+1:nlevgrnd) = 0.D0
      o2_oxid_depth_sat(c,nlevsoi+1:nlevgrnd) = 0.D0
      o2_oxid_depth_unsat(c,nlevsoi+1:nlevgrnd) = 0.D0
      o2_decomp_depth_sat(c,nlevsoi+1:nlevgrnd) = 0.D0
      o2_decomp_depth_unsat(c,nlevsoi+1:nlevgrnd) = 0.D0
      o2_aere_depth_sat(c,nlevsoi+1:nlevgrnd) = 0.D0
      o2_aere_depth_unsat(c,nlevsoi+1:nlevgrnd) = 0.D0
      co2_decomp_depth_sat(c,nlevsoi+1:nlevgrnd) = 0.D0
      co2_decomp_depth_unsat(c,nlevsoi+1:nlevgrnd) = 0.D0
      co2_oxid_depth_sat(c,nlevsoi+1:nlevgrnd) = 0.D0
      co2_oxid_depth_unsat(c,nlevsoi+1:nlevgrnd) = 0.D0
      ch4_aere_depth_sat(c,nlevsoi+1:nlevgrnd) = 0.D0
      ch4_aere_depth_unsat(c,nlevsoi+1:nlevgrnd) = 0.D0
      ch4_tran_depth_sat(c,nlevsoi+1:nlevgrnd) = 0.D0
      ch4_tran_depth_unsat(c,nlevsoi+1:nlevgrnd) = 0.D0
      co2_aere_depth_sat(c,nlevsoi+1:nlevgrnd) = 0.D0
      co2_aere_depth_unsat(c,nlevsoi+1:nlevgrnd) = 0.D0
      ch4_ebul_depth_sat(c,nlevsoi+1:nlevgrnd) = 0.D0
      ch4_ebul_depth_unsat(c,nlevsoi+1:nlevgrnd) = 0.D0
      conc_ch4_lake(c,nlevsoi+1:nlevgrnd) = 0.D0
      conc_o2_lake(c,nlevsoi+1:nlevgrnd) = 0.D0
      rootfr(c,nlevsoi+1:nlevgrnd) = 0.D0
      ch4stress_unsat(c,nlevsoi+1:nlevgrnd) = 0.D0
      ch4stress_sat(c,nlevsoi+1:nlevgrnd) = 0.D0
      fphr(c,nlevdecomp+1:nlevgrnd) = 0.D0
