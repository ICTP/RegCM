...
module mod_micro_wsm5
  ...
  real(rkx) , parameter :: peaut = .55_rkx
  ...
  ! a constant for terminal velocity of snow
  real(rkx) , parameter :: bvts = .41_rkx
  ...
  real(rkx) , parameter :: alpha = .12_rkx
...
#include <pfesat.inc>
  ...
  subroutine init_wsm5
    ...
    pacrs = mathpi*n0s*avts*g3pbs*.25
    ...
    pure real(rkx) function rgmma(x)
      ...
      real(rkx) , parameter :: euler = 0.577215664901532_rkx
...
!    end do
  ...
  subroutine wsm52d(t,q,qci,qrs,den,p,delz,delt,rain,snow,fall,ims,ime)
    ...
    real(rkx) , parameter :: xbi = xai + xls0/(rwat*ttp)
    !
    nval = ime-ims+1
    !
    ...
    do loop = 1 , loops
      ...
      do k = 1 , kz
        do i = ims , ime
          ...
          xni(i,k) = min(max(5.38e7_rkx*temp,1.e3_rkx),1.e6_rkx)
        end do
      end do
      !
      ...
      do k = 1 , kz
        do i = ims , ime
          ...
          !        (c->r)
          !
          if ( qrs(i,k,1) > qcrmin .and. qci(i,k,1) > minqq ) then
      ...
      do k = 1 , kz
        do i = ims , ime
          ...
          if ( supcol > d_zero ) then
            if ( qrs(i,k,2) > qcrmin .and. qci(i,k,2) > minqq ) then
              xmi = den(i,k)*qci(i,k,2)/xni(i,k)
          ...
          !
          if ( qrs(i,k,2) > qcrmin .and. qci(i,k,1) > minqq ) then
          ...
          if ( supcol > d_zero ) then
            ...
            if ( supsat > d_zero .and. ifsat /= 1 ) then
              ...
              xni0 = 1.e3_rkx*exp(0.1_rkx*supcol)
    ...
    pure real(rkx) function cpmcal(x)
      implicit none
      real(rkx) , intent(in) :: x
      cpmcal = cpd*(d_one-max(x,minqq)) + cpv*max(x,minqq)
    end function cpmcal
    ...
    pure real(rkx) function fpvs(t,ice)
      implicit none
