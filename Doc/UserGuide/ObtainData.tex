...
\item {\em HA\_XX} - HadGEM CMPI5 dataset with XX in RF,26,45,85
\item {\em CA\_XX} - CanESM CMPI5 dataset with XX in RF,26,45,85
\end{enumerate}
...
\footnote{As of RegCM 4.2 this input source is not tested}
...
\item {\em ECEXY} - ECMWF Ensemble forecast member, where $X$ is the model
version number, and $Y$ is the ensemble member number, i.e. for example ECE24
stands for ECMWF model version 2, ensemble number 4. Dataset contains 6 hourly
global data on model levels with surface geopotential and natural logarithm
of surface pressure.
