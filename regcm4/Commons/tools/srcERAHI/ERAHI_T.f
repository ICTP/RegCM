...
C
      real*4  t
      common /var_t/ t(NLONS,NLATS,NLEVS)
      integer nrec
      common /ncount/nrec
C
...
c
 ...
 10      format('../DATA/ERAHI/',I4,'/',A2,'/tmp.',I10,'.grib')
 ...
 20      format('EHI_',I10)
     ...
     &          ,access='direct')
         ...
         do k=1,NLEVS
            nrec=nrec+1
            write(39,rec=nrec) ((t(i,j,k),i=1,NLONS),j=1,NLATS)
         enddo
         close(39)
         write(*,*) 'Temperature is added for DATE: ',IDATE
      ENDDO
      stop
      end
C
...
c     reduced N80 Gaussian grid:
     ...
     &               iarrayt,  words_read_into_rn80, rarrayrn80  )
      call check_error_status('intf', ierror)
      call get_field_data(words_per_mb,iarrayt,rarrayrn80)
      k=0
      do j=NLATS,1,-1
      do i=1,NLONS
         k=k+1
         t(i,j,lev)=rarrayrn80(k)
      enddo
      enddo
