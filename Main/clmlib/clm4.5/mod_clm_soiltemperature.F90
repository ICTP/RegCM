...
! !ARGUMENTS:
    ...
    real(rk8), intent(out) :: c_h2osfc(lbc:ubc)           !heat capacity of surface water
!
! !CALLED FROM:
...
!
    ...
    real(rk8), pointer :: dlrad(:)           ! downward longwave radiation blow the canopy [W/m2]
    ...
    real(rk8), pointer :: eflx_fgr12(:)      ! heat flux between soil layer 1 and 2 (W/m2)
...
!
    real(rk8), pointer :: eflx_gnet(:)          ! net ground heat flux into the surface (W/m**2)
    real(rk8), pointer :: dgnetdT(:)            ! temperature derivative of ground net heat flux
...
!
    ...
    real(rk8) :: rt (lbc:ubc,-nlevsno+1:nlevgrnd)   ! "r" vector for tridiagonal solution
    real(rk8) :: cv (lbc:ubc,-nlevsno+1:nlevgrnd)   ! heat capacity [J/(m2 K)]
    real(rk8) :: tk (lbc:ubc,-nlevsno+1:nlevgrnd)   ! thermal conductivity [W/(m K)]
    real(rk8) :: fn (lbc:ubc,-nlevsno+1:nlevgrnd)   ! heat diffusion through the layer interface [W/m2]
    real(rk8) :: fn1(lbc:ubc,-nlevsno+1:nlevgrnd)   ! heat diffusion through the layer interface [W/m2]
    ...
    real(rk8) :: dlwrad_emit(lbc:ubc)               ! time derivative of emitted longwave radiation
    integer  :: lyr_top                            ! index of top layer of snowpack (-4 to 0) [idx]
    real(rk8) :: sabg_lyr_col(lbc:ubc,-nlevsno+1:1) ! absorbed solar radiation (col,lyr) [W/m2]
    real(rk8) :: eflx_gnet_top                      ! net energy flux into surface layer, pft-level [W/m2]
    real(rk8) :: hs_top(lbc:ubc)                    ! net energy flux into surface layer (col) [W/m2]
          ...
          if ( pi <= npfts(c) ) then
             ...
             if (pactive(p)) then
                if (ltype(l) /= isturb) then
                   ...
                   sabg_chk(p) = frac_sno_eff(c) * sabg_snow(p) + (1.D0 - frac_sno_eff(c) ) * sabg_soil(p)
                   ...
                   if (ctype(c) == icol_road_perv .or. ctype(c) == icol_road_imperv) then
                      ...
                      eflx_heat_from_ac_pft(p) = eflx_heat_from_ac(l)/(1.D0-wtlunit_roof(l))
          ...
          if ((ctype(c) == icol_sunwall .or. ctype(c) == icol_shadewall &
             ...
             if (j >= snl(c)+1) then
                if (j == snl(c)+1) then
                   dzp     = z(c,j+1)-z(c,j)
                   at(c,j) = 0.D0
                   bt(c,j) = 1+(1.D0-cnfac)*fact(c,j)*tk(c,j)/dzp-fact(c,j)*dhsdT(c)
          ...
          if (j >= 1) then    
             ...
             if ((ctype(c) == icol_sunwall .OR. ctype(c) == icol_shadewall) .and. j <= nlevurb) then
                ...
                if (satw > .1D-6) then
                   ...
                   dksat = tkmg(c,j)*tkwat**(fl*watsat(c,j))*tkice**((1.D0-fl)*watsat(c,j))
...
!             tinc(c,j) = t_soisno(c,j) - tfrz 
          ...
          if (ltype(l) == istsoil .or. ltype(l) == istcrop .or. ctype(c) == icol_road_perv) then
             if(t_soisno(c,j) < tfrz) then
                smp = hfus*(tfrz-t_soisno(c,j))/(grav*t_soisno(c,j)) * 1000.D0  !(mm)
          ...
          .and. ctype(c) /= icol_roof) .or. ( j <= nlevurb)) then
             ...
             if (j >= snl(c)+1) then
                ...
                if (imelt(c,j) > 0 .and. abs(hm(c,j)) > 0.D0) then
                   ...
                   if (j == 1) then
                      if (snl(c)+1 == 1 .AND. h2osno(c) > 0.D0 .AND. xm(c,j) > 0.D0) then
                   ...
                   if (abs(heatr) > 0.D0) then
                      ...
                      if (j <= 0) then    ! snow
                         if (h2osoi_liq(c,j)*h2osoi_ice(c,j)>0.D0) t_soisno(c,j) = tfrz
                      end if
