...
! !OTHER LOCAL VARIABLES:
   ...
   real(rk8), parameter :: yravg   = 20.0D0      ! length of years to average for gdd
...
! local pointers to implicit in/out scalars
   ...
   real(rk8), pointer :: prev_dayl(:)       ! daylength from previous albedo timestep (seconds)
   ...
   do fp = 1,num_soilp
      ...
      if (stress_decid(ivt(p)) == 1.D0) then
         ...
         if (dormant_flag(p) == 1.D0) then
            ...
            if (onset_gddflag(p) == 0.D0 .and. soilt < tfrz) onset_fdd(p) = onset_fdd(p) + fracday
         ...
         ! test for switching from growth period to offset period
            ...
            if (psi <= soilpsi_off) then
               ...
               if (offset_swi(p) >= crit_offset_swi .and. onset_flag(p) == 0.D0) offset_flag(p) = 1.D0
...
! local pointers to implicit in scalars
      ...
      real(rk8), pointer :: fertnitro(:)      ! max fertilizer to be applied in total (kgN/m2)
! local pointers to implicit out scalars
...
!           else ! not possible to have croplive and ivt==cornORsoy? (slevis)
         ...
         if ( (.not. croplive(p)) .and. (.not. cropplant(p)) ) then
            ...
            if (ivt(p) == nwcereal .or. ivt(p) == nwcerealirrig) then
               ...
               ! If hit the max planting julian day -- go ahead and plant
                  ...
                  if (ivt(p)==nsoybean .or. ivt(p) == nsoybeanirrig) gddmaturity(p)=min(gdd1020(p),hybgdd(ivt(p)))
                  if (ivt(p)==ncorn .or. ivt(p)==ncornirrig) gddmaturity(p)=max(950.D0, min(gdd820(p)*0.85D0, hybgdd(ivt(p))))
                  if (ivt(p)==nscereal .or. ivt(p) == nscerealirrig) gddmaturity(p)=min(gdd020(p),hybgdd(ivt(p)))
            ...
            if (ivt(p) == ncorn .or. ivt(p)==ncornirrig) then
               ...
               huigrain(p) = min(max(huigrain(p), grnfill(ivt(p))-0.1D0), grnfill(ivt(p)))
         ...
         if (croplive(p)) then
            ...
            if (t_ref2m_min(p) < 1.D30 .and. vf(p) /= 1.D0 .and. (ivt(p) == nwcereal .or. ivt(p) == nwcerealirrig)) then
...
!------------------------------------------------------------------------
        ...
        if (t_ref2m_min(p) <= tfrz-3.D0 .or. hdidx(p) /= 0.D0) then
           ...
           if (t_ref2m_max(p) >= tbase + tfrz + 10.D0) then
              ...
              if (hdidx(p) > hti) hdidx(p) = hdidx(p) - 0.02D0 * (t_ref2m_max(p)-tbase-tfrz-10.D0)
...
!
   ...
   integer , pointer :: ivt(:)          ! pft vegetation type
   real(rk8), pointer :: wtcol(:)        ! weight (relative to column) for this pft (0-1)
   real(rk8), pointer :: leafc_to_litter(:)     ! leaf C litterfall (gC/m2/s)
   real(rk8), pointer :: frootc_to_litter(:)    ! fine root N litterfall (gN/m2/s)
   real(rk8), pointer :: livestemc_to_litter(:) ! live stem C litterfall (gC/m2/s)
   real(rk8), pointer :: grainc_to_food(:)      ! grain C to food (gC/m2/s)
   real(rk8), pointer :: livestemn_to_litter(:) ! livestem N to litter (gN/m2/s)
   real(rk8), pointer :: grainn_to_food(:)      ! grain N to food (gN/m2/s)
