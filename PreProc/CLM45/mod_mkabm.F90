...
module mod_mkabm
  ...
  subroutine mkabm(abmfile,abm)
    implicit none
    character(len=*) , intent(in) :: abmfile
    real(rk8) , dimension(:,:) , intent(out) :: abm
    ...
    call checkncerr(istatus,__FILE__,__LINE__, &
      'Cannot find variable abm in file '//trim(inpfile))
    ...
    do i = 1 , domain%ntiles
      ...
      call checkncerr(istatus,__FILE__,__LINE__, &
        'Cannot read variable abm from file '//trim(inpfile))
    ...
    call bilinear(rvar,rmask,rlon,rlat,abm,xlon,xlat,vmin,vmisdat)
