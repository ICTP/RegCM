...
module mod_mkpeatf
  ...
  subroutine mkpeatf(peatffile,peatf)
    ...
    real(rk8) , dimension(:,:) , intent(out) :: peatf
    ...
    call checkncerr(istatus,__FILE__,__LINE__, &
      'Cannot find variable peatf in file '//trim(inpfile))
    ...
    do i = 1 , domain%ntiles
      ...
      call checkncerr(istatus,__FILE__,__LINE__, &
        'Cannot read variable peatf from file '//trim(inpfile))
    ...
    call bilinear(rvar,rmask,rlon,rlat,peatf,xlon,xlat,vmin,vmisdat)
