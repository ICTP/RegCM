module mod_clm_cncisoflux
    ...
    do fc = 1 , num_soilc
      ...
      do j = 1 , nlevdecomp
        do l = 1 , ndecomp_cascade_transitions
          if ( c%ccs%decomp_cpools_vr(cc,j,cascade_donor_pool(l)) /= 0.D0) then
            ccisof%decomp_cascade_hr_vr(cc,j,l) = &
                    c%ccf%decomp_cascade_hr_vr(cc,j,l) * &
                    (ccisos%decomp_cpools_vr(cc,j,cascade_donor_pool(l)) / &
                     c%ccs%decomp_cpools_vr(cc,j,cascade_donor_pool(l))) * 1.D0
          else
            ccisof%decomp_cascade_hr_vr(cc,j,l) = 0.D0
          end if
        end do
      end do
    end do
      ...
      do j = 1 , nlevdecomp
        do l = 1 , ndecomp_cascade_transitions
          if ( c%ccs%decomp_cpools_vr(cc,j,cascade_donor_pool(l)) /= 0.D0) then
            ...
            ccisof%decomp_cascade_ctransfer_vr(cc,j,l) = 0.D0
          end if
        end do
      end do
    end do
    ...
    do fc = 1 , num_soilc
      ...
      do j = 1 , nlevdecomp
        do l = 1 , ndecomp_pools
          if ( c%ccs%decomp_cpools_vr(cc,j,l) /= 0.D0) then
            ccisof%m_decomp_cpools_to_fire_vr(cc,j,l) = &
                    c%ccf%m_decomp_cpools_to_fire_vr(cc,j,l) * &
                    (ccisos%decomp_cpools_vr(cc,j,l) / &
                    c%ccs%decomp_cpools_vr(cc,j,l)) * 1.D0
          else
            ccisof%m_decomp_cpools_to_fire_vr(cc,j,l) = 0.D0
          end if
        end do
      end do
    end do
  ...
  subroutine CIsoFluxCalc(ciso_flux, ctot_flux, ciso_state, ctot_state, &
    ...
    real(rk8), pointer   :: ctot_state(:)     !INPUT  totC state, upstream pool
