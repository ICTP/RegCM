...
module mod_interp
  ...
  integer(ik4), pointer, contiguous, dimension(:,:) :: ic1dl, ic1dr
  integer(ik4), pointer, contiguous, dimension(:,:) :: ic1ul, ic1ur
  integer(ik4), pointer, contiguous, dimension(:,:) :: jc1dl, jc1dr
  integer(ik4), pointer, contiguous, dimension(:,:) :: jc1ul, jc1ur
  integer(ik4), pointer, contiguous, dimension(:,:) :: id1dl, id1dr
  integer(ik4), pointer, contiguous, dimension(:,:) :: id1ul, id1ur
  integer(ik4), pointer, contiguous, dimension(:,:) :: jd1dl, jd1dr
  integer(ik4), pointer, contiguous, dimension(:,:) :: jd1ul, jd1ur
  ...
  interface interp1d
    ...
    module procedure interp1d_r4_new
  ...
  pure subroutine interp1d_r4_new(xi,g,xo,f,alfa,ex1,ex2,zi,zg,sxi,sxo)
    !$acc routine seq
    implicit none
    integer(ik4), intent(in) :: sxi, sxo
    real(rk4), dimension(sxi), intent(in) :: xi, g
    real(rk4), dimension(sxo), intent(in) :: xo
    real(rk4), dimension(sxo), intent(out) :: f
    real(rk4), dimension(sxi), intent(out) :: zi,zg
    real(rk4), intent(in) :: alfa
    real(rk4), intent(in) :: ex1
    real(rk4), intent(in) :: ex2
  ...
  end subroutine interp1d_r4_new
  ...
  subroutine cressmcr3d(b3,b2,alon,alat,glon,glat,jx,iy,nlon,nlat,nlev,nf)
    ...
    real(rkx), dimension(nlon,nlat), intent(in) :: glat, glon
    real(rkx), dimension(nlon,nlat,nlev*nf), intent(in) :: b2
    integer(ik4) :: k, l, kin
    do l = 1, nf
      do k = 1, nlev
         kin = (l-1)*nlev+k
