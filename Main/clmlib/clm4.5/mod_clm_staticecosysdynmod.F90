module STATICEcosysdynMOD
  ...
  real(rk8) :: timwt(2)     ! time weights for month 1 and month 2
  ! lai for interpolation (2 months)
  real(rk8) , allocatable :: mlai2t(:,:)
  ! sai for interpolation (2 months)
  real(rk8) , allocatable :: msai2t(:,:)
  ! top vegetation height for interpolation (2 months)
  real(rk8) , allocatable :: mhvt2t(:,:)
  ! bottom vegetation height for interpolation(2 months)
  real(rk8) , allocatable :: mhvb2t(:,:)
...
!
    ...
    integer(ik4) , pointer :: frac_veg_nosno_alb(:) ! frac of vegetation not covered by snow [-]
!
!
...
!EOP
    ...
    integer(ik4) :: it(2)       ! month 1 and month 2 (step 1)
    integer(ik4) :: months(2)   ! months to be interpolated (1 to 12)
...
!-----------------------------------------------------------------------
  ...
  subroutine readAnnualVegetation ( )
    ...
    real(rk8) , pointer :: mlai(:,:)        ! lai read from input files
    integer(ik4) :: ier                        ! error code
    ...
    integer(ik4) :: closelatidx,closelonidx    ! single column vars
    real(rk8):: closelat,closelon          ! single column vars
...
! !USES:
    ...
    use pftvarcon   , only : noveg
    use fileutils   , only : getfil
    use spmdMod     , only : mpicom, MPI_REAL8, MPI_INTEGER
    use shr_scam_mod, only : shr_scam_getCloseLatLon
...
!EOP
    ...
    logical :: readvar
    real(rk8) , pointer :: mlai(:,:)        ! lai read from input files
    real(rk8) , pointer :: msai(:,:)        ! sai read from input files
    real(rk8) , pointer :: mhgtt(:,:)       ! top vegetation height
    real(rk8) , pointer :: mhgtb(:,:)       ! bottom vegetation height
    ...
    do k=1,2   !loop over months and read vegetated data
       ...
       if (.not. readvar) then
         call fatal(__FILE__,__LINE__, &
                 trim(subname)//' ERROR: MONTHLY_LAI NOT on fveg file' )
       end if
       ...
       if (.not. readvar) then
         call fatal(__FILE__,__LINE__, &
                 trim(subname)//' ERROR: MONTHLY_SAI NOT on fveg file' )

       call ncd_io(ncid=ncid, varname='MONTHLY_HEIGHT_TOP', flag='read', data=mhgtt, dim1name=grlnd, &
            nt=months(k), readvar=readvar)
       if (.not. readvar) then
         call fatal(__FILE__,__LINE__, &
                 trim(subname)//' ERROR: MONTHLY_HEIGHT_TOP NOT on fveg file' )
       end if
