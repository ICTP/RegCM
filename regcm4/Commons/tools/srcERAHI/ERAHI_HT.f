...
C
C  NLEVS  = NUMBER OF PRESSURE LEVELS IN ERA40 original DATASET.
C  NLATS  = NUMBER OF LATITUDES  ON ERA40 original GRID.
C  NLONS  = NUMBER OF LONGITUDES ON ERA40 original GRID.
C
      real*4  ht,lsm
      common /var_ht/ ht(NLONS,NLATS),lsm(NLONS,NLATS)
C
...
c
 ...
 10      format('../DATA/ERAHI/',I4,'/',A2,'/srf.',I10,'.grib')
 ...
 20      format('EHI_',I10)
     ...
     &          ,access='direct')
         ...
         write(39,rec=2) lsm
         close(39)
         write(*,*) 'Elevation is added for DATE: ',IDATE
      ENDDO
      stop
      end
C
...
C
      ...
      integer iarrayrn80(words_per_mb)

      integer iarrayht(words_per_mb)
      integer iarraylsm(words_per_mb)
      real*8  dave

      real rarrayrn80(words_per_mb)
      real rarrayn80r(words_per_mb)

      integer unitrn80

      integer ierror

      integer bytes_read_into_rn80

      integer words_read_into_n80r
...
c     Open input GRIB file containing spherical harmonic records:
      ...
      call pbopen( unitrn80, finame, 'r', ierror )
...
c     file:
      ...
      call pbgrib( unitrn80, iarrayrn80, bytes_per_mb,
     &             bytes_read_into_rn80, ierror )
...
c     reduced N80 Gaussian grid:
      ...
      words_read_into_n80r = words_per_mb

      ierror = intf( iarrayrn80,  words_per_mb,         rarrayrn80,
     &               iarrayht,  words_read_into_n80r, rarrayn80r  )
      call check_error_status('intf', ierror)
      call get_field_data(words_per_mb,iarrayht,rarrayn80r)
      k=0
      do j=NLATS,1,-1
      do i=1,NLONS
         k=k+1
         ht(i,j)=rarrayn80r(k)/9.80616
      enddo
      enddo
...
c     reduced N80 Gaussian grid:
      ...
      ierror = intf( iarrayrn80,  words_per_mb,         rarrayrn80,
     &               iarraylsm,  words_read_into_n80r, rarrayn80r  )
      call check_error_status('intf', ierror)
      call get_field_data(words_per_mb,iarraylsm,rarrayn80r)
      k=0
      do j=NLATS,1,-1
      do i=1,NLONS
         k=k+1
         lsm(i,j)=rarrayn80r(k)
      enddo
      enddo
