...
!
!   density at surface is stored in rhox2d(i,j), at half levels in rhohf(i,k,j).
!
    do k = 1 , kzm1
      do i = 2 , iym1
...
!   compute richardson number
    ...
    do k = kz , kmxpbl , -1
      do i = 2 , iym1
        vv = uatm(i,k,j)*uatm(i,k,j) + vatm(i,k,j)*vatm(i,k,j)
        ri(i,k) = egrav*(thvx(i,k,j)-th10(i,j))*za(i,k,j)/(th10(i,j)*vv)
      end do
    end do
