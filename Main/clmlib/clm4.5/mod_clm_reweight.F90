...
! !USES:
    use mod_clm_type
    use mod_clm_decomp , only : get_clump_bounds
!
! !ARGUMENTS:
    implicit none
    integer, intent(in) :: nc       ! clump index
...
!------------------------------------------------------------------------
    ...
    do c = begc,endc
       ...
       if (cptr%active(c) .and. .not. lptr%active(l)) then
          write(stderr,*) trim(subname),' ERROR: active column found on inactive landunit', &
                         'at c = ', c, ', l = ', l
          error_found = .true. 
       end if
    end do
    ...
    do p = begp,endp
       ...
       if (pptr%active(p) .and. .not. cptr%active(c)) then
          write(stderr,*) trim(subname),' ERROR: active pft found on inactive column', &
                         'at p = ', p, ', c = ', c
          error_found = .true. 
       end if
    end do
...
! !LOCAL VARIABLES:
    ...
    real(rk8), allocatable :: sumwtcol(:), sumwtlunit(:), sumwtgcell(:)
...
!------------------------------------------------------------------------------
    ...
    do c = begc,endc
       if (.not. weightsOkay(sumwtcol(c), active_only, cptr%active(c))) then
          write(stderr,*) trim(subname),' ERROR: at c = ',c,'total PFT weight is ',sumwtcol(c), &
    ...
    do l = begl,endl
       if (.not. weightsOkay(sumwtlunit(l), active_only, lptr%active(l))) then
          write(stderr,*) trim(subname),' ERROR: at l = ',l,'total PFT weight is ',sumwtlunit(l), &
                         'active_only = ', active_only
          error_found = .true.
       end if
    end do
    ...
    do g = begg,endg
       if (.not. weightsOkay(sumwtgcell(g), active_only, i_am_active=.true.)) then
          write(stderr,*) trim(subname),' ERROR: at g = ',g,'total PFT weight is ',sumwtgcell(g), &
                         'active_only = ', active_only
          error_found = .true.
       end if
    end do
    ...
    do l = begl,endl
       if (.not. weightsOkay(sumwtlunit(l), active_only, lptr%active(l))) then
          write(stderr,*) trim(subname),' ERROR: at l = ',l,'total col weight is ',sumwtlunit(l), &
                         'active_only = ', active_only
          error_found = .true.
       end if
    end do
    ...
    do g = begg,endg
       if (.not. weightsOkay(sumwtgcell(g), active_only, i_am_active=.true.)) then
          write(stderr,*) trim(subname),' ERROR: at g = ',g,'total col weight is ',sumwtgcell(g), &
                         'active_only = ', active_only
          error_found = .true.
       end if
    end do
    ...
    do g = begg,endg
       if (.not. weightsOkay(sumwtgcell(g), active_only, i_am_active=.true.)) then
          write(stderr,*) trim(subname),' ERROR: at g = ',g,'total lunit weight is ',sumwtgcell(g), &
                         'active_only = ', active_only
          error_found = .true.
       end if
    end do
...
! !ARGUMENTS:
    implicit none
    real(rk8), intent(in) :: sumwts              ! sum of weights of children, grandchildren or great-grandchildren
...
! LOCAL VARIABLES:
    ...
    real(rk8), parameter :: tolerance = 1.D-7  ! tolerance for checking whether weights sum to 1
!------------------------------------------------------------------------
