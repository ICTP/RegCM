...
!------------------------------------------------------------------
    ...
    subroutine rrtmg_lw &
             ...
             tauaer  , uflx    ,dflx    ,hr      ,uflxc   ,       &
             dflxc,  hrc, duflx_dt, duflxc_dt,                    &
    ...
    !     flags inflglw, iceflglw, and liqflglw; see text file
    !     rrtmg_lw_instructions and subroutine rrtmg_lw_cldprmc.f90 for
    !     further details):
    !
...
!     wx(4,nlayers) = wx(4,nlayers-1)
      ...
      ! At this point all molecular amounts in wkl and wx are in volume
      ! mixing ratio;
      ! convert to molec/cm2 based on coldry for use in rrtm.
      ! also, compute precipitable water vapor for diffusivity angle
      ! adjustments in rtrn and rtrnmr.
...
!       semiss(n) = 1.0_rb
      ...
      if ( icld .ge. 1 ) then
        ...
        ! If an extra layer is being used in RRTMG, set all cloud
        ! properties to zero in the extra layer.
