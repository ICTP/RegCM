...
module mod_humid
  ...
  subroutine humid1(im,jm,km,t,q,ps,sigma,ptop,p,rh)
    ...
    real(4) :: lh , qs , pt , sigp , satvp , rpt , wp , w1
    ...
    do j = 1 , jm
      do i = 1 , im
        do k = 1 , km
          pt = sigma(k)*(ps(j,i)-rpt) + rpt
          lh = slh0 - slh1*(t(k,j,i)-t0)       ! LATENT HEAT OF EVAP.
          satvp = slsvp1*exp(slsvp2*lh*(rt0-1.0/t(k,j,i)))
          qs = sep2*satvp/(pt-satvp)           ! SAT. MIXING RATIO
          rhs(k) = max(q(k,j,i)/qs,0.0)*100.0
        end do
        ! Now vertical interpolation on pressure level!
        sigp = (p-rpt)/(ps(j,i)-rpt)
        !
  ...
  subroutine humid2(im,jm,tas,qas,ps,rh)
    ...
    real(4) :: mx2s , lh , satvp , qs
    integer :: i , j
    do j = 1 , jm
      do i = 1 , im
        mx2s = 1.0/(1.0-qas(j,i))
        lh = slh0-slh1*(tas(j,i)-t0)
        satvp = slsvp1*exp(slsvp2*lh*(1.0/t0-1.0/tas(j,i)))
        qs = sep2*satvp/(ps(j,i)-satvp)
        rh(j,i) = (qas(j,i)/qs)*100.0
      end do
    end do
