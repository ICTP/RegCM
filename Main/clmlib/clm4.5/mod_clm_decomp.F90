...
!
     ...
     if ( OMP_GET_NUM_THREADS() == 1 .and. OMP_GET_MAX_THREADS() > 1 )then
        ...
        trim(subname)//' ERROR: Calling from inside a non-threaded '// &
          'region -- this results in bad performance' )
     end if
#endif
...
!
     ...
     if ( OMP_GET_NUM_THREADS() > 1 )then
        call fatal(__FILE__,__LINE__, &
         trim(subname)//' ERROR: Calling from inside a threaded region '//&
           '-- this is illegal' )
     end if
#endif
...
!-----------------------------------------------------------------------
    ...
    case default
       write(stderr,*) 'get_clmlevel_gsize does not match clmlevel type: ', trim(clmlevel)
...
!    case default
!       write(stderr,*) 'get_clmlevel_gsmap: Invalid expansion character: ',trim(clmlevel)
!       call fatal(__FILE__,__LINE__,'clm will now stop')
!    end select
!
