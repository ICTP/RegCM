module mod_clm_cnnitrifdenitrif
  ...
  subroutine nitrif_denitrif(lbc, ubc, num_soilc, filter_soilc)
    ...
    real(rk8) :: surface_tension_water = 73.D-3
...
! #endif
    ...
    do j = 1 , nlevdecomp
      do fc = 1 , num_soilc
        ...
        r_min(c,j) = 2.0D0 * surface_tension_water / &
               (rho_w * grav * abs(soilpsi(c,j)))
        r_max = 2.0D0 * surface_tension_water / (rho_w * grav * 0.1D0)
        r_psi(c,j) = sqrt(r_min(c,j) * r_max)
        ratio_diffusivity_water_gas(c,j) = (d_con_g(2,1) + &
               d_con_g(2,2)*t_soisno(c,j) ) * 1.D-4 / &
        ...
        if (anoxia_wtsat) then
         ! Average saturated fraction values into anaerobic_frac(c,j).
          ...
          anaerobic_frac(c,j) = (1.D0 - finundated(c))*anaerobic_frac(c,j) + &
        ...
        smin_no3_massdens_vr(c,j) = max(smin_no3_vr(c,j), 0.D0) * &
        ...
        fmax_denit_carbonsubstrate_vr(c,j) = (0.1D0 * &
               (soil_co2_prod(c,j)**1.3D0)) &
              / g_per_m3_sec__to__ug_per_gsoil_day
        !
        fmax_denit_nitrate_vr(c,j) = (1.15D0 * &
               smin_no3_massdens_vr(c,j)**0.57D0)  &
              / g_per_m3_sec__to__ug_per_gsoil_day

        ! find limiting denitrification rate
        f_denit_base_vr(c,j) = max(min(fmax_denit_carbonsubstrate_vr(c,j), &
               fmax_denit_nitrate_vr(c,j)),0.D0)
        ...
        wfps_vr(c,j) = max(min(h2osoi_vol(c,j)/watsat(c, j), 1.D0), 0.D0) * &
               100.D0
        fr_WFPS(c,j) = max(0.1D0, 0.015D0 * wfps_vr(c,j) - 0.32D0)
#ifdef LCH4
        if (anoxia_wtsat) then
          fr_WFPS(c,j) = fr_WFPS(c,j)*(1.D0 - finundated(c)) + &
                 finundated(c)*1.18D0
        end if
#endif
