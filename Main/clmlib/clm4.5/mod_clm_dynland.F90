...
! !USES:
   ...
   use mod_clm_varcon, only : istsoil,istice,istwet,istdlak,istslak,isturb,istice_mec
   use mod_clm_varcon, only : istcrop
   use mod_clm_varcon, only : icol_road_perv,icol_road_imperv,icol_roof
   use mod_clm_varcon, only : icol_sunwall,icol_shadewall
   use mod_clm_varcon, only : cpice,  cpliq
...
!-------------------------------------------------------------------------------
   ...
   do g = begg,endg ! loop over grid cells
      ...
      gcell_liq  (g) = 0.0D0   ! sum for one grid cell
      gcell_ice  (g) = 0.0D0   ! sum for one grid cell
      gcell_heat (g) = 0.0D0   ! sum for one grid cell
            ...
            ! in new lake code, all lakes are "istdlak", but have variable depth
               ...
               do k = 1,nlevlak
                  if (ltype(l) == istdlak) then
                     ...
                     heat = heat + cv*cptr%ces%t_lake(c,k) / 1.D6
                  end if
               end do
               ...
               else if ( h2osno(c) > 0.0D0 .and. ltype(l) /= istdlak) then
