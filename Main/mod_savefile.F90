...
module mod_savefile
  ...
  real(rk8) , public , pointer , dimension(:,:) :: solvd_io
...
#ifdef CLM
  real(rk8) , public , pointer , dimension(:,:) :: sols2d_io
  ...
  subroutine read_savefile(idate)
    ...
    if ( myid == iocpu ) then
      ...
      if ( idcsst == 1 ) then
        ncstatus = nf90_get_var(ncid,get_varid(ncid,'dtskin'),dtskin_io)
        call check_ok(__FILE__,__LINE__,'Cannot read dtskin')
      ...
      ncstatus = nf90_get_var(ncid,get_varid(ncid,'solvd'),solvd_io)
      call check_ok(__FILE__,__LINE__,'Cannot read solvd')
...
#ifdef CLM
      ncstatus = nf90_get_var(ncid,get_varid(ncid,'sols2d'),sols2d_io)
      call check_ok(__FILE__,__LINE__,'Cannot read sols2d')
      ncstatus = nf90_get_var(ncid,get_varid(ncid,'soll2d'),soll2d_io)
      call check_ok(__FILE__,__LINE__,'Cannot read soll2d')
      ncstatus = nf90_get_var(ncid,get_varid(ncid,'solsd2d'),solsd2d_io)
  ...
  subroutine write_savefile(idate,ltmp)
    ...
    if ( myid == iocpu ) then
      ...
      ncstatus = nf90_def_var(ncid,'tlef',nf90_double,wrkdim(1:3),varids(39))
      call check_ok(__FILE__,__LINE__,'Cannot create var tlef')
      ncstatus = nf90_def_var(ncid,'ssw',nf90_double,wrkdim(1:3),varids(40))
      call check_ok(__FILE__,__LINE__,'Cannot create var ssw')
      ncstatus = nf90_def_var(ncid,'rsw',nf90_double,wrkdim(1:3),varids(41))
      call check_ok(__FILE__,__LINE__,'Cannot create var rsw')
      ncstatus = nf90_def_var(ncid,'tsw',nf90_double,wrkdim(1:3),varids(42))
      call check_ok(__FILE__,__LINE__,'Cannot create var tsw')
      ncstatus = nf90_def_var(ncid,'tgrd',nf90_double,wrkdim(1:3),varids(43))
      call check_ok(__FILE__,__LINE__,'Cannot create var tgrd')
      ncstatus = nf90_def_var(ncid,'tgbrd',nf90_double,wrkdim(1:3),varids(44))
      call check_ok(__FILE__,__LINE__,'Cannot create var tgbrd')
      ncstatus = nf90_def_var(ncid,'sncv',nf90_double,wrkdim(1:3),varids(45))
      call check_ok(__FILE__,__LINE__,'Cannot create var sncv')
      ncstatus = nf90_def_var(ncid,'gwet',nf90_double,wrkdim(1:3),varids(46))
      call check_ok(__FILE__,__LINE__,'Cannot create var gwet')
      ncstatus = nf90_def_var(ncid,'snag',nf90_double,wrkdim(1:3),varids(47))
      call check_ok(__FILE__,__LINE__,'Cannot create var snag')
      ncstatus = nf90_def_var(ncid,'sfice',nf90_double,wrkdim(1:3),varids(48))
      call check_ok(__FILE__,__LINE__,'Cannot create var sfice')
      ncstatus = nf90_def_var(ncid,'ldew',nf90_double,wrkdim(1:3),varids(49))
      call check_ok(__FILE__,__LINE__,'Cannot create var ldmskldew')
      ncstatus = nf90_def_var(ncid,'ldmsk1',nf90_int,wrkdim(1:3),varids(50))
      call check_ok(__FILE__,__LINE__,'Cannot create var ldmsk1')
      ncstatus = nf90_def_var(ncid,'taf',nf90_double,wrkdim(1:3),varids(51))
      ...
      if ( lakemod == 1 ) then
        ...
        ncstatus = nf90_def_var(ncid,'hi',nf90_double,wrkdim(1:3),varids(54))
      ...
      wrkdim(2) = dimids(idicross)
      wrkdim(3) = dimids(idkh)
      ncstatus = nf90_def_var(ncid,'heatrt',nf90_double,wrkdim(1:3),varids(58))
      call check_ok(__FILE__,__LINE__,'Cannot create var heatrt')
      wrkdim(3) = dimids(idkf)
      ncstatus = nf90_def_var(ncid,'o3prof',nf90_double,wrkdim(1:3),varids(59))
      call check_ok(__FILE__,__LINE__,'Cannot create var o3prof')
      ncstatus = nf90_def_var(ncid,'flw',nf90_double,wrkdim(1:2),varids(60))
      call check_ok(__FILE__,__LINE__,'Cannot create var flw')
      ncstatus = nf90_def_var(ncid,'flwd',nf90_double,wrkdim(1:2),varids(61))
      call check_ok(__FILE__,__LINE__,'Cannot create var flwd')
      ncstatus = nf90_def_var(ncid,'fsw',nf90_double,wrkdim(1:2),varids(62))
      call check_ok(__FILE__,__LINE__,'Cannot create var fsw')
      ncstatus = nf90_def_var(ncid,'sinc',nf90_double,wrkdim(1:2),varids(63))
      call check_ok(__FILE__,__LINE__,'Cannot create var sinc')
      ncstatus = nf90_def_var(ncid,'ldmsk',nf90_int,wrkdim(1:2),varids(64))
      call check_ok(__FILE__,__LINE__,'Cannot create var ldmsk')
      if ( iocnflx == 2 ) then
        ncstatus = nf90_def_var(ncid,'zpbl',nf90_double,wrkdim(1:2),varids(65))
        call check_ok(__FILE__,__LINE__,'Cannot create var zpbl')
      end if
      if ( ichem == 1 ) then
        wrkdim(3) = dimids(idkh)
        wrkdim(4) = dimids(idntr)
        ncstatus = nf90_def_var(ncid,'chia',nf90_double,wrkdim(1:4),varids(66))
        call check_ok(__FILE__,__LINE__,'Cannot create var chia')
        ncstatus = nf90_def_var(ncid,'chib',nf90_double,wrkdim(1:4),varids(67))
      ...
      ncstatus = nf90_def_var(ncid,'dstor',nf90_double,wrkdim(1:3),varids(80))
      call check_ok(__FILE__,__LINE__,'Cannot create var dstor')
      ncstatus = nf90_def_var(ncid,'hstor',nf90_double,wrkdim(1:3),varids(81))
      ...
      if ( islab_ocean == 1 .and. do_restore_sst ) then
        ...
        ncstatus = nf90_def_var(ncid,'stepcount',nf90_double, &
                                wrkdim(1:1),varids(83))
        call check_ok(__FILE__,__LINE__,'Cannot create var stepcount')
      end if
#ifdef CLM
      wrkdim(1) = dimids(idjcross)
      wrkdim(2) = dimids(idicross)
      ncstatus = nf90_def_var(ncid,'sols2d',nf90_double,wrkdim(1:2),varids(84))
      call check_ok(__FILE__,__LINE__,'Cannot create var sols2d')
      ncstatus = nf90_def_var(ncid,'soll2d',nf90_double,wrkdim(1:2),varids(85))
      call check_ok(__FILE__,__LINE__,'Cannot create var soll2d')
      ncstatus = nf90_def_var(ncid,'solsd2d',nf90_double,wrkdim(1:2),varids(86))
      ...
      if ( idcsst == 1 ) then
        ncstatus = nf90_put_var(ncid,varids(27),dtskin_io)
        call check_ok(__FILE__,__LINE__,'Cannot write dtskin')
        ncstatus = nf90_put_var(ncid,varids(28),deltas_io)
        call check_ok(__FILE__,__LINE__,'Cannot write deltas')
        ncstatus = nf90_put_var(ncid,varids(29),tdeltas_io)
        call check_ok(__FILE__,__LINE__,'Cannot write tdeltas')
      end if
      if ( irrtm == 0 ) then
        ncstatus = nf90_put_var(ncid,varids(30),gasabsnxt_io)
        call check_ok(__FILE__,__LINE__,'Cannot write gasabsnxt')
        ncstatus = nf90_put_var(ncid,varids(31),gasabstot_io)
        call check_ok(__FILE__,__LINE__,'Cannot write gasabstot')
        ncstatus = nf90_put_var(ncid,varids(32),gasemstot_io)
        call check_ok(__FILE__,__LINE__,'Cannot write gasemstot')
      end if
      if ( ipptls > 0 ) then
        ncstatus = nf90_put_var(ncid,varids(33),fcc_io)
        call check_ok(__FILE__,__LINE__,'Cannot write fcc')
        if ( ipptls == 2 ) then
          ncstatus = nf90_put_var(ncid,varids(34),snownc_io)
          call check_ok(__FILE__,__LINE__,'Cannot write snownc')
        end if
      end if
      ncstatus = nf90_put_var(ncid,varids(35),solis_io)
      call check_ok(__FILE__,__LINE__,'Cannot write solis')
      ncstatus = nf90_put_var(ncid,varids(36),solvd_io)
      call check_ok(__FILE__,__LINE__,'Cannot write solvd')
      ncstatus = nf90_put_var(ncid,varids(37),solvs_io)
      call check_ok(__FILE__,__LINE__,'Cannot write solvs')
      ncstatus = nf90_put_var(ncid,varids(38),sabveg_io)
      call check_ok(__FILE__,__LINE__,'Cannot write sabveg')
      ncstatus = nf90_put_var(ncid,varids(39),tlef_io)
      call check_ok(__FILE__,__LINE__,'Cannot write tlef')
      ncstatus = nf90_put_var(ncid,varids(40),ssw_io)
      call check_ok(__FILE__,__LINE__,'Cannot write ssw')
      ncstatus = nf90_put_var(ncid,varids(41),rsw_io)
      call check_ok(__FILE__,__LINE__,'Cannot write rsw')
      ncstatus = nf90_put_var(ncid,varids(42),tsw_io)
      call check_ok(__FILE__,__LINE__,'Cannot write tsw')
      ncstatus = nf90_put_var(ncid,varids(43),tgrd_io)
      call check_ok(__FILE__,__LINE__,'Cannot write tgrd')
      ncstatus = nf90_put_var(ncid,varids(44),tgbrd_io)
      call check_ok(__FILE__,__LINE__,'Cannot write tgbrd')
      ncstatus = nf90_put_var(ncid,varids(45),sncv_io)
      call check_ok(__FILE__,__LINE__,'Cannot write sncv')
      ncstatus = nf90_put_var(ncid,varids(46),gwet_io)
      call check_ok(__FILE__,__LINE__,'Cannot write gwet')
      ncstatus = nf90_put_var(ncid,varids(47),snag_io)
      call check_ok(__FILE__,__LINE__,'Cannot write snag')
      ncstatus = nf90_put_var(ncid,varids(48),sfice_io)
      call check_ok(__FILE__,__LINE__,'Cannot write sfice')
      ncstatus = nf90_put_var(ncid,varids(49),ldew_io)
      call check_ok(__FILE__,__LINE__,'Cannot write ldew')
      ncstatus = nf90_put_var(ncid,varids(50),ldmsk1_io)
      call check_ok(__FILE__,__LINE__,'Cannot write ldmsk1')
      ncstatus = nf90_put_var(ncid,varids(51),taf_io)
      ...
      if ( lakemod == 1 ) then
        ...
        ncstatus = nf90_put_var(ncid,varids(54),hi_io)
        call check_ok(__FILE__,__LINE__,'Cannot write hi')
        ncstatus = nf90_put_var(ncid,varids(55),aveice_io)
        call check_ok(__FILE__,__LINE__,'Cannot write aveice')
        ncstatus = nf90_put_var(ncid,varids(56),hsnow_io)
      ...
      ncstatus = nf90_put_var(ncid,varids(60),flw_io)
      call check_ok(__FILE__,__LINE__,'Cannot write flw')
      ncstatus = nf90_put_var(ncid,varids(61),flwd_io)
      call check_ok(__FILE__,__LINE__,'Cannot write flwd')
      ncstatus = nf90_put_var(ncid,varids(62),fsw_io)
      call check_ok(__FILE__,__LINE__,'Cannot write fsw')
      ncstatus = nf90_put_var(ncid,varids(63),sinc_io)
      call check_ok(__FILE__,__LINE__,'Cannot write sinc')
      ncstatus = nf90_put_var(ncid,varids(64),ldmsk_io)
      call check_ok(__FILE__,__LINE__,'Cannot write ldmsk')
      if ( iocnflx == 2 ) then
        ncstatus = nf90_put_var(ncid,varids(65),zpbl_io)
        call check_ok(__FILE__,__LINE__,'Cannot write zpbl')
      end if
      if ( ichem == 1 ) then
        ncstatus = nf90_put_var(ncid,varids(66),chia_io)
        call check_ok(__FILE__,__LINE__,'Cannot write chia')
        ncstatus = nf90_put_var(ncid,varids(67),chib_io)
        call check_ok(__FILE__,__LINE__,'Cannot write chib')
        if ( igaschem == 1 .and. ichsolver > 0 ) then
          ncstatus = nf90_put_var(ncid,varids(68),chemall_io)
          call check_ok(__FILE__,__LINE__,'Cannot write chemall')
          ncstatus = nf90_put_var(ncid,varids(69),taucldsp_io)
          call check_ok(__FILE__,__LINE__,'Cannot write taucldsp')
        end if
        ncstatus = nf90_put_var(ncid,varids(70),remlsc_io)
        call check_ok(__FILE__,__LINE__,'Cannot write remlsc')
        ncstatus = nf90_put_var(ncid,varids(71),remcvc_io)
        call check_ok(__FILE__,__LINE__,'Cannot write remcvc')
        ncstatus = nf90_put_var(ncid,varids(72),remdrd_io)
        call check_ok(__FILE__,__LINE__,'Cannot write remdrd')
        ncstatus = nf90_put_var(ncid,varids(73),ssw2da_io)
        call check_ok(__FILE__,__LINE__,'Cannot write ssw2da')
        ncstatus = nf90_put_var(ncid,varids(74),sdelqk2d_io)
        call check_ok(__FILE__,__LINE__,'Cannot write sdelqk2d')
        ncstatus = nf90_put_var(ncid,varids(75),sdeltk2d_io)
        call check_ok(__FILE__,__LINE__,'Cannot write sdeltk2d')
        ncstatus = nf90_put_var(ncid,varids(76),sfracb2d_io)
        call check_ok(__FILE__,__LINE__,'Cannot write sfracb2d')
        ncstatus = nf90_put_var(ncid,varids(77),sfracs2d_io)
        call check_ok(__FILE__,__LINE__,'Cannot write sfracs2d')
        ncstatus = nf90_put_var(ncid,varids(78),sfracv2d_io)
        call check_ok(__FILE__,__LINE__,'Cannot write sfracv2d')
        ncstatus = nf90_put_var(ncid,varids(79),svegfrac2d_io)
        call check_ok(__FILE__,__LINE__,'Cannot write svegfrac2d')
      end if
      ncstatus = nf90_put_var(ncid,varids(80),dstor_io)
      call check_ok(__FILE__,__LINE__,'Cannot write dstor')
      ncstatus = nf90_put_var(ncid,varids(81),hstor_io)
      call check_ok(__FILE__,__LINE__,'Cannot write hstor')
      if ( islab_ocean == 1 .and. do_restore_sst ) then
        ncstatus = nf90_put_var(ncid,varids(82),qflux_restore_sst_io)
        call check_ok(__FILE__,__LINE__,'Cannot write qflux_restore_sst')
        ncstatus = nf90_put_var(ncid,varids(83),stepcount)
        call check_ok(__FILE__,__LINE__,'Cannot write stepcount')
      end if
#ifdef CLM
      ncstatus = nf90_put_var(ncid,varids(84),sols2d_io)
      call check_ok(__FILE__,__LINE__,'Cannot write sols2d')
      ncstatus = nf90_put_var(ncid,varids(85),soll2d_io)
      call check_ok(__FILE__,__LINE__,'Cannot write soll2d')
      ncstatus = nf90_put_var(ncid,varids(86),solsd2d_io)
