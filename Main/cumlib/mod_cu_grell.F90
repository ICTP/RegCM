...
module mod_cu_grell
  ...
  subroutine allocate_mod_cu_grell
    ...
    call getmem(mincld2d,jci1,jci2,ici1,ici2,'cu_grell:mincld2d')
    call getmem(shrmax2d,jci1,jci2,ici1,ici2,'cu_grell:shrmax2d')
    call getmem(shrmin2d,jci1,jci2,ici1,ici2,'cu_grell:shrmin2d')
    call getmem(edtmax2d,jci1,jci2,ici1,ici2,'cu_grell:edtmax2d')
    call getmem(edtmin2d,jci1,jci2,ici1,ici2,'cu_grell:edtmin2d')
    call getmem(edtmaxo2d,jci1,jci2,ici1,ici2,'cu_grell:edtmaxo2d')
    call getmem(edtmino2d,jci1,jci2,ici1,ici2,'cu_grell:edtmino2d')
    call getmem(edtmaxx2d,jci1,jci2,ici1,ici2,'cu_grell:edtmaxx2d')
    call getmem(edtminx2d,jci1,jci2,ici1,ici2,'cu_grell:edtminx2d')
    call getmem(htmax2d,jci1,jci2,ici1,ici2,'cu_grell:htmax2d')
    call getmem(htmin2d,jci1,jci2,ici1,ici2,'cu_grell:htmin2d')
    call getmem(kbmax2d,jci1,jci2,ici1,ici2,'cu_grell:kbmax2d')

    ncp = (jci2-jci1+1)*(ici2-ici1+1)

    call getmem(outq,1,ncp,1,kz,'cu_grell:outq')
    call getmem(outt,1,ncp,1,kz,'cu_grell:outt')
    call getmem(p,1,ncp,1,kz,'cu_grell:p')
    call getmem(po,1,ncp,1,kz,'cu_grell:po')
    call getmem(q,1,ncp,1,kz,'cu_grell:q')
    call getmem(qo,1,ncp,1,kz,'cu_grell:qo')
    call getmem(t,1,ncp,1,kz,'cu_grell:t')
    call getmem(tn,1,ncp,1,kz,'cu_grell:tn')
    call getmem(vsp,1,ncp,1,kz,'cu_grell:vsp')

    call getmem(pratec,1,ncp,'cu_grell:pratec')
    call getmem(psur,1,ncp,'cu_grell:psur')
    call getmem(ter11,1,ncp,'cu_grell:ter11')

    call getmem(dtauc,1,ncp,'cu_grell:dtauc')
    call getmem(pbcmax,1,ncp,'cu_grell:pbcmax')
    call getmem(mincld,1,ncp,'cu_grell:mincld')
    call getmem(shrmax,1,ncp,'cu_grell:shrmax')
    call getmem(shrmin,1,ncp,'cu_grell:shrmin')
    call getmem(edtmax,1,ncp,'cu_grell:edtmax')
    call getmem(edtmin,1,ncp,'cu_grell:edtmin')
    call getmem(edtmaxo,1,ncp,'cu_grell:edtmaxo')
    call getmem(edtmino,1,ncp,'cu_grell:edtmino')
    call getmem(edtmaxx,1,ncp,'cu_grell:edtmaxx')
    call getmem(edtminx,1,ncp,'cu_grell:edtminx')
    call getmem(htmax,1,ncp,'cu_grell:htmax')
    call getmem(htmin,1,ncp,'cu_grell:htmin')
    call getmem(kbmax,1,ncp,'cu_grell:kbmax')

    call getmem(kdet,1,ncp,'cu_grell:kdet')
    call getmem(kmin,1,ncp,'cu_grell:kmin')
    call getmem(k22,1,ncp,'cu_grell:k22')
    call getmem(kb,1,ncp,'cu_grell:kb')
    call getmem(kbcon,1,ncp,'cu_grell:kbcon')
    call getmem(ktop,1,ncp,'cu_grell:ktop')
    call getmem(iac,1,ncp,'cu_grell:iac')
    call getmem(jac,1,ncp,'cu_grell:jac')

    call getmem(dby,1,ncp,1,kz,'cu_grell:dby')
    call getmem(dbyo,1,ncp,1,kz,'cu_grell:dbyo')
    call getmem(dellah,1,ncp,1,kz,'cu_grell:dellah')
    call getmem(dellaq,1,ncp,1,kz,'cu_grell:dellaq')
    call getmem(dellat,1,ncp,1,kz,'cu_grell:dellat')
    call getmem(dkk,1,ncp,1,kz,'cu_grell:dkk')
    call getmem(he,1,ncp,1,kz,'cu_grell:he')
    call getmem(heo,1,ncp,1,kz,'cu_grell:heo')
    call getmem(hes,1,ncp,1,kz,'cu_grell:hes')
    call getmem(heso,1,ncp,1,kz,'cu_grell:heso')
    call getmem(pwc,1,ncp,1,kz,'cu_grell:pwc')
    call getmem(pwco,1,ncp,1,kz,'cu_grell:pwco')
    call getmem(pwcd,1,ncp,1,kz,'cu_grell:pwcd')
    call getmem(pwcdo,1,ncp,1,kz,'cu_grell:pwcdo')
    call getmem(qc,1,ncp,1,kz,'cu_grell:qc')
    call getmem(qco,1,ncp,1,kz,'cu_grell:qco')
    call getmem(qes,1,ncp,1,kz,'cu_grell:qes')
    call getmem(qeso,1,ncp,1,kz,'cu_grell:qeso')
    call getmem(qrcd,1,ncp,1,kz,'cu_grell:qrcd')
    call getmem(qrcdo,1,ncp,1,kz,'cu_grell:qrcdo')
    call getmem(tv,1,ncp,1,kz,'cu_grell:tv')
    call getmem(tvo,1,ncp,1,kz,'cu_grell:tvo')
    call getmem(xdby,1,ncp,1,kz,'cu_grell:xdby')
    call getmem(xhe,1,ncp,1,kz,'cu_grell:xhe')
    call getmem(xhes,1,ncp,1,kz,'cu_grell:xhes')
    call getmem(xpwc,1,ncp,1,kz,'cu_grell:xpwc')
    call getmem(xpwcd,1,ncp,1,kz,'cu_grell:xpwcd')
    call getmem(xq,1,ncp,1,kz,'cu_grell:xq')
    call getmem(xqc,1,ncp,1,kz,'cu_grell:xqc')
    call getmem(xqes,1,ncp,1,kz,'cu_grell:xqes')
    call getmem(xqrcd,1,ncp,1,kz,'cu_grell:xqrcd')
    call getmem(xt,1,ncp,1,kz,'cu_grell:xt')
    call getmem(xtv,1,ncp,1,kz,'cu_grell:xtv')
    call getmem(xz,1,ncp,1,kz,'cu_grell:xz')
    call getmem(z,1,ncp,1,kz,'cu_grell:z')
    call getmem(zo,1,ncp,1,kz,'cu_grell:zo')
    call getmem(cldf,1,ncp,1,kz,'cu_grell:cldf')

    call getmem(xac,1,ncp,'cu_grell:xac')
    call getmem(xao,1,ncp,'cu_grell:xao')
    call getmem(bu,1,ncp,'cu_grell:bu')
    call getmem(buo,1,ncp,'cu_grell:buo')
    call getmem(edt,1,ncp,'cu_grell:edt')
    call getmem(edto,1,ncp,'cu_grell:edto')
    call getmem(edtx,1,ncp,'cu_grell:edtx')
    call getmem(hcd,1,ncp,'cu_grell:hcd')
    call getmem(hcdo,1,ncp,'cu_grell:hcdo')
    call getmem(hkb,1,ncp,'cu_grell:hkb')
    call getmem(hkbo,1,ncp,'cu_grell:hkbo')
    call getmem(pwcav,1,ncp,'cu_grell:pwcav')
    call getmem(pwcavo,1,ncp,'cu_grell:pwcavo')
    call getmem(pwcev,1,ncp,'cu_grell:pwcev')
    call getmem(pwcevo,1,ncp,'cu_grell:pwcevo')
    call getmem(qcd,1,ncp,'cu_grell:qcd')
    call getmem(qcdo,1,ncp,'cu_grell:qcdo')
    call getmem(qcrit,1,ncp,'cu_grell:qcrit')
    call getmem(qck,1,ncp,'cu_grell:qck')
    call getmem(qcko,1,ncp,'cu_grell:qcko')
    call getmem(qkb,1,ncp,'cu_grell:qkb')
    call getmem(qkbo,1,ncp,'cu_grell:qkbo')
    call getmem(vshear,1,ncp,'cu_grell:vshear')
    call getmem(sdp,1,ncp,'cu_grell:sdp')
    call getmem(xxac,1,ncp,'cu_grell:xxac')
    call getmem(xhcd,1,ncp,'cu_grell:xhcd')
    call getmem(xhkb,1,ncp,'cu_grell:xhkb')
    call getmem(xmb,1,ncp,'cu_grell:xmb')
    call getmem(xpwcav,1,ncp,'cu_grell:xpwcav')
    call getmem(xpwcev,1,ncp,'cu_grell:xpwcev')
    call getmem(xqcd,1,ncp,'cu_grell:xqcd')
    call getmem(xqck,1,ncp,'cu_grell:xqck')
    call getmem(xqkb,1,ncp,'cu_grell:xqkb')
  ...
  subroutine cup
    ...
    do n = 1, nap
      ...
      if ( xac(n) >= d_zero ) then
        ...
        hkb(n) = d_half*(he(n,k22(n))+he(n,k22(n)+1))
        qkb(n) = d_half*(q(n,k22(n))+q(n,k22(n)+1))
        hkbo(n) = d_half*(heo(n,k22(n))+heo(n,k22(n)+1))
        qkbo(n) = d_half*(qo(n,k22(n))+qo(n,k22(n)+1))
        qck(n) = qkb(n)
        qcko(n) = qkbo(n)
      end if
    end do
...
!        pwco(n,k) = (qrcho-qeso(n,k))
    ...
    do n = 1, nap
      if ( xac(n) > xacact ) then
        ...
        dellah(n,1) = edt(n)*(dkk(n,2)*hcd(n)-dkk(n,2) * &
                      d_half*(he(n,1)+he(n,2)))*egrav/dp_s
        dellaq(n,1) = edt(n)*(dkk(n,2)*qrcd(n,2)-dkk(n,1) * &
      ...
      do n = 1, nap
        if ( xac(n) > xacact ) then
          if ( k /= 1 .and. k < ktop(n) ) then
            ...
            !
            detdo = (dkk(n,kk+1)-dkk(n,k))*(hcd(n)-dv2)
            detdoq = (dkk(n,k+1)-dkk(n,k))*(qrcd(n,k)-dv2q)
            dz = d_half*(z(n,k+1)-z(n,k-1))
            !
            ...
            dp_s = d_one/(50.0_rkx*(p(n,k-1)-p(n,k+1)))
            dellah(n,k) = ((aup-adw*dkk(n,k+1)*edt(n))*(dv1-dv2)+ &
                           (aup-adw*dkk(n,k)*edt(n))*(dv2-dv3))*  &
                    egrav*dp_s+adw*edt(n)*detdo*egrav*dp_s
            dellaq(n,k) = ((aup-adw*dkk(n,k+1)*edt(n))*(dv1q-dv2q)+ &
                           (aup-adw*dkk(n,k)*edt(n))*(dv2q-dv3q))*  &
                    egrav*dp_s+adw*edt(n)*detdoq*egrav*dp_s
            xhe(n,k) = dellah(n,k)*mbdt + he(n,k)
            xq(n,k) = dellaq(n,k)*mbdt + q(n,k)
            dellat(n,k) = rcpd*(dellah(n,k)-wlhv*dellaq(n,k))
            xt(n,k) = (mbdt*rcpd)*(dellah(n,k) - wlhv*dellaq(n,k)) + t(n,k)
      ...
      do n = 1, nap
        if ( xac(n) >= d_zero ) then
          if ( k < kmin(n) ) then
            ...
            xac(n) = xac(n)+edt(n)*dz*(egrav/(cpd*dt))*((dhh-dh)/(d_one+dg))
            !
            ...
            xao(n) = xao(n)+edto(n)*dz*(egrav/(cpd*dt))*((dhh-dh)/(d_one+dg))
            !
            ! modified by cloud
            !
