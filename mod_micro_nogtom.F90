...
module mod_micro_nogtom
  ...
  subroutine init_nogtom(ldmsk)
    ...
    integer(ik4) :: i , j , n
    ! Define species phase, 0 = vapour, 1 = liquid, 2 = ice 定义物种阶段
    iphase(iqqv) = 0
    iphase(iqql) = 1
    iphase(iqqi) = 2
    iphase(iqqr) = 1
    ...
    ! Set up melting/freezing index, 设置融化/冻结指数
    ! if an ice category melts/freezes, where does it go? 如果一类冰融化/结冰，它会去哪里
    ...
    ! Set the fall velocities 设置下落速度
    ...
    ! Set lfall 如果vqx(n)大于d_zero这个元素代表的物质或粒子具有下落或沉降的特性
    !$acc loop
    do n = 1 , nqx
  ...
  subroutine nogtom(mo2mc,mc2mo)
    ...
    ! total rain frac: fractional occurence of precipitation (%) 总降雨分数：降水出现的分数（%）
    ! for condensation 用于冷凝
    ! ice nuclei concentration 冰核浓度
    ! local real variables for autoconversion rate constants 自动转换速率常数的局部实变量
    real(rkx) :: alpha1 ! coefficient autoconversion cold cloud 系数自动转换冷云
    real(rkx) :: tmpa
    ! real(rkx) :: zqadj
    real(rkx) :: zrh
    real(rkx) :: beta , beta1
    ! local variables for condensation 凝结的局部变量
    real(rkx) :: cond , dtdp , cdmax
    ! local variables for melting 熔化的局部变量
    real(rkx) :: tdiff
    real(rkx) :: cons1
    ! constant for converting the fluxes unit measures 用于转换通量单位度量的常数
    real(rkx) :: prainx , psnowx
    ! local real constants for evaporation 蒸发的局部实常数
    ...
    real(rkx) :: qicefrac
    real(rkx) :: fluxq
    ! constants for deposition process 沉积过程常数
    real(rkx) :: vpice , vpliq , xadd , xbdd , cvds , &
                 qice0 , qinew , rainaut , snowaut
    ! constants for condensation and turbulent mixing erosion of clouds 云的凝结和湍流混合侵蚀常数
...
#ifndef __PGI ！预处理指令，用于检查宏__PGI是否未定义
    ...
    select case (iautoconv)
      case (1) ! Klein & Pincus (2000)
        selautoconv => klein_and_pincus !selautoconv是一个过程指针，之前已经声明为可以指向符合特定接口（如voidsub）的任何子程序
    ...
    ! Define the initial array qx 定义初始数组qx
    !$acc loop
    do k = 1 , kz
      do i = ici1 , ici2
        do j = jci1 , jci2
    ...
    !-----------------------------------
    ! initialization for cloud variables 云变量的初始化
    ! -------------------------------------
    ! Define qliq the function for mixed phase 定义qliq混合阶段的函数
    !     PHASE is calculated to distinguish the three cases: 计算PHASE是为了区分这三种情况
    !     PHASE = 1            water phase 水相
    !     PHASE = 0            ice phase
    !     0 < PHASE < 1        mixed phase
    ! Define pressure at full levels 定义满级压力
    ! pf = Pressure on fuLL levels (Pa)燃料液位上的压力（Pa）
    ! Define a new array for detrainment 定义一个新的数组以进行删除
    !$acc loop
    do k = 1 , kz !计算了一个与温度相关的量可能用于模拟云中液态水的存在或变化
      do i = ici1 , ici2
        do j = jci1 , jci2
    ...
    ! Reset total precipitation variables 重置总降水量变量
    pfplsx(:,:,:,:) = d_zero

    ! Compute supersaturations 计算过饱和度
    !$acc loop
    do k = 1 , kz
      do i = ici1 , ici2
        do j = jci1 , jci2
    ...
    !-------------------------------------
    ! Initial enthalpy and total water diagnostics 初始焓和总水诊断
    !-------------------------------------
    !
    ! Starting budget if requested 启动预算（如有要求）
    !
    if ( budget_compute ) then
      ...
      ! initialize the flux arrays 初始化通量阵列
      sumq0(:,:,:)     = d_zero
      sumh0(:,:,:)     = d_zero
      !$acc loop
      do k = 1 , kz
    ...
    ! -------------------------------
    ! Define saturation values 定义饱和度值
    !---------------------------
    !$acc loop
    do k = 1 , kz
      do i = ici1 , ici2
...
! save the precip production for chem. wet. dep. 
            ...
            if ( ltklt0 ) then
              ...
              !                         MELTING
              !---------------------------------------------------------------
              ! The melting of ice and snow are treated explicitly. 冰雪的融化得到了明确的处理
              ! First water and ice saturation are found 首次发现水和冰饱和度
              !---------------------------------------------
              ! ice saturation T < 273K
              ! liquid water saturation for T > 273K
              !---------------------------------------------
              ...
              if ( qicetot > d_zero ) then
                ! Calculate subsaturation 计算亚饱和
                ! qsice(j,i,k)-qxfg(iqqv),d_zero) 
                subsat = max(qsmixv-qxfg(iqqv),d_zero)
                ! Calculate difference between dry-bulb (t)  and the temperature 计算干球（t）与温度之间的差值
                ! at which the wet-bulb = 0degC
                ! Melting only occurs if the wet-bulb temperature >0 只有当湿球温度>0时才会发生熔化
                ! i.e. warming of ice particle due to melting > cooling 即冰粒因融化而变暖>冷却
                ! due to evaporation. 由于蒸发。
                ! The wet-bulb temperature is used in order to account for the 湿球温度用于说明在亚饱和空气中蒸发对熔化过程的热（冷却）影响。
                ! thermal (cooling) ect of evaporation on the melting process 当空气的相对湿度较低时，
                ! in sub-saturated air. The evaporation counteracts the latent 蒸发抵消了由于融化而产生的潜热
                ! heating due to melting and allows snow particles to survive 并使雪颗粒能够存活到略高的温度
                ! to slightly warmer temperatures when the relative 湿球温度近似为Wilson和Ballard（1999）描述的方案
                ...
                tdiff = tc
                ! Ensure CONS1 is positive so that MELTMAX = 0 if TDMTW0 < 0 确保CONS1为正，如果TDMTW0＜0，则MELTMAX＝0
