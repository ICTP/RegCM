...
! !USES:
  ...
  use mod_clm_megan,  only : shr_megan_megcomps_n, shr_megan_megcomp_t, shr_megan_linkedlist
  use mod_clm_megan,  only : shr_megan_mechcomps_n, shr_megan_mechcomps, shr_megan_mapped_emisfctrs
  use mod_clm_meganfactors,only : Agro, Amat, Anew, Aold, betaT, ct1, ct2, LDF, Ceo
...
!
    ...
    real(rk8) :: gamma                   ! activity factor (accounting for light, T, age, LAI conditions)
...
! !LOCAL VARIABLES:
    ...
    real(rk8), pointer :: efisop(:,:)      ! emission factors for isoprene for each pft [ug m-2 h-1]
...
! !LOCAL VARIABLES:
    ...
    real(rk8), parameter :: par0_sun = 200.D0              ! std conditions for past 24 hrs [umol/m2/s]
    real(rk8), parameter :: par0_shade = 50.D0             ! std conditions for past 24 hrs [umol/m2/s]
...
!-----------------------------------------------------------------------
  ...
  if ( (fsun240_in > 0.D0) .and. (fsun240_in < 1.D0) .and.  (forc_solad240_in > 0.D0) &
       .and. (forc_solai240_in > 0.D0)) then
     ...
     gamma_p_LDF = fsun_in * ( cp * alpha * par_sun_in * (1.D0 + alpha*alpha*par_sun_in*par_sun_in)**(-0.5D0) )
     ! SHADE:
     alpha = ca1 - ca2 * log(par240_sha_in)
     cp = ca3 * exp(ca2 * (par_sha_in-par0_shade))*par240_sha_in**(0.6D0)
     gamma_p_LDF = gamma_p_LDF + (1.D0-fsun_in) * (cp*alpha*par_sha_in*(1.D0 + alpha*alpha*par_sha_in*par_sha_in)**(-0.5D0))
  else
...
! !LOCAL VARIABLES:
    ...
    ! parameters
    real(rk8), parameter :: cce = 0.30D0                   ! factor to set emissions to unity @ std
    real(rk8), parameter :: cce1 = 0.24D0                  ! same as Cce but for non-accumulated vars
!-----------------------------------------------------------------------
    ...
    if ((clayfrac_in > 0) .and. (sandfrac_in > 0)) then 
       ...
       do j = 1,nlevsoi
          if  (sum(dz_in(1:j)) < root_depth_in)  then
             ...
             wilt = ((smpmax/sucsat_in(j))**(-1.D0/bsw_in(j))) * (watsat_in(j) - theta_ice)
...
! !LOCAL VARIABLES:
    ...
    real(rk8), parameter :: ct3 = 0.00831D0                ! empirical coefficient (0.0083 in User's Guide)
...
!-----------------------------------------------------------------------
    ...
    gamma_t_LDF = Eopt * ( ct2_in * exp(ct1_in * x)/(ct2_in - ct1_in * (1.D0 - exp(ct2_in * x))) )
...
! !LOCAL VARIABLES:
    ...
    real(rk8) :: fnew, fgro, fmat, fold  ! fractions of leaves at different phenological stages
    !-----------------------------------------------------------------------
    if ( (ivt_in == ndllf_dcd_brl_tree) .or. (ivt_in >= nbrdlf_dcd_trp_tree) ) then  ! non-evergreen
       
       if ( (elai_p_in > 0.0D0) .and. (elai_p_in < 1.D30) )then 
          elai_prev = 2.D0*elai_p_in-elai_in  ! have accumulated average lai over last timestep
...
!
! !USES:
    use mod_clm_varctl,    only : co2_ppmv      ! corresponds to CCSM_CO2_PPMV set in env_conf.xml
!
! !ARGUMENTS:
    implicit none
! !LOCAL VARIABLES:
    ...
    real(rk8)            :: ci               ! temporary sunlight/shade weighted cisun & cisha (umolCO2/mol)
    ...
    if ( (cisun_in > 0.D0) .and. (cisha_in > 0.D0) .and. (forc_pbot_in > 0.D0) .and. (fsun_in > 0.D0) ) then
