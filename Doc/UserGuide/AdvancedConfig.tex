...
!
 ...
 gnu2 = 0.1000,   ! nu factor for Asselin filter in leapfrog step (tracers).
                  ! MM5 manual , equation 2.4.6
                  ! Default 0.06,0.1 for hydro, 0.1,0.1 for nonhydro
 ckh = 1.0,       ! Background diffusion multiplication factor
 adyndif = 1.0,   ! Dynamical diffusion multiplication factor
 diffu_hgtf = 0,  ! Add topographic effect to diffusion
 upstream_mode = .true.,  ! Add off centering to advection
 upu = 0.150,             ! Maximum off-centering factor to use
 umax = 100.0,            ! Value of U for maximum off centering
 stability_enhance = .false., ! Do not allow horizontal advection to increase
                              ! tendencies if strong gradients are present
                              ! Enabled by default in non-hydro
 vert_stability_enhance = .false., ! Same for vertical advection
                                   ! Enabled by default in non-hydro
 ...
 c_rel_extrema = 0.2, ! Maximum gradient fraction for QX for advection to stop
...
\begin{Verbatim}
 ...
 lresamp = .true.,            ! Do a first resampling before interpolation
 smthbdy = .false.,           ! Perform extra smoothing in boundaries
 h2ohgt = .true.,             ! Allow water points to have hgt greater than 0
 ismthlev = 1,                ! Smoothing level (1-2-1, smoother-desmoother)
 roidem = 1.0,                ! Interpolation radius in ds unit for topography
...
\begin{Verbatim}
 ...
 ipcpcool =          1, ! Precipitation cooling effect over ocean surface
...
\begin{Verbatim}
 ...
 dtls      = 300.0,   ! Timescale for Qc removal (pptmax = qc/dtls)
...
\begin{Verbatim}
 ...
 ifaholtth10 = 0,  ! First approximation for obhukov length, th10 formula:
                   !      1 => 0.5 * (t+tg) * (1+0.61*q)
