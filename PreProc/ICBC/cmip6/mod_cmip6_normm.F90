...
module mod_cmip6_normm
  ...
  integer(ik8), parameter, dimension(nfiles) :: nrmm_start = [ &
   ...
   199001010300_ik8, 200001010300_ik8, 201001010300_ik8, 201501010000_ik8,  &
  ...
  contains
    ...
    subroutine read_hcoord_normm(ncid,lon,lat)
      ...
      call getmem(lon,1,nlon,'cmip6_nor:lon')
      call getmem(lat,1,nlat,'cmip6_nor:lat')
    ...
    subroutine read_hcoord_sst_normm(ncid,lon,lat)
      ...
      call getmem(lon,1,nlon,1,nlat,'cmip6_norm:lon')
      call getmem(lat,1,nlon,1,nlat,'cmip6_norm:lat')
    ...
    subroutine read_vcoord_normm(ncid,a,b,p0)
      ...
      call getmem(a,1,nlev,'cmip6_nor:a')
      call getmem(b,1,nlev,'cmip6_nor:b')
    ...
    recursive subroutine read_3d_normm(idate,v,lonlyc)
      ...
      if ( .not. associated(v%var) ) then
        ...
        call getmem(v%var,1,v%ni,1,v%nj,1,v%nk,'cmip6:normm:'//trim(v%vname))
#ifdef DEBUG
    ...
    recursive subroutine read_2d_normm(idate,v,lonlyc)
      ...
      if ( .not. associated(v%var) ) then
        v%ni = size(v%hcoord%lon1d)
        v%nj = size(v%hcoord%lat1d)
        call getmem(v%var,1,v%ni,1,v%nj,'cmip6:normm:'//trim(v%vname))
#ifdef DEBUG
