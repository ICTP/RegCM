...
! !USES:
  use mod_realkinds
  use mod_clm_type
  use mod_clm_varctl          , only : wrtdia, fpftdyn, iulog, create_glacier_mec_landunit
...
!-----------------------------------------------------------------------
...
#if (defined CN)
   call decomp_vertprofiles(begp, endp, begc, endc, filter%num_soilc, filter%soilc, filter%num_soilp, filter%soilp)
#endif
  ...
  ! VOC emission (A. Guenther's MEGAN (2006) model)
   ...
   call SLakeHydrology(begc, endc, begp, endp, filter%num_lakec, filter%lakec, &
                          filter%num_lakep, filter%lakep)
   call t_stopf('hylake')
   ...
   call CNEcosystemDyn(begc,endc,begp,endp,filter%num_soilc,&
                ...
                filter%pcropp, doalb)
   call CNAnnualUpdate(begc,endc,begp,endp,filter%num_soilc,&
                filter%soilc, filter%num_soilp, &
                filter%soilp)
#else
 ...
 ! CH4
#if (defined LCH4)
   call t_startf('ch4')
   call ch4 (begg, endg, begl, endl, begc, endc, begp, endp, filter%num_soilc, filter%soilc, &
             filter%num_lakec, filter%lakec, filter%num_soilp, filter%soilp)
   call t_stopf('ch4')
#endif
  ...
  if (monp1==1 .and. dayp1==1 .and. secp1==dtime .and. nstep>0)  then
     ...
     if (myid == italk) write(iulog,*) 'End of year. CNDV called now: ncdate=', &
  ...
  if (monp1==1 .and. dayp1==1 .and. secp1==dtime .and. nstep>0)  then
     ...
     call histCNDV()
     if (myid == italk) write(iulog,*) 'Annual CNDV calculations are complete'
...
! !USES:
  ...
  use spmdMod    , only : npes, MPI_REAL8, MPI_ANY_SOURCE, &
