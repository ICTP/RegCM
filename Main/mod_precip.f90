...
!         due to convection.
!         q = atms%qvb3d(i,1,j) + qcuten(i,1)*dt  [kg/kg][avg] 
!         tk = tb3d(i,1,j) + tcuten(i,1)*dt  [k][avg]
      q = atms%qvb3d(i,1,j)                              ![kg/kg][avg]
      tk = atms%tb3d(i,1,j)                              ![k][avg]
      tcel = tk - tzero                                  ![C][avg]
      p = atms%pb3d(i,1,j)*d_1000                        ![Pa][avg]
      rho = p/(rgas*tk)                                  ![kg/m3][avg]
      qcw = atms%qcb3d(i,1,j)                            ![kg/kg][avg]
...
!         due to convection.
!         q = atms%qvb3d(i,k,j) ! + qcuten(i,k)*dt                 
!         ![kg/kg][avg] tk = atms%tb3d(i,k,j) ! + tcuten(i,k)*dt            
!         ![k][avg]
      q = atms%qvb3d(i,k,j)                              ![kg/kg][avg]
      tk = atms%tb3d(i,k,j)                              ![k][avg]
      tcel = tk - tzero                                  ![C][avg]
      p = atms%pb3d(i,k,j)*d_1000                        ![Pa][avg]
      rho = p/(rgas*tk)                                  ![kg/m3][avg]
      qcw = atms%qcb3d(i,k,j)                            ![kg/kg][avg]
    ...
    do i = istart , iend
      ...
      do k = 2 , nk
        ...
        if ( remrat(i,k) > d_zero ) then
          do kk = 1 , k - 1
            rembc(i,k) = rembc(i,k) + remrat(i,kk)*atms%qcb3d(i,kk,j)    &
