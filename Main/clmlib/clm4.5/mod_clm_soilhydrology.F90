...
!
    ...
    real(rk8), pointer :: max_moist(:,:)    !maximum soil moisture (ice + liq, mm) 
    real(rk8), pointer :: moist(:,:)        !soil moisture in each VIC layers (liq, mm)
    real(rk8), pointer :: ice(:,:)          !ice len in each VIC layers(ice, mm) 
...
#if (defined VICHYDRO)
    real(rk8) :: A(lbc:ubc)                 !fraction of the saturated area
    real(rk8) :: ex(lbc:ubc)                !temporary variable (exponent)
    real(rk8) :: top_moist(lbc:ubc)         !temporary, soil moisture in top VIC layers
    real(rk8) :: top_max_moist(lbc:ubc)     !temporary, maximum soil moisture in top VIC layers
    real(rk8) :: top_ice(lbc:ubc)           !temporary, ice len in top VIC layers
    real(rk8) :: top_icefrac                !temporary, ice fraction in top VIC layers
...
!-----------------------------------------------------------------------
       ...
       do fc = 1, num_hydrologyc
         ...
         ! fractional impermeability
          ...
          if (origflag == 1) then
             ...
             icefrac(c,j) = min(1.D0,vol_ice(c,j)/watsat(c,j))
          endif

          fracice(c,j) = max(0.D0,exp(-3.D0*(1.D0-icefrac(c,j)))- exp(-3.D0))/(1.0D0-exp(-3.D0))
       end do
    end do
    ...
    do fc = 1, num_hydrologyc
       ...
       top_ice(c)     = max(0.D0,top_ice(c))
       max_infil(c)   = (1.D0+b_infil(c)) * top_max_moist(c)
       ex(c)          = b_infil(c) / (1.D0 + b_infil(c))
       ...
       top_fracice    = max(0.D0,exp(-3.D0*(1.D0-top_icefrac))- exp(-3.D0))/(1.0D0-exp(-3.D0))
#else
       ...
       if ( frost_table(c) > zwt(c)) then 
          ...
          if ( frost_table(c) > zwt_perched(c)) then 
             fsat(c) = wtfact(c) * exp(-0.5D0*fff(c)*zwt_perched(c))!*( frost_table(c) - zwt_perched(c))/4.0
          endif
       endif
       if (origflag == 1) then
#if (defined VICHYDRO)
...
#else
          fcov(c) = (1.D0 - fracice(c,1)) * fsat(c) + fracice(c,1)
#endif
       else
          fcov(c) = fsat(c)
       endif
...
! !USES:
    use mod_clm_type
    use mod_clm_varcon      , only : icol_roof, icol_road_imperv, icol_sunwall, &
...
! !ARGUMENTS:
    ...
    integer, intent(in) :: num_urbanc                   ! number of column urban points in column filter
    integer, intent(in) :: filter_urbanc(ubc-lbc+1)     ! column filter for urban points
    real(rk8), intent(out) :: vol_liq(lbc:ubc,1:nlevsoi) ! partial volume of liquid water in layer
!
! !CALLED FROM:
!
! !REVISION HISTORY:
...
!
    ...
    real(rk8), pointer :: frac_h2osfc(:)    ! fraction of ground covered by surface water (0 to 1)
    real(rk8), pointer :: frac_h2osfc_temp(:)    ! fraction of ground covered by surface water (0 to 1)
    real(rk8), pointer :: h2osoi_liq(:,:)   ! liquid water (kg/m2)
    real(rk8), pointer :: h2osoi_ice(:,:)   ! ice lens (kg/m2)
    real(rk8), pointer :: watsat(:,:)       ! volumetric soil water at saturation (porosity)
    real(rk8), pointer :: sucsat(:,:)       ! minimum soil suction (mm)
    ...
    real(rk8), pointer :: smpmin(:)         ! restriction for min of soil potential (mm)
    real(rk8), pointer :: dz(:,:)           ! layer depth (m)
    real(rk8), pointer :: hksat(:,:)        ! hydraulic conductivity at saturation (mm H2O /s)
...
!
    ...
    real(rk8) :: qflx_infl_excess(lbc:ubc)  ! infiltration excess runoff -> h2osfc
    ...
    real(rk8) :: h2osoi_vol                 !
#if (defined VICHYDRO)
    real(rk8) :: basis                      ! temporary, variable soil moisture holding capacity 
                                           ! in top VIC layers for runoff calculation
    real(rk8) :: rsurf_vic                  ! temp VIC surface runoff
    real(rk8) :: top_moist(lbc:ubc)         ! temporary, soil moisture in top VIC layers
    real(rk8) :: top_max_moist(lbc:ubc)     ! temporary, maximum soil moisture in top VIC layers
    real(rk8) :: top_ice(lbc:ubc)           ! temporary, ice len in top VIC layers
    real(rk8) :: top_icefrac                ! temporary, ice fraction in top VIC layers
...
!-----------------------------------------------------------------------
    ...
    do j = 1,nlevsoi
       do fc = 1, num_hydrologyc
          ...
          vol_ice(c,j) = min(watsat(c,j), h2osoi_ice(c,j)/(dz(c,j)*denice))
          eff_porosity(c,j) = max(0.01D0,watsat(c,j)-vol_ice(c,j))
    ...
    do fc = 1, num_hydrologyc
       ...
       if (ltype(clandunit(c)) == istsoil .or. ltype(clandunit(c))==istcrop) then
          ...
          qflx_in_soil(c) = (1.D0 - frac_h2osfc(c)) * (qflx_top_soil(c)  - qflx_surf(c))
         ...
         if(h2osfcflag==0) then 
          ...
          if (snl(c) >= 0) then
             ...
             qflx_infl(c) = qflx_top_soil(c) - qflx_surf(c) &
               - (1.0D0 - frac_sno(c)) * qflx_ev_soil(c)
          end if
          qflx_h2osfc_surf(c) = 0.D0
       endif
...
! !ARGUMENTS:
    ...
    integer , intent(in)  :: num_urbanc                   ! number of column urban points in column filter
    integer , intent(in)  :: filter_urbanc(ubc-lbc+1)     ! column filter for urban points
    real(rk8), intent(out) :: dwat(lbc:ubc,1:nlevsoi)      ! change of soil water [m3/m3]
    real(rk8), intent(out) :: hk(lbc:ubc,1:nlevsoi)        ! hydraulic conductivity [mm h2o/s]
    real(rk8), intent(out) :: dhkdw(lbc:ubc,1:nlevsoi)     ! d(hk)/d(vol_liq)
!
! !CALLED FROM:
! subroutine Hydrology2 in module Hydrology2Mod
!
...
!
    ...
    real(rk8), pointer :: smpmin(:)            ! restriction for min of soil potential (mm)
    real(rk8), pointer :: qflx_infl(:)         ! infiltration (mm H2O /s)
    real(rk8), pointer :: qflx_tran_veg_pft(:) ! vegetation transpiration (mm H2O/s) (+ = to atm)
    real(rk8), pointer :: qflx_tran_veg_col(:) ! vegetation transpiration (mm H2O/s) (+ = to atm)
    real(rk8), pointer :: eff_porosity(:,:)    ! effective porosity = porosity - vol_ice
    real(rk8), pointer :: watsat(:,:)          ! volumetric soil water at saturation (porosity)
    real(rk8), pointer :: hksat(:,:)           ! hydraulic conductivity at saturation (mm H2O /s)
    real(rk8), pointer :: bsw(:,:)             ! Clapp and Hornberger "b"
    real(rk8), pointer :: sucsat(:,:)          ! minimum soil suction (mm)
    real(rk8), pointer :: t_soisno(:,:)        ! soil temperature (Kelvin)
    real(rk8), pointer :: rootr_pft(:,:)       ! effective fraction of roots in each soil layer
    integer , pointer :: pfti(:)              ! beginning pft index for each column
    real(rk8), pointer :: fracice(:,:)         ! fractional impermeability (-)
    real(rk8), pointer :: h2osoi_vol(:,:)      ! volumetric soil water (0<=h2osoi_vol<=watsat) [m3/m3]
...
!
    real(rk8), pointer :: rootr_col(:,:)       ! effective fraction of roots in each soil layer
...
!
    ...
    real(rk8) :: rmx(lbc:ubc,1:nlevsoi+1)  ! "r" forcing term of tridiagonal matrix
    ...
    real(rk8) :: qin(lbc:ubc,1:nlevsoi+1)    ! flux of water into soil layer [mm h2o/s]
    real(rk8) :: qout(lbc:ubc,1:nlevsoi+1)   ! flux of water out of soil layer [mm h2o/s]
    ...
    real(rk8) :: dzq                         ! used in calculating qin, qout (difference in equilbirium matric potential)
    real(rk8) :: zimm(lbc:ubc,0:nlevsoi)     ! layer interface depth [mm]
    real(rk8) :: zq(lbc:ubc,1:nlevsoi+1)     ! equilibrium matric potential for each layer [mm]
    real(rk8) :: vol_eq(lbc:ubc,1:nlevsoi+1) ! equilibrium volumetric water content
...
!-----------------------------------------------------------------------
    ...
    do j = 1, nlevsoi
       do fc = 1, num_hydrologyc
          c = filter_hydrologyc(fc)
          zmm(c,j) = z(c,j)*1.D3
          dzmm(c,j) = dz(c,j)*1.D3
          zimm(c,j) = zi(c,j)*1.D3
          ! calculate icefrac up here
          vol_ice(c,j) = min(watsat(c,j), h2osoi_ice(c,j)/(dz(c,j)*denice))
          icefrac(c,j) = min(1.D0,vol_ice(c,j)/watsat(c,j))
          vwc_liq(c,j) = max(h2osoi_liq(c,j),1.0D-6)/(dz(c,j)*denh2o)
       end do
    end do
      ...
      !     this will only be used when zwt is below the soil column
       ...
       if(t_soisno(c,jwt(c)+1) < tfrz) then
          ...
          do j = nlevsoi,nlevgrnd
             if(zwt(c) <= zi(c,j)) then
                smp1 = hfus*(tfrz-t_soisno(c,j))/(grav*t_soisno(c,j)) * 1000.D0  !(mm)
                !smp1 = max(0.D0,smp1)
                smp1 = max(sucsat(c,nlevsoi),smp1)
                vwc_zwt(c) = watsat(c,nlevsoi)*(smp1/sucsat(c,nlevsoi))**(-1.D0/bsw(c,nlevsoi))
    ...
    do j=1,nlevsoi 
       do fc=1, num_hydrologyc
          ...
          ! rest of the layer
             ...
             tempi = 1.0D0
             temp0 = (((sucsat(c,j)+zwtmm(c)-zimm(c,j-1))/sucsat(c,j)))**(1.D0-1.D0/bsw(c,j))
             voleq1 = -sucsat(c,j)*watsat(c,j)/(1.D0-1.D0/bsw(c,j))/(zwtmm(c)-zimm(c,j-1))*(tempi-temp0)
             vol_eq(c,j) = (voleq1*(zwtmm(c)-zimm(c,j-1)) + watsat(c,j)*(zimm(c,j)-zwtmm(c)))/(zimm(c,j)-zimm(c,j-1))
             vol_eq(c,j) = min(watsat(c,j),vol_eq(c,j))
             vol_eq(c,j) = max(vol_eq(c,j),0.0D0)
          else
             tempi = (((sucsat(c,j)+zwtmm(c)-zimm(c,j))/sucsat(c,j)))**(1.D0-1.D0/bsw(c,j))
             temp0 = (((sucsat(c,j)+zwtmm(c)-zimm(c,j-1))/sucsat(c,j)))**(1.D0-1.D0/bsw(c,j))
             vol_eq(c,j) = -sucsat(c,j)*watsat(c,j)/(1.D0-1.D0/bsw(c,j))/(zimm(c,j)-zimm(c,j-1))*(tempi-temp0)
             vol_eq(c,j) = max(vol_eq(c,j),0.0D0)
             vol_eq(c,j) = min(watsat(c,j),vol_eq(c,j))
          endif
          zq(c,j) = -sucsat(c,j)*(max(vol_eq(c,j)/watsat(c,j),0.01D0))**(-bsw(c,j))
    ...
    do fc=1, num_hydrologyc
       ...
       if(jwt(c) == nlevsoi) then 
          tempi = 1.D0
          temp0 = (((sucsat(c,j)+zwtmm(c)-zimm(c,j))/sucsat(c,j)))**(1.D0-1.D0/bsw(c,j))
          vol_eq(c,j+1) = -sucsat(c,j)*watsat(c,j)/(1.D0-1.D0/bsw(c,j))/(zwtmm(c)-zimm(c,j))*(tempi-temp0)
          vol_eq(c,j+1) = max(vol_eq(c,j+1),0.0D0)
          vol_eq(c,j+1) = min(watsat(c,j),vol_eq(c,j+1))
          zq(c,j+1) = -sucsat(c,j)*(max(vol_eq(c,j+1)/watsat(c,j),0.01D0))**(-bsw(c,j))
    ...
    do j = 1, nlevsoi
       do fc = 1, num_hydrologyc
          ...
          if(origflag == 1) then
             s1 = 0.5D0*(h2osoi_vol(c,j) + h2osoi_vol(c,min(nlevsoi, j+1))) / &
                  (0.5D0*(watsat(c,j)+watsat(c,min(nlevsoi, j+1))))
          else
             s1 = 0.5D0*(vwc_liq(c,j) + vwc_liq(c,min(nlevsoi, j+1))) / &
                  (0.5D0*(watsat(c,j)+watsat(c,min(nlevsoi, j+1))))
          endif
          s1 = min(1.D0, s1)
          s2 = hksat(c,j)*s1**(2.D0*bsw(c,j)+2.D0)

          ! replace fracice with impedance factor, as in zhao 97,99
          if(origflag == 1) then
             imped(c,j)=(1.D0-0.5D0*(fracice(c,j)+fracice(c,min(nlevsoi, j+1))))
          else
             imped(c,j)=10.D0**(-e_ice*(0.5D0*(icefrac(c,j)+icefrac(c,min(nlevsoi, j+1)))))
          endif
          hk(c,j) = imped(c,j)*s1*s2
          dhkdw(c,j) = imped(c,j)*(2.D0*bsw(c,j)+3.D0)*s2* &
                       (1.D0/(watsat(c,j)+watsat(c,min(nlevsoi, j+1))))

          ! compute matric potential and derivative based on liquid water content only
          if(origflag == 1) then
             s_node = max(h2osoi_vol(c,j)/watsat(c,j), 0.01D0)
          else
             s_node = max(vwc_liq(c,j)/watsat(c,j), 0.01D0)
          endif
    ...
    do fc = 1, num_hydrologyc
       ...
       if(jwt(c) < nlevsoi) then
         ...
         dzmm(c,nlevsoi+1) = (1.D3*zwt(c) - zmm(c,nlevsoi))
       end if
    end do
    ...
    do fc = 1, num_hydrologyc
       ...
       if(j > jwt(c)) then !water table is in soil column
         ...
         qout(c,j+1)   =  0.D0  ! zero-flow bottom boundary condition
    ...
    do fc = 1,num_hydrologyc
       ...
       if(jwt(c) < nlevsoi) then
          ...
          if(jwt(c) == 0) then
             ...
             !             qcharge(c) = -ka * (wh_zwt-wh)/((zwt(c)-z(c,jwt(c)))*1000.D0)
...
!
    ...
    real(rk8), pointer :: qflx_qrgwl(:)     ! qflx_surf at glaciers, wetlands, lakes (mm H2O /s)
...
!
    ...
    integer  :: jwt(lbc:ubc)             ! index of the soil layer right above the water table (-)
    real(rk8) :: rsub_bot(lbc:ubc)        ! subsurface runoff - bottom drainage (mm/s)
    real(rk8) :: rsub_top(lbc:ubc)        ! subsurface runoff - topographic control (mm/s)
    real(rk8) :: fff(lbc:ubc)             ! decay factor (m-1)
    real(rk8) :: xsi(lbc:ubc)             ! excess soil water above saturation at layer i (mm)
    real(rk8) :: xsia(lbc:ubc)            ! available pore space at layer i (mm)
    real(rk8) :: xs1(lbc:ubc)             ! excess soil water above saturation at layer 1 (mm)
    real(rk8) :: smpfz(1:nlevsoi)         ! matric potential of layer right above water table (mm)
    real(rk8) :: wtsub                    ! summation of hk*dzmm for layers below water table (mm**2/s)
    real(rk8) :: rous                     ! aquifer yield (-)
    real(rk8) :: wh                       ! smpfz(jwt)-z(jwt) (mm)
    real(rk8) :: wh_zwt                   ! water head at the water table depth (mm)
    real(rk8) :: ws                       ! summation of pore space of layers below water table (mm)
    real(rk8) :: s_node                   ! soil wetness (-)
    real(rk8) :: dzsum                    ! summation of dzmm of layers below water table (mm)
    real(rk8) :: icefracsum               ! summation of icefrac*dzmm of layers below water table (-)
    real(rk8) :: fracice_rsub(lbc:ubc)    ! fractional impermeability of soil layers (-)
    real(rk8) :: ka                       ! hydraulic conductivity of the aquifer (mm/s)
    real(rk8) :: dza                      ! fff*(zwt-z(jwt)) (-)
    real(rk8) :: available_h2osoi_liq     ! available soil liquid water in a layer
...
#if (defined VICHYDRO)
    real(rk8) :: dsmax_tmp(lbc:ubc)       ! temporary variable for ARNO subsurface runoff calculation
    real(rk8) :: rsub_tmp                 ! temporary variable for ARNO subsurface runoff calculation
    real(rk8) :: frac                     ! temporary variable for ARNO subsurface runoff calculation
    real(rk8) :: rel_moist                ! relative moisture, temporary variable
    real(rk8) :: wtsub_vic                ! summation of hk*dzmm for layers in the third VIC layer
#endif
!-----------------------------------------------------------------------
...
! use analytical expression for specific yield
                ...
                qcharge_layer=min(qcharge_layer,0.D0)
                qcharge_tot = qcharge_tot - qcharge_layer

                if (qcharge_tot >= 0.) then 
                   zwt(c) = zwt(c) - qcharge_layer/s_y/1000.D0
                   exit
                else
                   zwt(c) = zi(c,j)
                endif
             ...
             enddo
             if (qcharge_tot > 0.) zwt(c) = zwt(c) - qcharge_tot/1000.D0/rous
          endif
...
! perched water table code
    do fc = 1, num_hydrologyc
        ...
        q_perch_max = 1.D-5 * sin(topo_slope(c) * (rpi/180.D0))
...
! if water table is above frost table, do not use topmodel baseflow formulation
          if (zwt(c) < frost_table(c) .and. t_soisno(c,k_frz) <= tfrz &
             ...
             q_perch = 0.D0
             do k = jwt(c)+1, k_frz
                imped=10.D0**(-e_ice*(0.5D0*(icefrac(c,k)+icefrac(c,min(nlevsoi, k+1)))))
                q_perch = q_perch + imped*hksat(c,k)*dzmm(c,k)
                wtsub = wtsub + dzmm(c,k)
             end do
             if (wtsub > 0.D0) q_perch = q_perch/wtsub
             ...
             do k = jwt(c)+1, k_frz
                ...
                if (rsub_top_tot >= 0.) then 
                   zwt(c) = zwt(c) - rsub_top_layer/eff_porosity(c,k)/1000.D0
                   exit
                else
                   zwt(c) = zi(c,k)
                endif
...
! sat_lev is an arbitrary saturation level used to determine perched water table
          ...
          if (k_frz > k_perch) then
             ...
             do k = k_perch+1, k_frz
                ...
                if (rsub_top_tot >= 0.) then 
                   zwt_perched(c) = zwt_perched(c) - rsub_top_layer/eff_porosity(c,k)/1000.D0
                   exit
                else
...
! use analytical expression for specific yield
                ...
                if (rsub_top_tot >= 0.) then 
                   zwt(c) = zwt(c) - rsub_top_layer/s_y/1000.D0
...
!--  remove residual rsub_top  ---------------------------------------------
            zwt(c) = zwt(c) - rsub_top_tot/1000.D0/rous
...
!-- recompute jwt  ---------------------------------------------------------
       ...
       zwt(c) = max(0.0D0,zwt(c))
       zwt(c) = min(80.D0,zwt(c))
    ...
    do j = nlevsoi,2,-1
       do fc = 1, num_hydrologyc
          c = filter_hydrologyc(fc)
          xsi(c)            = max(h2osoi_liq(c,j)-eff_porosity(c,j)*dzmm(c,j),0.D0)
    ...
    do fc = 1, num_hydrologyc
       c = filter_hydrologyc(fc)
       xs1(c)          = max(max(h2osoi_liq(c,1),0.D0)-max(0.D0,(pondmx+watsat(c,1)*dzmm(c,1)-h2osoi_ice(c,1))),0.D0)
    ...
    do j = 1, nlevsoi-1
       do fc = 1, num_hydrologyc
          ...
          if (h2osoi_liq(c,j) < watmin) then
             ...
             if(j == jwt(c)) then 
                zwt(c) = zwt(c) + xs(c)/eff_porosity(c,j)/1000.D0
             endif
          else
             xs(c) = 0.D0
          end if
    ...
    do fc = 1, num_hydrologyc
       ...
       if (h2osoi_liq(c,j) < watmin) then
          ...
          searchforwater: do i = nlevsoi-1, 1, -1
            available_h2osoi_liq = max(h2osoi_liq(c,i)-watmin-xs(c),0.D0)
