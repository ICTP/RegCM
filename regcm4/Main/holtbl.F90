...
!
      ...
      do j = 1 , jx
        jbm1 = j-1
        if(jbm1.eq.0) jbm1 = jx
#endif 
        do k = 1 , kz
          do i = 2 , iym1
            dumr = 4./(psb(i,j)+psb(i,jbm1)+psb(i-1,j)+psb(i-1,jbm1))
...
!         compute friction velocity
          uflxsfx = uvdrag(i,j)*ubx3d(i,kz,j)
          vflxsfx = uvdrag(i,j)*vbx3d(i,kz,j)
...
!       calculate coefficients at dot points for u and v wind
         ...
         do k = 1 , kz
           do i = 2 , iym1
             ...
             idxm1 = max0(idxm1,2)
             if ( k.gt.1 )                                             &
                & betak(i,k) = 0.25*(akzz1(idx,k,jbm1)+                 &
                &                    akzz1(idxm1,k,jbm1)+               &
                &                    akzz1(idx,k,j)+akzz1(idxm1,k,j))
             alphak(i,k) = 0.25*(akzz2(idx,k,jbm1)+akzz2(idxm1,k,jbm1)+  &
...
!
        ...
        do i = 2 , iym1
          ...
          drgdot = 0.25*(uvdrag(idxm1,jbm1)+uvdrag(idxm1,j)          &
                 & +uvdrag(idx,jbm1)+uvdrag(idx,j))
...
!
        do k = 1 , kz
          do i = 2 , iym1
            dumr = 0.25*(psb(i,j)+psb(i,jbm1)+psb(i-1,j)+psb(i-1,jbm1))
