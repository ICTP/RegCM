...
module mod_cmip6_mpilr
  ...
  character(len=*), parameter :: mpilr_version = 'v20190710'
  character(len=*), parameter :: mpilr_version1 = 'v20190815'

  public :: read_3d_mpilr, read_2d_mpilr, read_fx_mpilr, read_sst_mpilr

  contains

    subroutine read_hcoord_mpilr(ncid,lon,lat)
      implicit none (type, external)
      ...
      call getmem(lon,1,nlon,'cmip6_mpi:lon')
      call getmem(lat,1,nlat,'cmip6_mpi:lat')
    ...
    end subroutine read_hcoord_mpilr

    subroutine read_hcoord_sst_mpilr(ncid,lon,lat)
      implicit none (type, external)
      integer(ik4), intent(in) :: ncid
      real(rkx), pointer, contiguous, dimension(:,:), intent(inout) :: lon, lat
      integer(ik4) :: istatus, idimid, ivarid
      ...
      call getmem(lon,1,nlon,1,nlat,'cmip6_mpi:lon')
      call getmem(lat,1,nlon,1,nlat,'cmip6_mpi:lat')
    ...
    end subroutine read_hcoord_sst_mpilr

    subroutine read_vcoord_mpilr(ncid,ap,b)
      implicit none (type, external)
      integer(ik4), intent(in) :: ncid
      ...
      call getmem(ap,1,nlev,'cmip6_mpi:ap')
      call getmem(b,1,nlev,'cmip6_mpi:b')
    ...
    end subroutine read_vcoord_mpilr

    recursive subroutine read_3d_mpilr(idate,v,lonlyc)
      implicit none (type, external)
      ...
      if ( .not. associated(v%hcoord) ) then
        allocate(v%hcoord)
        call read_hcoord_mpilr(v%ncid,v%hcoord%lon1d,v%hcoord%lat1d)
      end if
      if ( .not. associated(v%vcoord) ) then
        allocate(v%vcoord)
        call read_vcoord_mpilr(v%ncid,v%vcoord%ak,v%vcoord%bk)
      end if
      if ( .not. associated(v%var) ) then
        v%ni = size(v%hcoord%lon1d)
        v%nj = size(v%hcoord%lat1d)
        v%nk = size(v%vcoord%ak)
        call getmem(v%var,1,v%ni,1,v%nj,1,v%nk,'cmip6:mpilr:'//trim(v%vname))
#ifdef DEBUG
    ...
    end subroutine read_3d_mpilr

    recursive subroutine read_2d_mpilr(idate,v,lonlyc)
      implicit none (type, external)
      type(rcm_time_and_date), intent(in) :: idate
      ...
      if ( .not. associated(v%var) ) then
        v%ni = size(v%hcoord%lon1d)
        v%nj = size(v%hcoord%lat1d)
        call getmem(v%var,1,v%ni,1,v%nj,'cmip6:mpilr:'//trim(v%vname))
#ifdef DEBUG
      ...
      call cmip6_error(istatus,__FILE__,__LINE__, &
          'Error read variable '//v%vname//' from '//trim(v%filename)//'.')
    end subroutine read_2d_mpilr

    recursive subroutine read_fx_mpilr(v)
      implicit none (type, external)
      type(cmip6_2d_var), pointer, intent(inout) :: v
      integer(ik4) :: istatus

      v%filename = trim(cmip6_fxpath(mpilr_version,v%vname))
#ifdef DEBUG
      write(stderr,*) 'Opening ',trim(v%filename)
#endif
      istatus = nf90_open(v%filename,nf90_nowrite,v%ncid)
      ...
      call cmip6_error(istatus,__FILE__,__LINE__, &
          'Error close file '//trim(v%filename)//'.')
    end subroutine read_fx_mpilr

    recursive subroutine read_sst_mpilr(idate,v,lat,lon)
      implicit none (type, external)
      type(rcm_time_and_date), intent(in) :: idate
      ...
      if ( v%ncid == -1 ) then
        ...
        y = 1850 + ((year-1850)/20)*20
        write(v%filename,'(a,i4,a,i4,a)') &
          trim(cmip6_path(y,'Oday',mpilr_version,v%vname)), &
          y, '0101-', y+19, '1231.nc'
#ifdef DEBUG
        write(stderr,*) 'Opening ',trim(v%filename)
#endif
      ...
      if ( .not. associated(v%hcoord) ) then
        allocate(v%hcoord)
        call read_hcoord_sst_mpilr(v%ncid,v%hcoord%lon2d,v%hcoord%lat2d)
      ...
      if ( irec > v%nrec ) then
        ...
        v%nrec = -1
        irec = 1
        call read_sst_mpilr(idate,v,lat,lon)
      end if
      istart(1) = 1
      istart(2) = 1
      istart(3) = irec
    ...
    end subroutine read_sst_mpilr

end module mod_cmip6_mpilr
