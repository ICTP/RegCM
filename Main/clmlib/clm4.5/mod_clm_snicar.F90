...
! !PUBLIC DATA MEMBERS:
  ...
  real(rk8), public, parameter :: snw_rds_min = 54.526D0  ! minimum allowed snow effective radius (also "fresh snow" value) [microns]
  ...
  real(rk8), public, parameter :: scvng_fct_mlt_bcphi = 0.20D0   ! scavenging factor for hydrophillic BC inclusion in meltwater [frc]
  real(rk8), public, parameter :: scvng_fct_mlt_bcpho = 0.03D0   ! scavenging factor for hydrophobic BC inclusion in meltwater [frc]
  real(rk8), public, parameter :: scvng_fct_mlt_ocphi = 0.20D0   ! scavenging factor for hydrophillic OC inclusion in meltwater [frc]
  real(rk8), public, parameter :: scvng_fct_mlt_ocpho = 0.03D0   ! scavenging factor for hydrophobic OC inclusion in meltwater [frc]
  real(rk8), public, parameter :: scvng_fct_mlt_dst1  = 0.02D0   ! scavenging factor for dust species 1 inclusion in meltwater [frc]
  real(rk8), public, parameter :: scvng_fct_mlt_dst2  = 0.02D0   ! scavenging factor for dust species 2 inclusion in meltwater [frc]
  real(rk8), public, parameter :: scvng_fct_mlt_dst3  = 0.01D0   ! scavenging factor for dust species 3 inclusion in meltwater [frc]
  real(rk8), public, parameter :: scvng_fct_mlt_dst4  = 0.01D0   ! scavenging factor for dust species 4 inclusion in meltwater [frc]
...
! !PRIVATE DATA MEMBERS:
  ...
  real(rk8), parameter :: snw_rds_max     = 1500.D0      ! maximum allowed snow effective radius [microns]
  real(rk8), parameter :: snw_rds_refrz   = 1000.D0      ! effective radius of re-frozen snow [microns]

  real(rk8), parameter :: min_snw = 1.0D-30             ! minimum snow mass required for SNICAR RT calculation [kg m-2]

  !real(rk8), parameter :: C1_liq_Brun89 = 1.28D-17    ! constant for liquid water grain growth [m3 s-1], from Brun89
  real(rk8), parameter :: C1_liq_Brun89 = 0.D0           ! constant for liquid water grain growth [m3 s-1], from Brun89: zeroed to accomodate dry snow aging
  real(rk8), parameter :: C2_liq_Brun89 = 4.22D-13     ! constant for liquid water grain growth [m3 s-1], from Brun89: corrected for LWC in units of percent

  real(rk8), parameter :: tim_cns_bc_rmv  = 2.2D-8     ! time constant for removal of BC in snow on sea-ice [s-1] (50% mass removal/year)
  real(rk8), parameter :: tim_cns_oc_rmv  = 2.2D-8     ! time constant for removal of OC in snow on sea-ice [s-1] (50% mass removal/year)
  real(rk8), parameter :: tim_cns_dst_rmv = 2.2D-8     ! time constant for removal of dust in snow on sea-ice [s-1] (50% mass removal/year)
  ...
  real(rk8), parameter :: xdrdt = 1.0D0                  ! arbitrary factor applied to snow aging rate
  ...
  ! (arrays declared here, but are set in iniTimeConst)
  real(rk8), pointer :: snowage_tau(:,:,:) ! (idx_rhos_max,idx_Tgrd_max,idx_T_max)
  real(rk8), pointer :: snowage_kappa(:,:,:) ! (idx_rhos_max,idx_Tgrd_max,idx_T_max)
  real(rk8), pointer :: snowage_drdt0(:,:,:) ! idx_rhos_max,idx_Tgrd_max,idx_T_max)
...
! !INTERFACE:
  ...
  subroutine SNICAR_RT (flg_snw_ice, lbc, ubc, num_nourbanc, filter_nourbanc,  &
    ...
    real(rk8), intent(in)  :: coszen(lbc:ubc)                      ! cosine of solar zenith angle for next time step (col) [unitless]
    integer , intent(in)  :: flg_slr_in                           ! flag: =1 for direct-beam incident flux, =2 for diffuse incident flux
    real(rk8), intent(in)  :: h2osno_liq(lbc:ubc,-nlevsno+1:0)     ! liquid water content (col,lyr) [kg/m2]
    real(rk8), intent(in)  :: h2osno_ice(lbc:ubc,-nlevsno+1:0)     ! ice content (col,lyr) [kg/m2]
    integer,  intent(in)  :: snw_rds(lbc:ubc,-nlevsno+1:0)        ! snow effective radius (col,lyr) [microns, m^-6]
    real(rk8), intent(in)  :: mss_cnc_aer_in(lbc:ubc,-nlevsno+1:0,sno_nbr_aer)  ! mass concentration of all aerosol species (col,lyr,aer) [kg/kg]
    real(rk8), intent(in)  :: albsfc(lbc:ubc,numrad)               ! albedo of surface underlying snow (col,bnd) [frc]
    real(rk8), intent(out) :: albout(lbc:ubc,numrad)               ! snow albedo, averaged into 2 bands (=0 if no sun or no snow) (col,bnd) [frc]
    real(rk8), intent(out) :: flx_abs(lbc:ubc,-nlevsno+1:1,numrad) ! absorbed flux in each layer per unit flux incident on top of snowpack (col,lyr,bnd) [frc]
    ...
    real(rk8), pointer :: h2osno(:)           ! snow liquid water equivalent (col) [kg/m2]   
    ...
    real(rk8), pointer :: londeg(:)           ! longitude (degrees) (debugging only)
    real(rk8), pointer :: latdeg(:)           ! latitude (degrees) (debugging only)
...
!-----------------------------------------------------------------------
    ...
    real(rk8):: flx_slrd_lcl(1:numrad_snw)         ! direct beam incident irradiance [W/m2] (set to 1)
    real(rk8):: flx_slri_lcl(1:numrad_snw)         ! diffuse incident irradiance [W/m2] (set to 1)
    real(rk8):: mss_cnc_aer_lcl(-nlevsno+1:0,1:sno_nbr_aer) ! aerosol mass concentration (lyr,aer_nbr) [kg/kg]
    real(rk8):: h2osno_lcl                         ! total column snow mass [kg/m2]
    real(rk8):: h2osno_liq_lcl(-nlevsno+1:0)       ! liquid water mass [kg/m2]
    real(rk8):: h2osno_ice_lcl(-nlevsno+1:0)       ! ice mass [kg/m2]
    real(rk8):: albsfc_lcl(1:numrad_snw)           ! albedo of underlying surface [frc]
    real(rk8):: ss_alb_snw_lcl(-nlevsno+1:0)       ! single-scatter albedo of ice grains (lyr) [frc]
    real(rk8):: asm_prm_snw_lcl(-nlevsno+1:0)      ! asymmetry parameter of ice grains (lyr) [frc]
    real(rk8):: ext_cff_mss_snw_lcl(-nlevsno+1:0)  ! mass extinction coefficient of ice grains (lyr) [m2/kg]
    real(rk8):: ss_alb_aer_lcl(sno_nbr_aer)        ! single-scatter albedo of aerosol species (aer_nbr) [frc] 
    real(rk8):: asm_prm_aer_lcl(sno_nbr_aer)       ! asymmetry parameter of aerosol species (aer_nbr) [frc]
    real(rk8):: ext_cff_mss_aer_lcl(sno_nbr_aer)   ! mass extinction coefficient of aerosol species (aer_nbr) [m2/kg]
    ...
    real(rk8):: flx_wgt(1:numrad_snw)              ! weights applied to spectral bands, specific to direct and diffuse cases (bnd) [frc]
    ...
    real(rk8):: albedo                             ! temporary snow albedo [frc]
    real(rk8):: flx_sum                            ! temporary summation variable for NIR weighting
    real(rk8):: albout_lcl(numrad_snw)             ! snow albedo by band [frc]
    real(rk8):: flx_abs_lcl(-nlevsno+1:1,numrad_snw)! absorbed flux per unit incident flux at top of snowpack (lyr,bnd) [frc]
 
    real(rk8):: L_snw(-nlevsno+1:0)                ! h2o mass (liquid+solid) in snow layer (lyr) [kg/m2]
    real(rk8):: tau_snw(-nlevsno+1:0)              ! snow optical depth (lyr) [unitless]
    real(rk8):: L_aer(-nlevsno+1:0,sno_nbr_aer)    ! aerosol mass in snow layer (lyr,nbr_aer) [kg/m2] 
    real(rk8):: tau_aer(-nlevsno+1:0,sno_nbr_aer)  ! aerosol optical depth (lyr,nbr_aer) [unitless]
    real(rk8):: tau_sum                            ! cumulative (snow+aerosol) optical depth [unitless]
    real(rk8):: tau_clm(-nlevsno+1:0)              ! column optical depth from layer bottom to snowpack top (lyr) [unitless] 
    real(rk8):: omega_sum                          ! temporary summation of single-scatter albedo of all aerosols [frc]
    real(rk8):: g_sum                              ! temporary summation of asymmetry parameter of all aerosols [frc]

    real(rk8):: tau(-nlevsno+1:0)                  ! weighted optical depth of snow+aerosol layer (lyr) [unitless]
    real(rk8):: omega(-nlevsno+1:0)                ! weighted single-scatter albedo of snow+aerosol layer (lyr) [frc]
    real(rk8):: g(-nlevsno+1:0)                    ! weighted asymmetry parameter of snow+aerosol layer (lyr) [frc]
    real(rk8):: tau_star(-nlevsno+1:0)             ! transformed (i.e. Delta-Eddington) optical depth of snow+aerosol layer (lyr) [unitless]
    real(rk8):: omega_star(-nlevsno+1:0)           ! transformed (i.e. Delta-Eddington) SSA of snow+aerosol layer (lyr) [frc]
    real(rk8):: g_star(-nlevsno+1:0)               ! transformed (i.e. Delta-Eddington) asymmetry paramater of snow+aerosol layer (lyr) [frc]
    ...
    real(rk8):: F_direct(-nlevsno+1:0)             ! direct-beam radiation at bottom of layer interface (lyr) [W/m^2]
    real(rk8):: F_net(-nlevsno+1:0)                ! net radiative flux at bottom of layer interface (lyr) [W/m^2]
    real(rk8):: F_abs(-nlevsno+1:0)                ! net absorbed radiative energy (lyr) [W/m^2]
    real(rk8):: F_abs_sum                          ! total absorbed energy in column [W/m^2]
    real(rk8):: F_sfc_pls                          ! upward radiative flux at snowpack top [W/m^2]
    real(rk8):: F_btm_net                          ! net flux at bottom of snowpack [W/m^2]                    
    real(rk8):: F_sfc_net                          ! net flux at top of snowpack [W/m^2]
    real(rk8):: energy_sum                         ! sum of all energy terms; should be 0.0 [W/m^2]
    real(rk8):: F_direct_btm                       ! direct-beam radiation at bottom of snowpack [W/m^2]
    real(rk8):: mu_not                             ! cosine of solar zenith angle (used locally) [frc]

    integer :: err_idx                            ! counter for number of times through error loop [nbr]
    real(rk8):: lat_coord                          ! gridcell latitude (debugging only)
    real(rk8):: lon_coord                          ! gridcell longitude (debugging only)
    integer :: sfctype                            ! underlying surface type (debugging only)
    real(rk8):: pi                                 ! 3.1415...


    ! intermediate variables for radiative transfer approximation:
    real(rk8):: gamma1(-nlevsno+1:0)               ! two-stream coefficient from Toon et al. (lyr) [unitless]
    real(rk8):: gamma2(-nlevsno+1:0)               ! two-stream coefficient from Toon et al. (lyr) [unitless]
    real(rk8):: gamma3(-nlevsno+1:0)               ! two-stream coefficient from Toon et al. (lyr) [unitless]
    real(rk8):: gamma4(-nlevsno+1:0)               ! two-stream coefficient from Toon et al. (lyr) [unitless]
    real(rk8):: lambda(-nlevsno+1:0)               ! two-stream coefficient from Toon et al. (lyr) [unitless]
    real(rk8):: GAMMA(-nlevsno+1:0)                ! two-stream coefficient from Toon et al. (lyr) [unitless]
    real(rk8):: mu_one                             ! two-stream coefficient from Toon et al. (lyr) [unitless]
    real(rk8):: e1(-nlevsno+1:0)                   ! tri-diag intermediate variable from Toon et al. (lyr) 
    real(rk8):: e2(-nlevsno+1:0)                   ! tri-diag intermediate variable from Toon et al. (lyr) 
    real(rk8):: e3(-nlevsno+1:0)                   ! tri-diag intermediate variable from Toon et al. (lyr) 
    real(rk8):: e4(-nlevsno+1:0)                   ! tri-diag intermediate variable from Toon et al. (lyr) 
    real(rk8):: C_pls_btm(-nlevsno+1:0)            ! intermediate variable: upward flux at bottom interface (lyr) [W/m2]
    real(rk8):: C_mns_btm(-nlevsno+1:0)            ! intermediate variable: downward flux at bottom interface (lyr) [W/m2]
    real(rk8):: C_pls_top(-nlevsno+1:0)            ! intermediate variable: upward flux at top interface (lyr) [W/m2]
    real(rk8):: C_mns_top(-nlevsno+1:0)            ! intermediate variable: downward flux at top interface (lyr) [W/m2]
    real(rk8):: A(-2*nlevsno+1:0)                  ! tri-diag intermediate variable from Toon et al. (2*lyr)
    real(rk8):: B(-2*nlevsno+1:0)                  ! tri-diag intermediate variable from Toon et al. (2*lyr)
    real(rk8):: D(-2*nlevsno+1:0)                  ! tri-diag intermediate variable from Toon et al. (2*lyr)
    real(rk8):: E(-2*nlevsno+1:0)                  ! tri-diag intermediate variable from Toon et al. (2*lyr)
    real(rk8):: AS(-2*nlevsno+1:0)                 ! tri-diag intermediate variable from Toon et al. (2*lyr)
    real(rk8):: DS(-2*nlevsno+1:0)                 ! tri-diag intermediate variable from Toon et al. (2*lyr)
    real(rk8):: X(-2*nlevsno+1:0)                  ! tri-diag intermediate variable from Toon et al. (2*lyr)
    real(rk8):: Y(-2*nlevsno+1:0)                  ! tri-diag intermediate variable from Toon et al. (2*lyr)
    ...
    do fc = 1,num_nourbanc
       ...
       if ((coszen(c_idx) > 0.D0) .and. (h2osno_lcl > min_snw)) then     
          ...
          do i=snl_top,snl_btm,1
             if ((snw_rds_lcl(i) < snw_rds_min_tbl) .or. (snw_rds_lcl(i) > snw_rds_max_tbl)) then
                write (stderr,*) "SNICAR ERROR: snow grain radius of ", snw_rds_lcl(i), " out of bounds."
                write (stderr,*) "NSTEP= ", nstep
                write (stderr,*) "flg_snw_ice= ", flg_snw_ice
                write (stderr,*) "column: ", c_idx, " level: ", i, " snl(c)= ", snl_lcl
          ...
          if (numrad_snw==3) then
             ...
             if (flg_slr_in == 1) then
                flx_wgt(1) = 1.D0
                flx_wgt(2) = 0.66628670195247D0
                flx_wgt(3) = 0.33371329804753D0
             ! Diffuse:
             elseif (flg_slr_in == 2) then
                flx_wgt(1) = 1.D0
                flx_wgt(2) = 0.77887652162877D0
                flx_wgt(3) = 0.22112347837123D0
             endif
          ...
          elseif(numrad_snw==5) then
             ...
             if (flg_slr_in == 1) then
                flx_wgt(1) = 1.D0
                flx_wgt(2) = 0.49352158521175D0
                flx_wgt(3) = 0.18099494230665D0
                flx_wgt(4) = 0.12094898498813D0
                flx_wgt(5) = 0.20453448749347D0
             ! Diffuse:
             elseif (flg_slr_in == 2) then
                flx_wgt(1) = 1.D0
                flx_wgt(2) = 0.58581507618433D0
                flx_wgt(3) = 0.20156903770812D0
                flx_wgt(4) = 0.10917889346386D0
                flx_wgt(5) = 0.10343699264369D0
             endif
          endif
          ...
          do bnd_idx = 1,numrad_snw
             ...
             do while (flg_dover > 0)
                ...
                if (flg_slr_in == 1) then
                   flx_slrd_lcl(bnd_idx) = 1.D0/(mu_not*pi) ! this corresponds to incident irradiance of 1.0
                   flx_slri_lcl(bnd_idx) = 0.D0
                else
                   flx_slrd_lcl(bnd_idx) = 0.D0
                   flx_slri_lcl(bnd_idx) = 1.D0
                endif
                ...
                ! (should make condition more generic than "1.D0")
                if (F_abs_sum > 1.D0) then
                   trip = 1
                endif
                ...
                elseif((trip == 1).and.(flg_dover == 4).and.(err_idx >= 20)) then
                   flg_dover = 0
                   write(stderr,*) "SNICAR ERROR: FOUND A WORMHOLE. STUCK IN INFINITE LOOP! Called from: ", flg_snw_ice
                   write(stderr,*) "SNICAR STATS: snw_rds(0)= ", snw_rds(c_idx,0)
                   write(stderr,*) "SNICAR STATS: L_snw(0)= ", L_snw(0)
                   write(stderr,*) "SNICAR STATS: h2osno= ", h2osno_lcl, " snl= ", snl_lcl
             ...
             if (abs(energy_sum) > 0.00001D0) then
                write (stderr,"(a,e12.6,a,i6,a,i6)") "SNICAR ERROR: Energy conservation error of : ", energy_sum, &
             ...
             if (albout_lcl(bnd_idx) > 1.0) then
                ...
                write (stderr,*) "SNICAR ERROR: Albedo > 1.0 at c: ", c_idx, " NSTEP= ",nstep
                write (stderr,*) "SNICAR STATS: bnd_idx= ",bnd_idx
                write (stderr,*) "SNICAR STATS: albout_lcl(bnd)= ",albout_lcl(bnd_idx), " albsfc_lcl(bnd_idx)= ",albsfc_lcl(bnd_idx)
                write (stderr,*) "SNICAR STATS: landtype= ", sfctype
                write (stderr,*) "SNICAR STATS: h2osno= ", h2osno_lcl, " snl= ", snl_lcl
                write (stderr,*) "SNICAR STATS: coszen= ", coszen(c_idx), " flg_slr= ", flg_slr_in
       ...
       elseif ( (coszen(c_idx) > 0.D0) .and. (h2osno_lcl < min_snw) .and. (h2osno_lcl > 0.D0) ) then
...
! !INTERFACE:
  subroutine SnowAge_grain(lbc, ubc, num_snowc, filter_snowc, num_nosnowc, filter_nosnowc)
    ...
    real(rk8), pointer :: t_soisno(:,:)         ! soil and snow temperature (col,lyr) [K]
    integer,  pointer :: snl(:)                ! negative number of snow layers (col) [nbr]
    real(rk8), pointer :: t_grnd(:)             ! ground temperature (col) [K]
    real(rk8), pointer :: dz(:,:)               ! layer thickness (col,lyr) [m]
    real(rk8), pointer :: h2osno(:)             ! snow water (col) [mm H2O]
    real(rk8), pointer :: snw_rds(:,:)          ! effective grain radius (col,lyr) [microns, m-6]
    real(rk8), pointer :: snw_rds_top(:)        ! effective grain radius, top layer (col) [microns, m-6]
    real(rk8), pointer :: sno_liq_top(:)        ! liquid water fraction (mass) in top snow layer (col) [frc]
    real(rk8), pointer :: h2osoi_liq(:,:)       ! liquid water content (col,lyr) [kg m-2]
    real(rk8), pointer :: h2osoi_ice(:,:)       ! ice content (col,lyr) [kg m-2]
    ...
    real(rk8), pointer :: qflx_snow_grnd_col(:) ! snow on ground after interception (col) [kg m-2 s-1]
    real(rk8), pointer :: qflx_snwcp_ice(:)     ! excess precipitation due to snow capping [kg m-2 s-1]
    real(rk8), pointer :: qflx_snofrz_lyr(:,:)  ! snow freezing rate (col,lyr) [kg m-2 s-1]
    ...
    real(rk8) :: t_snotop                    ! temperature at upper layer boundary [K]
    real(rk8) :: t_snobtm                    ! temperature at lower layer boundary [K]
    real(rk8) :: dTdz(lbc:ubc,-nlevsno:0)    ! snow temperature gradient (col,lyr) [K m-1]
    real(rk8) :: bst_tau                     ! snow aging parameter retrieved from lookup table [hour]
    real(rk8) :: bst_kappa                   ! snow aging parameter retrieved from lookup table [unitless]
    real(rk8) :: bst_drdt0                   ! snow aging parameter retrieved from lookup table [um hr-1]
    real(rk8) :: dr                          ! incremental change in snow effective radius [um]
    real(rk8) :: dr_wet                      ! incremental change in snow effective radius from wet growth [um]
    real(rk8) :: dr_fresh                    ! difference between fresh snow r_e and current r_e [um]
    ...
    real(rk8) :: frc_oldsnow                 ! fraction of layer mass that is old snow [frc]
    real(rk8) :: frc_refrz                   ! fraction of layer mass that is re-frozen snow [frc]
    real(rk8) :: frc_liq                     ! fraction of layer mass that is liquid water[frc]    
    real(rk8) :: dtime                       ! land model time step [sec]
    real(rk8) :: rhos                        ! snow density [kg m-3]
    real(rk8) :: h2osno_lyr                  ! liquid + solid H2O in snow layer [kg m-2]
    real(rk8) :: cdz(-nlevsno+1:0)           ! column average layer thickness [m]
!--------------------------------------------------------------------------!
    ! Assign local pointers to derived subtypes components (column-level)
    ...
    do fc = 1, num_snowc
       ...
       do i=snl_top,snl_btm,1
          ...
          frc_liq = min(0.1D0, (h2osoi_liq(c_idx,i) / (h2osoi_liq(c_idx,i)+h2osoi_ice(c_idx,i))))

          !dr_wet = 1E6D0*(dtime*(C1_liq_Brun89 + C2_liq_Brun89*(frc_liq**(3))) / (4*rpi*(snw_rds(c_idx,i)/1E6)**(2)))
          !simplified, units of microns:
          dr_wet = 1D18*(dtime*(C2_liq_Brun89*(frc_liq**(3))) / &
                  (4*rpi*snw_rds(c_idx,i)**(2)))
          ...
          if (do_capsnow(c_idx)) then
             newsnow = max(0.D0, (qflx_snwcp_ice(c_idx)*dtime))
          else
             newsnow = max(0.D0, (qflx_snow_grnd_col(c_idx)*dtime))
          endif

          ! snow that has re-frozen [kg/m2]
          refrzsnow = max(0.D0, (qflx_snofrz_lyr(c_idx,i)*dtime))
          ...
          if ((frc_refrz + frc_newsnow) > 1.D0) then
             frc_refrz = frc_refrz / (frc_refrz + frc_newsnow)
             frc_newsnow = 1.D0 - frc_refrz
             frc_oldsnow = 0.D0
          else
             frc_oldsnow = 1.D0 - frc_refrz - frc_newsnow
          endif
  ...
  subroutine SnowOptics_init( )
    ...
    if (myid == italk) then
       ...
       write (stdout,*) 'SNICAR: Mie single scatter albedos for direct-beam ice, rds=100um: ', &
            ss_alb_snw_drc(71,1), ss_alb_snw_drc(71,2), ss_alb_snw_drc(71,3),     &
            ss_alb_snw_drc(71,4), ss_alb_snw_drc(71,5)
       write (stdout,*) 'SNICAR: Mie single scatter albedos for diffuse ice, rds=100um: ',     &
            ss_alb_snw_dfs(71,1), ss_alb_snw_dfs(71,2), ss_alb_snw_dfs(71,3),     &
            ss_alb_snw_dfs(71,4), ss_alb_snw_dfs(71,5)
       if (DO_SNO_OC) then
          write (stdout,*) 'SNICAR: Including OC aerosols from snow radiative transfer calculations'
       else
          write (stdout,*) 'SNICAR: Excluding OC aerosols from snow radiative transfer calculations'
       endif
       write (stdout,*) 'SNICAR: Mie single scatter albedos for hydrophillic BC: ', &
            ss_alb_bc1(1), ss_alb_bc1(2), ss_alb_bc1(3), ss_alb_bc1(4), ss_alb_bc1(5)
       write (stdout,*) 'SNICAR: Mie single scatter albedos for hydrophobic BC: ', &
            ss_alb_bc2(1), ss_alb_bc2(2), ss_alb_bc2(3), ss_alb_bc2(4), ss_alb_bc2(5)
       if (DO_SNO_OC) then
          write (stdout,*) 'SNICAR: Mie single scatter albedos for hydrophillic OC: ', &
               ss_alb_oc1(1), ss_alb_oc1(2), ss_alb_oc1(3), ss_alb_oc1(4), ss_alb_oc1(5)
          write (stdout,*) 'SNICAR: Mie single scatter albedos for hydrophobic OC: ', &
               ss_alb_oc2(1), ss_alb_oc2(2), ss_alb_oc2(3), ss_alb_oc2(4), ss_alb_oc2(5)
       endif
       write (stdout,*) 'SNICAR: Mie single scatter albedos for dust species 1: ', &
            ss_alb_dst1(1), ss_alb_dst1(2), ss_alb_dst1(3), ss_alb_dst1(4), ss_alb_dst1(5)
       write (stdout,*) 'SNICAR: Mie single scatter albedos for dust species 2: ', &
            ss_alb_dst2(1), ss_alb_dst2(2), ss_alb_dst2(3), ss_alb_dst2(4), ss_alb_dst2(5)
       write (stdout,*) 'SNICAR: Mie single scatter albedos for dust species 3: ', &
            ss_alb_dst3(1), ss_alb_dst3(2), ss_alb_dst3(3), ss_alb_dst3(4), ss_alb_dst3(5)
       write (stdout,*) 'SNICAR: Mie single scatter albedos for dust species 4: ', &
  ...
  subroutine SnowAge_init( )
   ...
   if (myid == italk) &
     write(stdout,*) subname,trim(fsnowaging)
   ...
   if (myid == italk) then
      ...
      ! print some diagnostics:
      write (stdout,*) 'SNICAR: snowage tau for T=263K, dTdz = 100 K/m, rhos = 150 kg/m3: ', snowage_tau(3,11,9)
      write (stdout,*) 'SNICAR: snowage kappa for T=263K, dTdz = 100 K/m, rhos = 150 kg/m3: ', snowage_kappa(3,11,9)
      write (stdout,*) 'SNICAR: snowage dr/dt_0 for T=263K, dTdz = 100 K/m, rhos = 150 kg/m3: ', snowage_drdt0(3,11,9)
   endif
