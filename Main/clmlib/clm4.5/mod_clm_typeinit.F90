...
! !ARGUMENTS:
  ...
  subroutine init_pft_pstate_type(ibeg,iend,pps)
    ...
    if ( crop_prog ) then
      ...
      allocate(pps%cropplant(ibeg:iend), source=.false.)
      allocate(pps%harvdate(ibeg:iend), source=bigint)
    ...
    allocate(pps%lfpftd(ibeg:iend), source=nan)      !F. Li and S. Levis
  ...
  subroutine init_pft_cflux_type(ibeg,iend,pcf)
    ...
    allocate(pcf%m_livestemc_storage_to_fire(ibeg:iend), source=nan)
  ...
  subroutine init_column_pstate_type(ibeg,iend,cps)
    ...
    allocate(cps%snl(ibeg:iend), source=bigint)      !* cannot be averaged up
    ...
    allocate(cps%frac_iceold(ibeg:iend,-nlevsno+1:nlevgrnd), source=spval)
    ...
    allocate(cps%wf2(ibeg:iend), source=nan)
!   allocate(cps%xirrig(ibeg:iend), source=0.0_rk8)
    allocate(cps%max_dayl(ibeg:iend), source=0.0_rk8)
    ...
    allocate(cps%savedtke1(ibeg:iend), source=nan)
  ...
  subroutine init_column_ch4_type(ibeg,iend,cch4)
    ...
    allocate(cch4%fphr(ibeg:iend,1:nlevgrnd), source=nan)
  ...
  subroutine init_column_nflux_type(ibeg,iend,cnf)
    ...
    allocate(cnf%harvest_n_to_litr_lig_n(ibeg:iend, &
                 1:nlevdecomp_full), source=nan)
    allocate(cnf%harvest_n_to_cwdn(ibeg:iend, 1:nlevdecomp_full), source=nan)
    ...
    allocate(cnf%sminn_to_denit_decomp_cascade(ibeg:iend, &
            1:ndecomp_cascade_transitions), source=nan)
    allocate(cnf%sminn_to_denit_excess_vr(ibeg:iend, &
            1:nlevdecomp_full), source=nan)
  ...
  subroutine init_gridcell_wflux_type(ibeg,iend,gwf)
    ...
    allocate(gwf%qflx_runoffg(ibeg:iend), source=0._rk8)
    allocate(gwf%qflx_snwcp_iceg(ibeg:iend), source=0._rk8)
