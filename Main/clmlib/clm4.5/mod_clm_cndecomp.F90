...
! !USES:
   ...
   use mod_clm_varpar   , only: nlevsoi,nlevgrnd,nlevdecomp,ndecomp_cascade_transitions,ndecomp_pools
...
!
   ...
   real(rk8), pointer :: decomp_cascade_ctransfer_vr(:,:,:)     ! vertically-resolved het. resp. from decomposing C pools (gC/m3/s)
   real(rk8), pointer :: decomp_pools_hr(:,:)                   ! het. resp. from decomposing C pools (gC/m2/s)
   real(rk8), pointer :: decomp_cascade_ctransfer(:,:)          ! vertically-resolved C transferred along deomposition cascade (gC/m3/s)
   ...
   real(rk8), pointer :: gross_nmin(:)            ! gross rate of N mineralization (gN/m2/s)
   real(rk8), pointer :: net_nmin(:)              ! net rate of N mineralization (gN/m2/s)
   ! For methane code
#ifdef LCH4
   real(rk8), pointer :: fphr(:,:)                ! fraction of potential SOM + LITTER heterotrophic respiration
...
! !OTHER LOCAL VARIABLES:
   ...
   real(rk8):: p_decomp_cpool_loss(lbc:ubc,1:nlevdecomp,1:ndecomp_cascade_transitions) !potential C loss from one pool to another
   real(rk8):: pmnf_decomp_cascade(lbc:ubc,1:nlevdecomp,1:ndecomp_cascade_transitions) !potential mineral N flux, from one pool to another
   real(rk8):: immob(lbc:ubc,1:nlevdecomp)        !potential N immobilization
      ...
      do j = 1,nlevdecomp
         do fc = 1,num_soilc
            ...
            if (decomp_cpools_vr(c,j,cascade_donor_pool(k)) .gt. 0.D0) then
               ...
               decomp_cascade_ctransfer_vr(c,j,k) = (1.D0 - rf_decomp_cascade(c,j,k)) * p_decomp_cpool_loss(c,j,k)
               if (decomp_npools_vr(c,j,cascade_donor_pool(k)) > 0.D0 .and. cascade_receiver_pool(k) .ne. i_atm) then
   ...
   do j = 1,nlevdecomp
      do fc = 1,num_soilc
         ...
         if (phr_vr(c,j) > 0.D0) then
            ...
            fphr(c,j) = max(fphr(c,j), 0.01D0) ! Prevent overflow errors for 0 respiration
         else
            fphr(c,j) = 1.D0
         end if
      end do
