...
The new netCDF output format allow user to use a number of general purpose
...
\begin{enumerate}
\item Only equirectangular projection or Plate Carr\'ee is supported. Some
...
in the \verb=CTL= file, which allows the user specify a supplementary file
...
created a website dedicated to visualizing RegCM3 output using the NCAR Command Language (NCL). While this scripts where built using RegCM3 model output
converted to netCDF using an external converter, they may be easily ported
to use the native netCDF RegCM V4.1 file format.

\begin{Verbatim}
http://es.ucsc.edu/~msnyder/NCL/index.html
\end{Verbatim}

A dedicated page also exist on the NCAR \verb=NCL= site itself:
...
A very basic script under that directory, \verb=terplt.ncl=, may be used
as example on how to plot a variable form a RegCM file using the \verb=NCL=
projection engin to display using the native projected grid.
